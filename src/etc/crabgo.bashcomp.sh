...
_crabgo()
{
	...
	if [[ $cword -gt $dd_i ]]; then
		...
		if [[ "${cmd}" = @(test|bench) ]]; then
			...
			# Fallback to filename completion, useful with `crabgo run`.
			_filedir
		fi
		...
		if [[ "$cur" == -* ]]; then
			...
			_ensure_crabgo_commands_cache_filled
			COMPREPLY=( $( compgen -W "$__crabgo_commands_cache" -- "$cur" ) )
		fi
	else
		case "${prev}" in
			--vcs)
...
complete -F _crabgo crabgo

__crabgo_commands_cache=
_ensure_crabgo_commands_cache_filled(){
	if [[ -z $__crabgo_commands_cache ]]; then
		__crabgo_commands_cache="$(crabgo --list 2>/dev/null | awk 'NR>1 {print $1}')"
	fi
}

_locate_manifest(){
	crabgo locate-project --message-format plain 2>/dev/null
}
