...
use crabgo::util::toml::StringOrVec;
use crabgo::util::CliError;
use crabgo::util::{self, closest_msg, command_prelude, CrabgoResult, CliResult, Config};
use crabgo_util::{ProcessBuilder, ProcessError};
...
fn main() {
    ...
    let result = if let Some(lock_addr) = crabgo::ops::fix_get_proxy_lock_addr() {
        crabgo::ops::fix_exec_rustc(config.get(), &lock_addr).map_err(|e| CliError::from(e))
    } else {
        let _token = crabgo::util::job::setup();
        cli::main(&mut config)
    };

    match result {
        Err(e) => crabgo::exit_with_error(e, &mut config.get_mut().shell()),
        Ok(()) => {}
    }
}

/// Table for defining the aliases which come builtin in `Crabgo`.
...
fn aliased_command(config: &Config, command: &str) -> CrabgoResult<Option<Vec<String>>> {
...
fn list_commands(config: &Config) -> BTreeMap<String, CommandInfo> {
    ...
    commands.insert(
        "help".to_string(),
        CommandInfo::BuiltIn {
            about: Some("Displays help for a crabgo subcommand".to_string()),
        },
    );
...
fn find_external_subcommand(config: &Config, cmd: &str) -> Option<PathBuf> {
    let command_exe = format!("crabgo-{}{}", cmd, env::consts::EXE_SUFFIX);
    ...
    let command = match path {
        ...
        None => {
            let err = if cmd.starts_with('+') {
                anyhow::format_err!(
                    "no such command: `{}`\n\n\t\
                    Crabgo does not handle `+toolchain` directives.\n\t\
                    Did you mean to invoke `crabgo` through `rustup` instead?",
                    cmd
                )
            } else {
                ...
                anyhow::format_err!(
                    "no such command: `{}`{}\n\n\t\
                    View all installed commands with `crabgo --list`",
                    cmd,
                    did_you_mean
                )
            };
...
fn execute_subcommand(config: &Config, cmd_path: Option<&PathBuf>, args: &[&OsStr]) -> CliResult {
    let crabgo_exe = config.crabgo_exe()?;
...
fn search_directories(config: &Config) -> Vec<PathBuf> {
    ...
    // See https://github.com/rust-lang/crabgo/issues/11020 for details.
    //
...
fn init_git(config: &Config) {
    ...
    //
    // Note that we only disable this when Crabgo is run as a binary. If Crabgo is used as a library,
...
fn init_git_transports(config: &Config) {
    ...
    match crabgo::ops::needs_custom_http_transport(config) {
        Ok(true) => {}
        _ => return,
    }

    let handle = match crabgo::ops::http_handle(config) {
        Ok(handle) => handle,
        Err(..) => return,
    };
