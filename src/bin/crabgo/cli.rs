...
use crabgo::core::shell::Shell;
use crabgo::core::{features, CliUnstable};
use crabgo::{self, drop_print, drop_println, CliResult, Config};
...
use crabgo::core::features::HIDDEN;

lazy_static::lazy_static! {
    // Maps from commonly known external commands (not builtin to crabgo) to their
...
To pass the arguments to the subcommand, remove `--`",
            ...
            (None, Ok(Some(alias))) => {
                ...
                // (binary of the form `crabgo-<subcommand>`)
...
pub fn cli() -> Command {
    ...
    Command::new("crabgo")
        ...
        // Doesn't mix well with our list of common crabgo commands.  See clap-rs/clap#3108 for
...
Some common crabgo commands are (see all commands with --list):
    ...
    update      Update dependencies listed in Crabgo.lock
    search      Search registry for crates
    publish     Package and upload this package to the registry
    install     Install a Rust binary. Default location is $HOME/.crabgo/bin
    ...
    pub fn get_mut(&mut self) -> &mut Config {
        self.config.get_or_insert_with(|| match Config::default() {
            ...
            Err(e) => {
                ...
                crabgo::exit_with_error(e.into(), &mut shell)
            }
        })
    }
}
