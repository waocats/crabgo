...
pub fn cli() -> Command {
    subcommand("fix")
        ...
        .arg_timings()
        .after_help("Run `crabgo help fix` for more detailed information.\n")
}

pub fn exec(config: &mut Config, args: &ArgMatches) -> CliResult {
    let ws = args.workspace(config)?;
    // This is a legacy behavior that causes `crabgo fix` to pass `--test`.
    let test = matches!(
    ...
    // Unlike other commands default `crabgo fix` to all targets to fix as much
    // code as we can.
    let mut opts =
        args.compile_options(config, mode, Some(&ws), ProfileChecking::LegacyTestOnly)?;

    if !opts.filter.is_specific() {
        // crabgo fix with no target selection implies `--all-targets`.
