...
use crabgo::core::Verbosity;
use crabgo::ops::{self, CompileFilter, Packages};
...
pub fn cli() -> Command {
    subcommand("run")
        ...
        .arg_timings()
        .after_help("Run `crabgo help run` for more detailed information.\n")
}
    ...
    if let Packages::Packages(opt_in) = &compile_opts.spec {
        if let Some(pattern) = opt_in.iter().find(|s| is_glob_pattern(s)) {
            return Err(anyhow::anyhow!(
                "`crabgo run` does not support glob pattern `{}` on package selection",
                pattern,
            )
            .into());
        }
