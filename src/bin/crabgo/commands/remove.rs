...
use crabgo::core::PackageIdSpec;
use crabgo::core::Workspace;
use crabgo::ops::crabgo_remove::remove;
use crabgo::ops::crabgo_remove::RemoveOptions;
use crabgo::ops::resolve_ws;
use crabgo::util::command_prelude::*;
use crabgo::util::print_available_packages;
use crabgo::util::toml_mut::dependency::Dependency;
use crabgo::util::toml_mut::dependency::MaybeWorkspace;
use crabgo::util::toml_mut::dependency::Source;
use crabgo::util::toml_mut::manifest::DepTable;
use crabgo::util::toml_mut::manifest::LocalManifest;
...
pub fn cli() -> clap::Command {
    clap::Command::new("remove")
        ...
        // .alias("rm")
        .about("Remove dependencies from a Crabgo.toml manifest file")
...
pub fn exec(config: &mut Config, args: &ArgMatches) -> CliResult {
    ...
    let spec = match packages.len() {
        ...
        _ => {
            ...
            return Err(CliError::new(
                anyhow::format_err!(
                    "`crabgo remove` could not determine which package to modify. \
...
fn gc_workspace(workspace: &Workspace<'_>) -> CrabgoResult<()> {
    let mut manifest: toml_edit::Document =
        crabgo_util::paths::read(workspace.root_manifest())?.parse()?;
    ...
    if is_modified {
        crabgo_util::paths::write(workspace.root_manifest(), manifest.to_string().as_bytes())?;
    }
