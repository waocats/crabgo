use crabgo::sources::CRATES_IO_REGISTRY;
use crabgo::util::print_available_packages;
use indexmap::IndexMap;
use indexmap::IndexSet;

use crabgo::core::dependency::DepKind;
use crabgo::core::FeatureValue;
use crabgo::ops::crabgo_add::add;
use crabgo::ops::crabgo_add::AddOptions;
use crabgo::ops::crabgo_add::DepOp;
use crabgo::ops::resolve_ws;
use crabgo::util::command_prelude::*;
use crabgo::util::interning::InternedString;
use crabgo::util::toml_mut::manifest::DepTable;
use crabgo::CrabgoResult;

pub fn cli() -> Command {
    clap::Command::new("add")
        .about("Add dependencies to a Crabgo.toml manifest file")
        .override_usage(
            "\
       crabgo add [OPTIONS] <DEP>[@<VERSION>] ...
       crabgo add [OPTIONS] --path <PATH> ...
       crabgo add [OPTIONS] --git <URL> ..."
        )
        .after_help("Run `crabgo help add` for more detailed information.\n")
...
- `<name>`, like `crabgo add serde` (latest version will be used)
- `<name>@<version-req>`, like `crabgo add serde@1` or `crabgo add serde@=1.0.38`"
            )
                .group("selected"),
...
Without any other information, crabgo will use latest commit on the main branch.")
                .group("selected"),
            clap::Arg::new("branch")
                .long("branch")
                .action(ArgAction::Set)
...
pub fn exec(config: &mut Config, args: &ArgMatches) -> CliResult {
    ...
    let spec = match packages.len() {
        ...
        _ => {
            ...
            return Err(CliError::new(
                anyhow::format_err!(
                    "`crabgo add` could not determine which package to modify. \
...
fn parse_dependencies(config: &Config, matches: &ArgMatches) -> CrabgoResult<Vec<DepOp>> {
