...
use anyhow::bail;
use crabgo::{drop_println, CrabgoResult};
use serde::Serialize;

pub fn cli() -> Command {
    subcommand("locate-project")
        .about("Print a JSON representation of a Crabgo.toml file's location")
        .arg_quiet()
        .arg_manifest_path()
        .arg(
            opt(
        ...
        .arg(flag("workspace", "Locate Crabgo.toml of the workspace root"))
        .after_help("Run `crabgo help locate-project` for more detailed information.\n")
}
...
impl MessageFormat {
    fn parse(args: &ArgMatches) -> CrabgoResult<Self> {
