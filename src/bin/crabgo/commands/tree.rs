...
use crabgo::ops::tree::{self, EdgeKind};
use crabgo::ops::Packages;
use crabgo::util::print_available_packages;
use crabgo::util::CrabgoResult;
...
pub fn cli() -> Command {
    subcommand("tree")
        ...
        .arg(
            // Backwards compatibility with old crabgo-tree.
            flag("version", "Print version info and exit")
                .short('V')
                .hide(true),
        )
        .after_help("Run `crabgo help tree` for more detailed information.\n")
}
...
pub fn exec(config: &mut Config, args: &ArgMatches) -> CliResult {
    if args.flag("version") {
        ...
        crabgo::drop_print!(config, "{}", version);
        return Ok(());
    }
    ...
    if args.flag("all") {
        config.shell().warn(
            "The `crabgo tree` --all flag has been changed to --no-dedupe, \
    ...
    if args.is_present_with_zero_values("invert") {
        match &packages {
            Packages::Packages(ps) => {
                // Backwards compatibility with old syntax of `crabgo tree -i -p foo`.
...
fn parse_edge_kinds(config: &Config, args: &ArgMatches) -> CrabgoResult<(HashSet<EdgeKind>, bool)> {
