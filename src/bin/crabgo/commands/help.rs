...
use crabgo::util::errors::CrabgoResult;
use crabgo::{drop_println, Config};
use crabgo_util::paths::resolve_executable;
...
pub fn cli() -> Command {
    subcommand("help")
        .about("Displays help for a crabgo subcommand")
...
fn try_help(config: &Config, subcommand: &str) -> CrabgoResult<bool> {
...
fn extract_man(subcommand: &str, extension: &str) -> Option<Vec<u8>> {
    let extract_name = OsString::from(format!("crabgo-{}.{}", subcommand, extension));
...
/// display it.
fn write_and_spawn(name: &str, contents: &[u8], command: &str) -> CrabgoResult<()> {
    let prefix = format!("crabgo-{}.", name);
