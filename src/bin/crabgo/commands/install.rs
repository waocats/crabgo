...
use crabgo::core::{GitReference, SourceId, Workspace};
use crabgo::ops;
use crabgo::util::IntoUrl;

use crabgo_util::paths;

pub fn cli() -> Command {
    subcommand("install")
        .about("Install a Rust binary. Default location is $HOME/.crabgo/bin")
        .arg_quiet()
        .arg(
            Arg::new("crate")
        ...
        .arg_message_format()
        .arg_timings()
        .after_help("Run `crabgo help install` for more detailed information.\n")
}
...
pub fn exec(config: &mut Config, args: &ArgMatches) -> CliResult {
    ...
    // but in these particular cases we need it to fix rust-lang/crabgo#10283.
    ...
    let krates = args
        ...
        .collect::<crate::CrabgoResult<Vec<_>>>()?;
    ...
    let workspace = if from_cwd {
        ...
        Workspace::new(&path.join("Crabgo.toml"), config).ok()
    } else {
        None
    };
...
) -> crate::CrabgoResult<(&'k str, Option<&'k str>)> {
