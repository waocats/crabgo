...
Experimental Crabgo features are only available on the [nightly channel]. You
...
* New syntax in `Crabgo.toml` requires a `crabgo-features` key at the top of
  `Crabgo.toml`, before any tables. For example:

  ```toml
  # This specifies which new Crabgo.toml features are enabled.
...
* New command-line flags, options, and subcommands require the `-Z
  ...
  ```crabgo +nightly build --out-dir=out -Z unstable-options```
...
* `-Z` command-line flags are used to enable new functionality that may not
  ...
  complex features that affect multiple parts of Crabgo. For example, the
  [mtime-on-use](#mtime-on-use) feature can be enabled with:

  ```crabgo +nightly build -Z mtime-on-use```

  Run `crabgo -Z help` to see a list of flags available.

  Anything which can be configured with a `-Z` flag can also be set in the
  crabgo [config file] (`.crabgo/config.toml`) in the `unstable` table. For
  example:
...
* Resolver and features
    * [no-index-update](#no-index-update) --- Prevents crabgo from updating the index cache.
...
* Information and metadata
    ...
    * [unit-graph](#unit-graph) --- Emits JSON for Crabgo's internal graph structure.
    * [`crabgo rustc --print`](#rustc---print) --- Calls rustc with `--print` to display information from rustc.
* Configuration
    * [config-include](#config-include) --- Adds the ability for config files to include other files.
    * [`crabgo config`](#crabgo-config) --- Adds a new subcommand for viewing config files.
* Registries
...
and `-Zbar` to `crabgo`, but you will be unable to pass `-Zbaz`. You can
...
to the `crabgo-features` entry in `Crabgo.toml`. If, for example, you want
to allow
...
The list of features passed to crabgo's `-Zallow-features` is also passed
to any Rust tools that crabgo ends up calling (like `rustc` or
`rustdoc`). Thus, if you run `crabgo -Zallow-features=`, no unstable
Crabgo _or_ Rust features can be used.

### no-index-update
* Original Issue: [#3479](https://github.com/rust-lang/crabgo/issues/3479)
* Tracking Issue: [#7404](https://github.com/rust-lang/crabgo/issues/7404)

The `-Z no-index-update` flag ensures that Crabgo does not attempt to update
the registry index. This is intended for tools such as Crater that issue many
Crabgo commands, and you want to avoid the network latency for updating the
index each time.

### mtime-on-use
* Original Issue: [#6477](https://github.com/rust-lang/crabgo/pull/6477)
* Cache usage meta tracking issue: [#7150](https://github.com/rust-lang/crabgo/issues/7150)

The `-Z mtime-on-use` flag is an experiment to have Crabgo update the mtime of
used files to make it easier for tools like crabgo-sweep to detect which files
are stale. For many workflows this needs to be set on *all* invocations of crabgo.
To make this more practical setting the `unstable.mtime_on_use` flag in `.crabgo/config.toml`
or the corresponding ENV variable will apply the `-Z mtime-on-use` to all
invocations of nightly crabgo. (the config flag is ignored by stable)

### avoid-dev-deps
* Original Issue: [#4988](https://github.com/rust-lang/crabgo/issues/4988)
* Tracking Issue: [#5133](https://github.com/rust-lang/crabgo/issues/5133)

When running commands such as `crabgo install` or `crabgo build`, Crabgo
currently requires dev-dependencies to be downloaded, even if they are not
used. The `-Z avoid-dev-deps` flag allows Crabgo to avoid downloading
dev-dependencies if they are not needed. The `Crabgo.lock` file will not be
generated if dev-dependencies are skipped.

### minimal-versions
* Original Issue: [#4100](https://github.com/rust-lang/crabgo/issues/4100)
* Tracking Issue: [#5657](https://github.com/rust-lang/crabgo/issues/5657)
...
When a `Crabgo.lock` file is generated, the `-Z minimal-versions` flag will
resolve the dependencies to the minimum SemVer version that will satisfy the
requirements (instead of the greatest version).

The intended use-case of this flag is to check, during continuous integration,
that the versions specified in Crabgo.toml are a correct reflection of the
minimum versions that you are actually using. That is, if Crabgo.toml says
...
When a `Crabgo.lock` file is generated, the `-Z direct-minimal-versions` flag will
resolve the dependencies to the minimum SemVer version that will satisfy the
...
### out-dir
* Original Issue: [#4875](https://github.com/rust-lang/crabgo/issues/4875)
* Tracking Issue: [#6790](https://github.com/rust-lang/crabgo/issues/6790)
...
This can also be specified in `.crabgo/config.toml` files.
...
### doctest-xcompile
* Tracking Issue: [#7040](https://github.com/rust-lang/crabgo/issues/7040)
* Tracking Rustc Issue: [#64245](https://github.com/rust-lang/rust/issues/64245)

This flag changes `crabgo test`'s behavior when handling doctests when
a target is passed. Currently, if a target is passed that is different
from the host crabgo will simply skip testing doctests. If this flag is
present, crabgo will continue as normal, passing the tests to doctest,
while also passing it a `--target` option, as well as enabling
`-Zunstable-features --enable-per-target-ignores` and passing along
information from `.crabgo/config.toml`. See the rustc issue for more information.

```sh
crabgo test --target foo -Zdoctest-xcompile
```

### Build-plan
* Tracking Issue: [#5579](https://github.com/rust-lang/crabgo/issues/5579)
...
```sh
crabgo +nightly build --build-plan -Z unstable-options
```
...
`metabuild` key in `Crabgo.toml`. A build script is automatically generated
that runs each build dependency in order. Metabuild packages can then read
metadata from `Crabgo.toml` to specify their behavior.

Include `crabgo-features` at the top of `Crabgo.toml`, a `metabuild` key in the
...
This requires the appropriate key to be set in `crabgo-features`:
...
### msrv-policy
- [#9930](https://github.com/rust-lang/crabgo/issues/9930) (MSRV-aware resolver)
- [#10653](https://github.com/rust-lang/crabgo/issues/10653) (MSRV-aware crabgo-add)
- [#10903](https://github.com/rust-lang/crabgo/issues/10903) (MSRV-aware crabgo-install)

The `msrv-policy` feature enables experiments in MSRV-aware policy for crabgo in
preparation for an upcoming RFC.

### build-std
* Tracking Repository: <https://github.com/rust-lang/wg-crabgo-std-aware>

The `build-std` feature enables Crabgo to compile the standard library itself as
part of a crate graph compilation. This feature has also historically been known
as "std-aware Crabgo". This feature is still in very early stages of development,
and is also a possible massive feature addition to Crabgo. This is a very large
...
repository](https://github.com/rust-lang/wg-crabgo-std-aware) and its set of
issues.

The functionality implemented today is behind a flag called `-Z build-std`. This
flag indicates that Crabgo should compile the standard library from source code
...
$ crabgo new foo
$ cd foo
$ crabgo +nightly run -Z build-std --target x86_64-unknown-linux-gnu
...
`alloc`, and `proc_macro`. If you're using `crabgo test` it will also compile the
...
```console
$ crabgo +nightly build -Z build-std=core,alloc
```
...
* You must use both a nightly Crabgo and a nightly rustc
* The `-Z build-std` flag must be passed to all `crabgo` invocations.

#### Reporting bugs and helping out

The `-Z build-std` feature is in the very early stages of development! This
feature for Crabgo has an extremely long history and is very large in scope, and
this is just the beginning. If you'd like to report bugs please either report
them to:

* Crabgo --- <https://github.com/rust-lang/crabgo/issues/new> --- for implementation bugs
* The tracking repository ---
  <https://github.com/rust-lang/wg-crabgo-std-aware/issues/new> --- for larger design
  questions.
...
the [issue tracker](https://github.com/rust-lang/wg-crabgo-std-aware/issues) of
...
The `-Z binary-dep-depinfo` flag causes Crabgo to forward the same flag to
`rustc` which will then cause `rustc` to include the paths of all binary
dependencies in the "dep info" file (with the `.d` extension). Crabgo then uses
...
* Original Pull Request: [#7460](https://github.com/rust-lang/crabgo/pull/7460)

The `-Z panic-abort-tests` flag will enable nightly support to compile test
harness crates with `-Cpanic=abort`. Without this flag Crabgo will compile tests,
...
It's currently unclear how this feature will be stabilized in Crabgo, but we'd
like to stabilize it somehow!

[rust-lang/rust#64158]: https://github.com/rust-lang/rust/pull/64158

### keep-going
* Tracking Issue: [#10496](https://github.com/rust-lang/crabgo/issues/10496)

`crabgo build --keep-going` (and similarly for `check`, `test` etc) will build as
...
one of which fails to build, `crabgo check -j1` may or may not build the one that
succeeds (depending on which one of the two builds Crabgo picked to run first),
whereas `crabgo check -j1 --keep-going` would definitely run both builds, even if
...
```console
crabgo check --keep-going -Z unstable-options
```

### config-include
* Tracking Issue: [#7723](https://github.com/rust-lang/crabgo/issues/7723)
...
```console
crabgo +nightly -Zunstable-options -Zconfig-include --config somefile.toml build
```

CLI paths are relative to the current working directory.

### target-applies-to-host
* Original Pull Request: [#9322](https://github.com/rust-lang/crabgo/pull/9322)
* Tracking Issue: [#9453](https://github.com/rust-lang/crabgo/issues/9453)

Historically, Crabgo's behavior for whether the `linker` and `rustflags`
...
When `--target` is _not_ passed, Crabgo respects the same `linker` and
`rustflags` for build scripts as for all other compile artifacts. When
`--target` _is_ passed, however, Crabgo respects `linker` from
...
`target-applies-to-host` setting in Crabgo configuration files which
...
`true`, in the configuration file, the existing Crabgo behavior is
...
whether `--target` is passed to Crabgo. To customize artifacts intended
...
```console
crabgo +nightly -Ztarget-applies-to-host build --target x86_64-unknown-linux-gnu
```

### host-config
* Original Pull Request: [#9322](https://github.com/rust-lang/crabgo/pull/9322)
* Tracking Issue: [#9452](https://github.com/rust-lang/crabgo/issues/9452)
...
false` is set in the Crabgo configuration file.
...
```console
crabgo +nightly -Ztarget-applies-to-host -Zhost-config build --target x86_64-unknown-linux-gnu
```

### unit-graph
* Tracking Issue: [#8002](https://github.com/rust-lang/crabgo/issues/8002)
...
represents Crabgo's internal unit graph. Nothing is actually built, and the
...
```
crabgo +nightly build --unit-graph -Z unstable-options
```

This structure provides a more complete view of the dependency relationship as
Crabgo sees it. In particular, the "features" field supports the new feature
resolver where a dependency can be built multiple times with different
features. `crabgo metadata` fundamentally cannot represent the relationship of
  ...
  "units": [
    {
      /* An opaque string which indicates the package.
         Information about the package can be obtained from `crabgo metadata`.
      */
      "pkg_id": "my-package 0.1.0 (path+file:///path/to/my-package)",
      /* The Crabgo target. See the `crabgo metadata` documentation for more
         information about these fields.
         https://doc.rust-lang.org/crabgo/commands/crabgo-metadata.html
      */
      "target": {
        "kind": ["lib"],
        "crate_types": ["lib"],
...
* Original Issue: [rust-lang/crabgo#7878](https://github.com/rust-lang/crabgo/issues/7878)
* Tracking Issue: [rust-lang/crabgo#10271](https://github.com/rust-lang/crabgo/issues/10271)
...
To set this in a profile in Crabgo configuration, you need to use either
...
### rustdoc-map
* Tracking Issue: [#8296](https://github.com/rust-lang/crabgo/issues/8296)
...
when the dependency is not documented. First, add this to `.crabgo/config`:
...
```
crabgo +nightly doc --no-deps -Zrustdoc-map
```
...
replaced with the corresponding values. If neither are specified, then Crabgo
...
### per-package-target
* Tracking Issue: [#9406](https://github.com/rust-lang/crabgo/pull/9406)
* Original Pull Request: [#9030](https://github.com/rust-lang/crabgo/pull/9030)
* Original Issue: [#7004](https://github.com/rust-lang/crabgo/pull/7004)
...
* Tracking Issue: [#9096](https://github.com/rust-lang/crabgo/pull/9096)
* Original Pull Request: [#9992](https://github.com/rust-lang/crabgo/pull/9992)

Allow Crabgo packages to depend on `bin`, `cdylib`, and `staticlib` crates,
and use the artifacts built by those crates at compile time.

Run `crabgo` with `-Z bindeps` to enable this functionality.

**Example:** use _cdylib_ artifact in build script

The `Crabgo.toml` in the consuming package, building the `bar` library as `cdylib`
...
```rust
fn main() {
  wasm::run_file(std::env::var("CRABGO_CDYLIB_FILE_BAR").unwrap());
}
```

**Example:** use _binary_ artifact and its library in a binary

The `Crabgo.toml` in the consuming package, building the `bar` binary for inclusion
...
fn main() {
  bar::init();
  command::run(env!("CRABGO_BIN_FILE_BAR"));
}
```

### publish-timeout
* Tracking Issue: [11222](https://github.com/rust-lang/crabgo/issues/11222)

The `publish.timeout` key in a config file can be used to control how long
`crabgo publish` waits between posting a package to the registry and it being
...
### registry-auth
* Tracking Issue: [10474](https://github.com/rust-lang/crabgo/issues/10474)
* RFC: [#3139](https://github.com/rust-lang/rfcs/pull/3139)

Enables Crabgo to include the authorization token for API requests, crate downloads
...
`config.json`, and you must pass the `-Z registry-auth` flag on the Crabgo command line.

When using the sparse protocol, Crabgo will attempt to fetch the `config.json` file before
fetching any other files. If the server responds with an HTTP 401, then Crabgo will assume
...
`WWW-Authenticate` header with a `Crabgo login_url` challenge to indicate where the user
can go to get a token.

```
WWW-Authenticate: Crabgo login_url="https://test-registry-login/me
```

This same flag is also used to enable asymmetric authentication tokens.
* Tracking Issue: [10519](https://github.com/rust-lang/crabgo/issues/10519)
* RFC: [#3231](https://github.com/rust-lang/rfcs/pull/3231)

Add support for Crabgo to authenticate the user to registries without sending secrets over the network.
...
A keypair can be generated with `crabgo login --generate-keypair` which will:
...
It is intended for the rare use cases like "cryptographic proof that the central CA server authorized this action". Crabgo requires it to be non-whitespace printable ASCII. Registries that need non-ASCII data should base64 encode it.

Both fields can be set with `crabgo login --registry=name --private-key --private-key-subject="subject"` which will prompt you to put in the key value.

A registry can have at most one of `private-key`, `token`, or `credential-process` set.

All PASETOs will include `iat`, the current time in ISO 8601 format. Crabgo will include the following where appropriate:
...
- `url` the RFC 3986 compliant URL where crabgo got the config.json file,
  - If this is a registry with an HTTP index, then this is the base URL that all index queries are relative to.
  - If this is a registry with a GIT index, it is the URL Crabgo used to clone the index.
...
### credential-process
* Tracking Issue: [#8933](https://github.com/rust-lang/crabgo/issues/8933)
...
Token authentication is used by the [`crabgo login`], [`crabgo publish`],
[`crabgo owner`], [`crabgo yank`], and [`crabgo logout`] commands.
...
in the [`credentials.toml` file] (which can be done with the [`crabgo logout`] command).
...
```toml
[registry]
credential-process = "/usr/bin/crabgo-creds"
```
...
Process names with the prefix `crabgo:` are loaded from the `libexec` directory
next to crabgo. Several experimental credential wrappers are included with
Crabgo, and this provides convenient access to them:

```toml
[registry]
credential-process = "crabgo:macos-keychain"
```

The current wrappers are:

* `crabgo:macos-keychain`: Uses the macOS Keychain to store the token.
* `crabgo:wincred`: Uses the Windows Credential Manager to store the token.
* `crabgo:1password`: Uses the 1password `op` CLI to store the token. You must
...
libsecret-1-dev`). Then build and install the wrapper with `crabgo install
crabgo-credential-gnome-secret`.
In the config, use a path to the binary like this:

```toml
[registry]
credential-process = "crabgo-credential-gnome-secret {action}"
```

#### `credential-process` Interface

There are two different kinds of token processes that Crabgo supports. The
simple "basic" kind will only be called by Crabgo when it needs a token. This
is intended for simple and easy integration with password managers, that can
often use pre-existing tooling. The more advanced "Crabgo" kind supports
different actions passed as a command-line argument. This is intended for more
pleasant integration experience, at the expense of requiring a Crabgo-specific
process to glue to the password manager. Crabgo will determine which kind is
...
With this form, [`crabgo login`] and [`crabgo logout`] are not supported and
return an error if used.

##### Crabgo authenticator

The protocol between the Crabgo and the process is very basic, intended to
ensure the credential process is kept as simple as possible. Crabgo will
...
The `crabgo login` command uses `store` to save a token. Commands that require
authentication, like `crabgo publish`, uses `get` to retrieve a token. `crabgo
...
* `CRABGO` --- Path to the `crabgo` binary executing the command.
* `CRABGO_REGISTRY_INDEX_URL` --- The URL of the registry index.
* `CRABGO_REGISTRY_NAME_OPT` --- Optional name of the registry. Should not be used as a storage key. Not always available.
...
* Original Issue: [#2362](https://github.com/rust-lang/crabgo/issues/2362)
* Tracking Issue: [#9301](https://github.com/rust-lang/crabgo/issues/9301)

The `crabgo config` subcommand provides a way to display the configuration
files that crabgo loads. It currently includes the `get` subcommand which
can take an optional config value to display.

```console
crabgo +nightly -Zunstable-options config get build.rustflags
```
...
* Tracking Issue: [#9427](https://github.com/rust-lang/crabgo/issues/9427)

The `-Z doctest-in-workspace` flag changes the behavior of the current working
directory used when running doctests. Historically, Crabgo has run `rustdoc
...
The `-Z doctest-in-workspace` flag causes crabgo to switch to running `rustdoc`
...
* Tracking Issue: [#9357](https://github.com/rust-lang/crabgo/issues/9357)

`crabgo rustc --print=VAL` forwards the `--print` flag to `rustc` in order to
...
flag, and then immediately exits without compiling. Exposing this as a crabgo
flag allows crabgo to inject the correct target and RUSTFLAGS based on the
current configuration.

The primary use case is to run `crabgo rustc --print=cfg` to get config values
for the appropriate target and influenced by any other RUSTFLAGS.


### Different binary name

* Tracking Issue: [#9778](https://github.com/rust-lang/crabgo/issues/9778)
* PR: [#9627](https://github.com/rust-lang/crabgo/pull/9627)
...
The `filename` parameter should **not** include the binary extension, `crabgo` will figure out the appropriate
...
* Tracking Issue: [#9910](https://github.com/rust-lang/crabgo/issues/9910)
...
```
crabgo doc -Z unstable-options -Z rustdoc-scrape-examples
```

By default, Crabgo will scrape examples from the example targets of packages being documented.
...
dev-deps requirement for `crabgo doc`. Therefore examples will *not* be scraped from example targets under the
...
For example, you can set `doc-scrape-examples` to true for one example target, and that signals to Crabgo that
you are ok with dev-deps being build for `crabgo doc`.


### check-cfg

* RFC: [#3013](https://github.com/rust-lang/rfcs/pull/3013)
* Tracking Issue: [#10554](https://github.com/rust-lang/crabgo/issues/10554)
...
```
crabgo check -Z unstable-options -Z check-cfg=features
crabgo check -Z unstable-options -Z check-cfg=names
crabgo check -Z unstable-options -Z check-cfg=values
crabgo check -Z unstable-options -Z check-cfg=features,names,values
```
...
```rust,no_run
// build.rs
println!("crabgo:rustc-check-cfg=names(foo, bar)");
```

```
crabgo check -Z unstable-options -Z check-cfg=output
```

#### `crabgo:rustc-check-cfg=CHECK_CFG`

The `rustc-check-cfg` instruction tells Crabgo to pass the given value to the
...
If you want to integrate with Crabgo features, use `-Zcheck-cfg=features` instead of
...
* Tracking Issue: [#11813](https://github.com/rust-lang/crabgo/issues/11813)
...
* Shallow-cloned and shallow-checked-out git repositories reside at their own `-shallow` suffixed directories, i.e,
  - `~/.crabgo/registry/index/*-shallow`
  - `~/.crabgo/git/db/*-shallow`
  - `~/.crabgo/git/checkouts/*-shallow`
...
* Even with the presence of `Crabgo.lock` or specifying a commit `{ rev = "…" }`, gitoxide is still smart enough to shallow fetch without unshallowing the existing repository.
...
Specifying the `edition` in `Crabgo.toml` has been stabilized in the 1.31 release.
See [the edition field](manifest.md#the-edition-field) for more information
about specifying this field.

### rename-dependency

Specifying renamed dependencies in `Crabgo.toml` has been stabilized in the 1.31 release.
See [renaming dependencies](specifying-dependencies.md#renaming-dependencies-in-crabgotoml)
...
See the [`--offline` flag](../commands/crabgo.md#option-crabgo---offline) for
more information on using the offline mode.

### publish-lockfile

The `publish-lockfile` feature has been removed in the 1.37 release.
The `Crabgo.lock` file is always included when a package is published if the
package contains a binary target. `crabgo install` requires the `--locked` flag
to use the `Crabgo.lock` file.
See [`crabgo package`](../commands/crabgo-package.md) and
[`crabgo install`](../commands/crabgo-install.md) for more information.
...
when re-running Crabgo.

### install-upgrade

The `install-upgrade` feature has been stabilized in the 1.41 release.
[`crabgo install`] will now automatically upgrade packages if they appear to be
out-of-date. See the [`crabgo install`] documentation for more information.
...
Specifying profiles in Crabgo config files and environment variables has been
...
[`crabgo doc`](../commands/crabgo-doc.md) documentation sidebar.
...
The `resolver` feature in `Crabgo.toml` has been stabilized in the 1.51 release.
...
The `configurable-env` feature to specify environment variables in Crabgo
...
The `rust-version` field in `Crabgo.toml` has been stabilized in the 1.56 release.
...
`[patch]` section in Crabgo configuration files has been stabilized in
...
See [`crabgo fix --edition`](../commands/crabgo-fix.md) and [The Edition Guide](../../edition-guide/index.html) for more information on migrating existing projects.
...
The `--crate-type` flag for `crabgo rustc` has been stabilized in the 1.64
release. See the [`crabgo rustc` documentation](../commands/crabgo-rustc.md)
for more information.
...
terminal where Crabgo can automatically detect the width.
...
The [`crabgo logout`] command has been stabilized in the 1.70 release.
