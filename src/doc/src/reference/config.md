...
This document explains how Crabgo’s configuration system works, as well as
...
Crabgo allows local configuration for a particular package as well as global
configuration. It looks for configuration files in the current directory and
all parent directories. If, for example, Crabgo were invoked in
`/projects/foo/bar/baz`, then the following configuration files would be
probed for and unified in this order:

* `/projects/foo/bar/baz/.crabgo/config.toml`
* `/projects/foo/bar/.crabgo/config.toml`
* `/projects/foo/.crabgo/config.toml`
* `/projects/.crabgo/config.toml`
* `/.crabgo/config.toml`
* `$CRABGO_HOME/config.toml` which defaults to:
    * Windows: `%USERPROFILE%\.crabgo\config.toml`
    * Unix: `$HOME/.crabgo/config.toml`
...
At present, when being invoked from a workspace, Crabgo does not read config
...
there are Crabgo configs at `/projects/foo/bar/baz/mylib/.crabgo/config.toml`
and `/projects/foo/bar/baz/mybin/.crabgo/config.toml`, Crabgo does not read
those configuration files if it is invoked from the workspace root
(`/projects/foo/bar/baz/`).

> **Note:** Crabgo also reads config files without the `.toml` extension, such as
> `.crabgo/config`. Support for the `.toml` extension was added in version 1.39
> and is the preferred form. If both files exist, Crabgo will use the file
...
target = "triple"             # build for the target triple (ignored by `crabgo install`)
...
[doc]
browser = "chromium"          # browser to use with `crabgo doc --open`,
                              # overrides the `BROWSER` environment variable

[env]
# Set ENV_VAR_NAME=value for any process run by Crabgo
ENV_VAR_NAME = "value"
# Set even if already present in environment
ENV_VAR_NAME_2 = { value = "value", force = true }
# Value is relative to .crabgo directory containing `config.toml`, make absolute
...
[install]
root = "/some/path"         # `crabgo install` destination directory
...
[patch.<registry>]
# Same keys as for [patch] in Crabgo.toml
...
[term]
quiet = false          # whether crabgo output is quiet
verbose = false        # whether crabgo provides verbose output
color = 'auto'         # whether crabgo colorizes output
progress.when = 'auto' # whether crabgo shows progress bar
progress.width = 80    # width of progress bar
```

### Environment variables

Crabgo can also be configured through environment variables in addition to the
TOML configuration files. For each configuration key of the form `foo.bar` the
environment variable `CRABGO_FOO_BAR` can also be used to define the value.
...
defined by the `CRABGO_TARGET_X86_64_UNKNOWN_LINUX_GNU_RUNNER` environment
variable.
...
supported due to [technical issues](https://github.com/rust-lang/crabgo/issues/5416).

In addition to the system above, Crabgo recognizes a few other specific
[environment variables][env].

### Command-line overrides

Crabgo also accepts arbitrary configuration overrides through the
`--config` command-line option. The argument should be in TOML syntax of
`KEY=VALUE`:

```console
crabgo --config net.git-fetch-with-cli=true fetch
```
...
crabgo --config http.proxy=\"http://example.com\" …

# Spaces may be used.
crabgo --config "net.git-fetch-with-cli = true" …

# TOML array example. Single quotes make it easier to read and write.
crabgo --config 'build.rustdocflags = ["--html-in-header", "header.html"]' …

# Example of a complex TOML key.
crabgo --config "target.'cfg(all(target_arch = \"arm\", target_os = \"none\"))'.runner = 'my-runner'" …

# Example of overriding a profile setting.
crabgo --config profile.dev.package.image.opt-level=3 …
```

The `--config` option can also be used to pass paths to extra
configuration files that Crabgo should use for a specific invocation.
...
> **Note:** To maintain consistency with existing `.crabgo/config.toml` probing behavior,
...
> at the same level of discovered `.crabgo/config.toml` in your project.
> For instance, given a project `/my/project`,
> it is recommended to put config files under `/my/project/.crabgo`
...
# Directory is relative to the parent where `.crabgo/config.toml` is located.
# For example, `/my/project/.crabgo/config.toml` would result in `/my/project/vendor`.
directory = "vendor"
```

### Executable paths with arguments

Some Crabgo commands invoke external programs, which can be configured as a path
...
If Crabgo is passing other arguments to the program such as a path to open or
...
Crabgo will search `PATH` for its executable.

### Credentials

Configuration values with sensitive information are stored in the
`$CRABGO_HOME/credentials.toml` file. This file is automatically created and updated
by [`crabgo login`] and [`crabgo logout`]. It follows the same format as Crabgo config files.
...
Tokens are used by some Crabgo commands such as [`crabgo publish`] for
...
`CRABGO_REGISTRY_TOKEN` environment variable. Tokens for other registries may
be specified with environment variables of the form
`CRABGO_REGISTRIES_<name>_TOKEN` where `<name>` is the name of the registry in
all capital letters.
...
* Default: see below
* Environment: `CRABGO_ALIAS_<name>`

The `[alias]` table defines CLI command aliases. For example, running `crabgo
b` is an alias for running `crabgo build`. Each key in the table is the
...
subcommand and arguments. The following aliases are built-in to Crabgo:
...
* Default: "rustc"
* Environment: `CRABGO_BUILD_RUSTC` or `RUSTC`
...
* Type: string (program path)
* Default: none
* Environment: `CRABGO_BUILD_RUSTC_WRAPPER` or `RUSTC_WRAPPER`
...
* Type: string (program path)
* Default: none
* Environment: `CRABGO_BUILD_RUSTC_WORKSPACE_WRAPPER` or `RUSTC_WORKSPACE_WRAPPER`
...
* Type: string (program path)
* Default: "rustdoc"
* Environment: `CRABGO_BUILD_RUSTDOC` or `RUSTDOC`
...
* Environment: `CRABGO_BUILD_TARGET`
...
* Type: string (path)
* Default: "target"
* Environment: `CRABGO_BUILD_TARGET_DIR` or `CRABGO_TARGET_DIR`
...
* Type: string or array of strings
* Default: none
* Environment: `CRABGO_BUILD_RUSTFLAGS` or `CRABGO_ENCODED_RUSTFLAGS` or `RUSTFLAGS`
...
1. `CRABGO_ENCODED_RUSTFLAGS` environment variable.
2. `RUSTFLAGS` environment variable.
3. All matching `target.<triple>.rustflags` and `target.<cfg>.rustflags`
   config entries joined together.
4. `build.rustflags` config value.

Additional flags may also be passed with the [`crabgo rustc`] command.
...
It is not recommended to pass in flags that Crabgo itself usually manages. For
...
> compiler, this may cause a conflict with future versions of Crabgo which may
> issue the same or similar flags on its own which may interfere with the
> flags you specify. This is an area where Crabgo may not always be backwards
> compatible.

##### `build.rustdocflags`
* Type: string or array of strings
* Default: none
* Environment: `CRABGO_BUILD_RUSTDOCFLAGS` or `CRABGO_ENCODED_RUSTDOCFLAGS` or `RUSTDOCFLAGS`
...
1. `CRABGO_ENCODED_RUSTDOCFLAGS` environment variable.
2. `RUSTDOCFLAGS` environment variable.
3. `build.rustdocflags` config value.

Additional flags may also be passed with the [`crabgo rustdoc`] command.

##### `build.incremental`
* Type: bool
* Default: from profile
* Environment: `CRABGO_BUILD_INCREMENTAL` or `CRABGO_INCREMENTAL`
...
The `CRABGO_INCREMENTAL` environment variable can be set to `1` to force enable
...
* Type: string (path)
* Default: none
* Environment: `CRABGO_BUILD_DEP_INFO_BASEDIR`
...
`"."` would strip all paths starting with the parent directory of the `.crabgo`
directory.

##### `build.pipelining`

This option is deprecated and unused. Crabgo always has pipelining enabled.

#### `[doc]`

The `[doc]` table defines options for the [`crabgo doc`] command.
...
This option sets the browser to be used by [`crabgo doc`], overriding the
`BROWSER` environment variable when opening documentation with the `--open`
option.

#### `[crabgo-new]`

The `[crabgo-new]` table defines defaults for the [`crabgo new`] command.
...
* Type: string
* Default: "git" or "none"
* Environment: `CRABGO_CRABGO_NEW_VCS`
...
build scripts, rustc invocations, `crabgo run` and `crabgo build`.
...
is relative to the parent directory of the `.crabgo` directory that contains the
...
* Type: string
* Default: "always"
* Environment: `CRABGO_FUTURE_INCOMPAT_REPORT_FREQUENCY`

Controls how often we display a notification to the terminal when a future incompat report is available. Possible values:

* `always` (default): Always display a notification when a command (e.g. `crabgo build`) produces a future incompat report
...
* Type: boolean
* Default: false
* Environment: `CRABGO_HTTP_DEBUG`

If `true`, enables debugging of HTTP requests. The debug information can be
seen by setting the `CRABGO_LOG=crabgo::ops::registry=debug` environment
...
* Type: string
* Default: none
* Environment: `CRABGO_HTTP_PROXY` or `HTTPS_PROXY` or `https_proxy` or `http_proxy`

Sets an HTTP and HTTPS proxy to use. The format is in [libcurl format] as in
`[protocol://]host[:port]`. If not set, Crabgo will also check the `http.proxy`
...
* Type: integer
* Default: 30
* Environment: `CRABGO_HTTP_TIMEOUT` or `HTTP_TIMEOUT`
...
* Type: string (path)
* Default: none
* Environment: `CRABGO_HTTP_CAINFO`

Path to a Certificate Authority (CA) bundle file, used to verify TLS
certificates. If not specified, Crabgo attempts to use the system certificates.

##### `http.check-revoke`
* Type: boolean
* Default: true (Windows) false (all others)
* Environment: `CRABGO_HTTP_CHECK_REVOKE`
...
* Default: none
* Environment: `CRABGO_HTTP_SSL_VERSION`
...
* Type: integer
* Default: 10
* Environment: `CRABGO_HTTP_LOW_SPEED_LIMIT`
...
connection is considered too slow and Crabgo will abort and retry.

##### `http.multiplexing`
* Type: boolean
* Default: true
* Environment: `CRABGO_HTTP_MULTIPLEXING`

When `true`, Crabgo will attempt to use the HTTP2 protocol with multiplexing.
This allows multiple requests to use the same connection, usually improving
performance when fetching multiple files. If `false`, Crabgo will use HTTP 1.1
without pipelining.

##### `http.user-agent`
* Type: string
* Default: Crabgo's version
* Environment: `CRABGO_HTTP_USER_AGENT`

Specifies a custom user-agent header to use. The default if not specified is a
string that includes Crabgo's version.

#### `[install]`

The `[install]` table defines defaults for the [`crabgo install`] command.

##### `install.root`
* Type: string (path)
* Default: Crabgo's home directory
* Environment: `CRABGO_INSTALL_ROOT`

Sets the path to the root directory for installing executables for [`crabgo
...
The default if not specified is Crabgo's home directory (default `.crabgo` in
your home directory).
...
* Type: boolean
* Default: false
* Environment: `CRABGO_NET_GIT_FETCH_WITH_CLI`

If this is `true`, then Crabgo will use the `git` executable to fetch registry
...
requirements that Crabgo does not support. See [Git
...
* Type: boolean
* Default: false
* Environment: `CRABGO_NET_OFFLINE`

If this is `true`, then Crabgo will avoid accessing the network, and attempt to
proceed with locally cached data. If `false`, Crabgo will access the network as
...
Crabgo will attempt to load known hosts keys from common locations supported in
OpenSSH, and will join those with any listed in a Crabgo configuration file.
If any matching entry has the correct key, the connection will be allowed.

Crabgo comes with the host keys for [github.com][github-keys] built-in. If
...
`Crabgo.toml`](overriding-dependencies.md#the-patch-section), you can
override them in the crabgo configuration file to apply those patches to
any affected build. The format is identical to the one used in
`Crabgo.toml`.

Since `.crabgo/config.toml` files are not usually checked into source
control, you should prefer patching using `Crabgo.toml` where possible to
ensure that other developers can compile your crate in their own
environments. Patching through crabgo configuration files is generally
only appropriate when the patch section is automatically generated by an
external build tool.

If a given dependency is patched both in a crabgo configuration file and
a `Crabgo.toml` file, the patch in the configuration file is used. If
multiple configuration files patch the same dependency, standard crabgo
configuration merging is used, which prefers the value defined closest
to the current directory, with `$HOME/.crabgo/config.toml` taking the
lowest precedence.
...
override settings specified in `Crabgo.toml`. It has the same syntax and
options as profiles specified in `Crabgo.toml`. See the [Profiles chapter] for
details about the options.

[Profiles chapter]: profiles.md

##### `[profile.<name>.build-override]`
* Environment: `CRABGO_PROFILE_<name>_BUILD_OVERRIDE_<key>`
...
* Environment: `CRABGO_PROFILE_<name>_CODEGEN_UNITS`
...
* Default: See profile docs.
* Environment: `CRABGO_PROFILE_<name>_DEBUG`
...
* Type: string
* Default: See profile docs.
* Environment: `CRABGO_PROFILE_<name>_SPLIT_DEBUGINFO`
...
* Type: boolean
* Default: See profile docs.
* Environment: `CRABGO_PROFILE_<name>_DEBUG_ASSERTIONS`
...
* Type: boolean
* Default: See profile docs.
* Environment: `CRABGO_PROFILE_<name>_INCREMENTAL`
...
* Default: See profile docs.
* Environment: `CRABGO_PROFILE_<name>_LTO`
...
* Type: boolean
* Default: See profile docs.
* Environment: `CRABGO_PROFILE_<name>_OVERFLOW_CHECKS`
...
* Default: See profile docs.
* Environment: `CRABGO_PROFILE_<name>_OPT_LEVEL`
...
* Type: string
* default: See profile docs.
* Environment: `CRABGO_PROFILE_<name>_PANIC`
...
* Type: boolean
* default: See profile docs.
* Environment: `CRABGO_PROFILE_<name>_RPATH`
...
* Type: string (url)
* Default: none
* Environment: `CRABGO_REGISTRIES_<name>_INDEX`
...
* Type: string
* Default: none
* Environment: `CRABGO_REGISTRIES_<name>_TOKEN`
...
commands like [`crabgo publish`] that require authentication.
...
* Type: string
* Default: `sparse`
* Environment: `CRABGO_REGISTRIES_CRATES_IO_PROTOCOL`

Specifies the protocol used to access crates.io. Allowed values are `git` or `sparse`.

`git` causes Crabgo to clone the entire index of all packages ever published to [crates.io] from <https://github.com/rust-lang/crates.io-index/>.
...
* Type: string
* Default: `"crates-io"`
* Environment: `CRABGO_REGISTRY_DEFAULT`

The name of the registry (from the [`registries` table](#registries)) to use
by default for registry commands like [`crabgo publish`].
...
* Type: string
* Default: none
* Environment: `CRABGO_REGISTRY_TOKEN`
...
`debug_assertions` or Crabgo features like `feature="foo"`.
...
* Type: string (program path)
* Default: none
* Environment: `CRABGO_TARGET_<triple>_LINKER`
...
* Type: string or array of strings ([program path with args])
* Default: none
* Environment: `CRABGO_TARGET_<triple>_RUNNER`
...
an argument. This applies to [`crabgo run`], [`crabgo test`] and [`crabgo bench`]
...
* Type: string or array of strings
* Default: none
* Environment: `CRABGO_TARGET_<triple>_RUSTFLAGS`
...
* Type: boolean
* Default: false
* Environment: `CRABGO_TERM_QUIET`

Controls whether or not log messages are displayed by Crabgo.
...
* Type: boolean
* Default: false
* Environment: `CRABGO_TERM_VERBOSE`

Controls whether or not extra detailed messages are displayed by Crabgo.
...
* Type: string
* Default: "auto"
* Environment: `CRABGO_TERM_PROGRESS_WHEN`
...
* Type: integer
* Default: none
* Environment: `CRABGO_TERM_PROGRESS_WIDTH`
