...
Crabgo installs crates and fetches dependencies from a "registry". The default
...
support publishing new crates directly from Crabgo.
...
details about the protocol between Crabgo and a registry.

### Using an Alternate Registry

To use a registry other than [crates.io], the name and index URL of the
registry must be added to a [`.crabgo/config.toml` file][config]. The `registries`
...
Crabgo sparse registry URL with the `sparse+` prefix.
...
```ignore
CRABGO_REGISTRIES_MY_REGISTRY_INDEX=https://my-intranet:8080/git/index
```
...
directly to the registry from Crabgo. Several of Crabgo's commands such as
[`crabgo publish`] take a `--registry` command-line flag to indicate which
registry to use. For example, to publish the package in the current directory:

1. `crabgo login --registry=my-registry`
    ...
    `CRABGO_REGISTRIES_MY_REGISTRY_TOKEN`.

2. `crabgo publish --registry=my-registry`

Instead of always passing the `--registry` command-line option, the default
registry may be set in [`.crabgo/config.toml`][config] with the `registry.default`
key. For example:
...
Setting the `package.publish` key in the `Crabgo.toml` manifest restricts which
...
The authentication information saved by [`crabgo login`] is stored in the
`credentials.toml` file in the Crabgo home directory (default `$HOME/.crabgo`). It
...
### Registry Protocols
Crabgo supports two remote registry protocols: `git` and `sparse`. If the registry
index URL starts with `sparse+`, Crabgo uses the sparse protocol. Otherwise
Crabgo uses the `git` protocol.

The `git` protocol stores index metadata in a git repository and requires Crabgo to clone
the entire repo.

The `sparse` protocol fetches individual metadata files using plain HTTP requests.
Since Crabgo only downloads the metadata for relevant crates, the `sparse` protocol can
