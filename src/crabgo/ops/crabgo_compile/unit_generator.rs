...
use crate::util::{closest_msg, CrabgoResult};
...
impl<'a> UnitGenerator<'a, '_> {
        ...
        explicit_kinds
            ...
            .map(move |kind| {
                let unit_for = if initial_target_mode.is_any_test() {
                    ...
                    //
                    // Forcing the lib to be compiled three times during `crabgo
    ...
    fn create_proposals(&self) -> CrabgoResult<Vec<Proposal<'_>>> {
    ...
    fn create_docscrape_proposals(&self, doc_units: &[Unit]) -> CrabgoResult<Vec<Proposal<'a>>> {
        ...
        //
        // If scrape-examples caused `crabgo doc` to start requiring dev-deps, this
...
Rustdoc did not scrape the following examples because they require dev-dependencies: {example_str}
    ...
    /// and their code remains unchecked despite crabgo not returning any errors
    fn unmatched_target_filters(&self, units: &[Unit]) -> CrabgoResult<()> {
        let mut shell = self.ws.config().shell();
    ...
    fn proposals_to_units(&self, proposals: Vec<Proposal<'_>>) -> CrabgoResult<Vec<Unit>> {
    ...
    pub fn generate_root_units(&self) -> CrabgoResult<Vec<Unit>> {
    ...
    pub fn generate_scrape_units(&self, doc_units: &[Unit]) -> CrabgoResult<Vec<Unit>> {
