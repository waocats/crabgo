//! Filters and their rules to select which Crabgo targets will be built.
...
#[derive(Debug, Clone)]
/// Indicates which Crabgo targets will be selected to be built.
pub enum FilterRule {
    /// All included.
    All,
    /// Just a subset of Crabgo targets based on names given.
    Just(Vec<String>),
}

/// Filter to apply to the root package to select which Crabgo targets will be built.
...
pub enum CompileFilter {
    /// The default set of Crabgo targets.
    Default {
        /// Flag whether targets can be safely skipped when required-features are not satisfied.
        required_features_filterable: bool,
    },
    /// Only includes a subset of all Crabgo targets.
    Only {
        /// Include all Crabgo targets.
        all_targets: bool,
        lib: LibRule,
        bins: FilterRule,
        examples: FilterRule,
...
impl FilterRule {
    ...
    ///
    /// In the current Crabgo implementation, filter without a rule implies
    /// Crabgo will follows the default behaviour to filter targets.
...
impl CompileFilter {
    ...
    /// function only recognizes test targets, which means crabgo might compile
    ...
    /// Indicates if Crabgo needs to build any dev dependency.
    ...
    /// Selects targets for "crabgo run". for logic to select targets for other
