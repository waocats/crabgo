...
//!        empty.  Note that this is the only point in crabgo that currently uses threads.
...
//!
//! **Note**: "target" inside this module generally refers to ["Crabgo Target"],
...
//! ["Crabgo Target"]: https://doc.rust-lang.org/nightly/crabgo/reference/crabgo-targets.html
...
use crate::util::{profile, CrabgoResult, StableHasher};
...
pub struct CompileOptions {
    ...
    /// defined in the crabgo metadata for a crate.
    pub honor_rust_version: bool,
}

impl CompileOptions {
    pub fn new(config: &Config, mode: CompileMode) -> CrabgoResult<CompileOptions> {
        let jobs = None;
...
pub fn compile<'a>(ws: &Workspace<'a>, options: &CompileOptions) -> CrabgoResult<Compilation<'a>> {
...
pub fn create_bcx<'a, 'cfg>(
...
) -> CrabgoResult<BuildContext<'a, 'cfg>> {
    let CompileOptions {
        ref build_config,
        ref spec,
        ref cli_features,
    ...
    match build_config.mode {
        ...
        | CompileMode::RunCustomBuild => {
            if ws.config().get_env("RUST_FLAGS").is_ok() {
                config.shell().warn(
                    "Crabgo does not read `RUST_FLAGS` environment variable. Did you mean `RUSTFLAGS`?",
                )?;
            }
        }
        CompileMode::Doc { .. } | CompileMode::Doctest | CompileMode::Docscrape => {
            if ws.config().get_env("RUSTDOC_FLAGS").is_ok() {
                config.shell().warn(
                    "Crabgo does not read `RUSTDOC_FLAGS` environment variable. Did you mean `RUSTDOCFLAGS`?"
                )?;
            }
        }
    }
    ...
    // The ordering here affects some error messages coming out of crabgo, so
    ...
    // TODO: In theory, Crabgo should also dedupe the roots, but I'm uncertain
        ...
        .filter(|unit| rustdoc_document_private_items || unit.target.is_bin())
    {
        ...
        if unit.target.is_bin() {
            ...
            // sometimes and ignore them at other times. But crabgo consistently passes
    ...
    if honor_rust_version {
        ...
        for unit in unit_graph.keys() {
            ...
            let guidance = if ws.is_ephemeral() {
                if ws.ignore_lock() {
                    "Try re-running crabgo install with `--locked`".to_string()
                } else {
                    String::new()
                }
...
/// collision in the `UnitGraph`, and Crabgo would end up randomly choosing one
/// value or the other.
///
...
///
/// This is primarily used by `crabgo rustc --crate-type`.
