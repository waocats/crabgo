...
impl Packages {
    ...
    pub fn from_flags(all: bool, exclude: Vec<String>, package: Vec<String>) -> CrabgoResult<Self> {
    ...
    pub fn to_package_id_specs(&self, ws: &Workspace<'_>) -> CrabgoResult<Vec<PackageIdSpec>> {
    ...
    pub fn get_packages<'ws>(&self, ws: &'ws Workspace<'_>) -> CrabgoResult<Vec<&'ws Package>> {
...
fn opt_patterns_and_names(
    opt: &[String],
) -> CrabgoResult<(Vec<(glob::Pattern, bool)>, BTreeSet<&str>)> {
...
pub fn build_glob(pat: &str) -> CrabgoResult<glob::Pattern> {
