...
/// is both backwards and forwards compatible. Crabgo keeps both files in sync,
/// updating both v1 and v2 at the same time. Additionally, if it detects
/// changes in v1 that are not in v2 (such as when an older version of Crabgo
/// is used), it will automatically propagate those changes to v2.
///
/// This maintains a filesystem lock, preventing other instances of Crabgo from
...
struct CrateListingV2 {
    ...
    /// Forwards compatibility. Unknown keys from future versions of Crabgo
...
/// then Crabgo will inform the user that it is "up to date".
///
...
impl InstallTracker {
    ...
    pub fn load(config: &Config, root: &Filesystem) -> CrabgoResult<InstallTracker> {
    ...
    pub fn check_upgrade(
        ...
        target: &str,
        _rustc: &str,
    ) -> CrabgoResult<(Freshness, BTreeMap<String, Option<PackageId>>)> {
        ...
        if matching_duplicates.len() == duplicates.len() {
            ...
            if source_id.is_path() {
                // `crabgo install --path ...` is always rebuilt.
                return Ok((Freshness::Dirty(Some(DirtyReason::Forced)), duplicates));
            }
...
impl CrateListingV1 {
    ...
    fn save(&self, lock: &FileLock) -> CrabgoResult<()> {
...
impl CrateListingV2 {
    ...
    /// i.e., `crabgo +new install foo ; crabgo +old install bar ; crabgo +new install bar`
...
pub fn resolve_root(flag: Option<&str>, config: &Config) -> CrabgoResult<Filesystem> {
...
pub fn path_source(source_id: SourceId, config: &Config) -> CrabgoResult<PathSource<'_>> {
    let path = source_id
        .url()
        .to_file_path()
...
where
    T: Source,
    F: FnMut(&mut T) -> CrabgoResult<Vec<Package>>,
{
    // This operation may involve updating some sources or making a few queries
    // which may involve frobbing caches, as a result make sure we synchronize
    // with other global Crabgos
    ...
    fn multi_err(kind: &str, git_url: &str, mut pkgs: Vec<&Package>) -> String {
        ...
        format!(
            ...
            crabgo will always search the entire repo for any Crabgo.toml.\n\
            Please specify a package, e.g. `crabgo install --git {} {}`.",
            kind,
            pkgs.iter()
                .map(|p| p.name().as_str())
                .collect::<Vec<_>>()
...
fn one<I, F>(mut i: I, f: F) -> CrabgoResult<Option<I::Item>>
where
    I: Iterator,
