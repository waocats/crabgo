...
use crate::util::toml as crabgo_toml;
use crate::util::Filesystem;

use anyhow::Context as _;

pub fn load_pkg_lockfile(ws: &Workspace<'_>) -> CrabgoResult<Option<Resolve>> {
    if !ws.root().join("Crabgo.lock").exists() {
        return Ok(None);
    }

    let root = Filesystem::new(ws.root().to_path_buf());
    let mut f = root.open_ro("Crabgo.lock", ws.config(), "Crabgo.lock file")?;

    let mut s = String::new();
    f.read_to_string(&mut s)
        .with_context(|| format!("failed to read file: {}", f.path().display()))?;

    let resolve = (|| -> CrabgoResult<Option<Resolve>> {
        let resolve: toml::Table = crabgo_toml::parse_document(&s, f.path(), ws.config())?;
...
/// Generate a toml String of Crabgo.lock from a Resolve.
pub fn resolve_to_string(ws: &Workspace<'_>, resolve: &mut Resolve) -> CrabgoResult<String> {
    let (_orig, out, _ws_root) = resolve_to_string_orig(ws, resolve);
    Ok(out)
}

pub fn write_pkg_lockfile(ws: &Workspace<'_>, resolve: &mut Resolve) -> CrabgoResult<()> {
    ...
    if !ws.config().lock_update_allowed() {
        ...
        anyhow::bail!(
            ...
            ws.root().to_path_buf().join("Crabgo.lock").display(),
            flag,
            flag
        );
    }
    ...
    ws_root
        .open_rw("Crabgo.lock", ws.config(), "Crabgo.lock file")
        .and_then(|mut f| {
            f.file().set_len(0)?;
            f.write_all(out.as_bytes())?;
            Ok(())
        })
        .with_context(|| format!("failed to write {}", ws.root().join("Crabgo.lock").display()))?;
    Ok(())
}
...
fn resolve_to_string_orig(
    ...
    let orig = ws_root.open_ro("Crabgo.lock", ws.config(), "Crabgo.lock file");
...
fn serialize_resolve(resolve: &Resolve, orig: Option<&str>) -> String {
    ...
    let marker_line = "# This file is automatically @generated by Crabgo.";
    ...
    // Historical versions of Crabgo in the old format accidentally left trailing
