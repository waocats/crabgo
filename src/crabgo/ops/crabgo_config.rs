//! Implementation of `crabgo config` subcommand.
...
impl FromStr for ConfigFormat {
    type Err = Error;
    fn from_str(s: &str) -> CrabgoResult<Self> {
        match s {
...
pub fn get(config: &Config, opts: &GetOptions<'_>) -> CrabgoResult<()> {
...
fn print_toml_unmerged(config: &Config, opts: &GetOptions<'_>, key: &ConfigKey) -> CrabgoResult<()> {
    ...
    fn trim_cv(mut cv: &mut CV, key: &ConfigKey) -> CrabgoResult<bool> {
