...
#[derive(Deserialize)]
struct CrabgoNewConfig {
    #[deprecated = "crabgo-new no longer supports adding the authors field"]
    #[allow(dead_code)]
    name: Option<String>,
...
fn get_name<'a>(path: &'a Path, opts: &'a NewOptions) -> CrabgoResult<&'a str> {
...
fn check_path(path: &Path, shell: &mut Shell) -> CrabgoResult<()> {
...
multiple possible binary sources found:
  {}
  {}
cannot automatically generate Crabgo.toml as the main target would be ambiguous",
                    &x.relative_path,
                    &i.relative_path
                );
            }
...
pub fn new(opts: &NewOptions, config: &Config) -> CrabgoResult<()> {
    let path = &opts.path;
    if path.exists() {
        anyhow::bail!(
...
pub fn init(opts: &NewOptions, config: &Config) -> CrabgoResult<NewProjectKind> {
    // This is here just as a random location to exercise the internal error handling.
    if config.get_env_os("__CRABGO_TEST_INTERNAL_ERROR").is_some() {
    ...
    if path.join("Crabgo.toml").exists() {
        anyhow::bail!("`crabgo init` cannot be run on existing Crabgo packages")
    }
    ...
    if version_control == None {
        ...
        // if none exists, maybe create git, like in `crabgo new`
...
impl IgnoreList {
    ...
    /// file.
    fn format_existing<T: BufRead>(&self, existing: T, vcs: VersionControl) -> CrabgoResult<String> {
            ...
            match item {
                ...
                Err(err) => match err.kind() {
                    ErrorKind::InvalidData => {
                        return Err(anyhow!(
                            "Character at line {} is invalid. Crabgo only supports UTF-8.",
                            i
                        ))
                    }
...
fn write_ignore_file(base_path: &Path, list: &IgnoreList, vcs: VersionControl) -> CrabgoResult<()> {
...
fn init_vcs(path: &Path, vcs: VersionControl, config: &Config) -> CrabgoResult<()> {
    match vcs {
        VersionControl::Git => {
            if !path.join(".git").exists() {
...
fn mk(config: &Config, opts: &MkOptions<'_>) -> CrabgoResult<()> {
    let path = opts.path;
    let name = opts.name;
    let cfg = config.get::<CrabgoNewConfig>("crabgo-new")?;
    ...
    if !opts.bin {
        ignore.push("/Crabgo.lock", "^Crabgo.lock$", "Crabgo.lock");
    }
    ...
    let mut crabgotoml_path_specifier = String::new();

    // Calculate what `[lib]` and `[[bin]]`s we need to append to `Crabgo.toml`.

    for i in &opts.source_files {
        if i.bin {
            if i.relative_path != "src/main.rs" {
                crabgotoml_path_specifier.push_str(&format!(
                    r#"
[[bin]]
name = "{}"
path = {}
...
"#,
    ...
    // Create `Crabgo.toml` file with necessary `[lib]` and `[[bin]]` sections, if needed.

    paths::write(
        &path.join("Crabgo.toml"),
        format!(
            r#"[package]
name = "{}"
version = "0.1.0"
edition = {}
{}
# See more keys and their definitions at https://doc.rust-lang.org/crabgo/reference/manifest.html
...
"
        ...
        if !path_of_source_file.is_file() {
            ...
            if let Err(e) = crabgo_util::ProcessBuilder::new("rustfmt")
    ...
    if let Err(e) = Workspace::new(&path.join("Crabgo.toml"), config) {
