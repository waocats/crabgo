...
use crate::util::{try_canonicalize, CrabgoResult, Config};
...
pub fn vendor(ws: &Workspace<'_>, opts: &VendorOptions<'_>) -> CrabgoResult<()> {
    ...
    if config.shell().verbosity() != Verbosity::Quiet {
        if vendor_config.source.is_empty() {
            ...
            crate::drop_eprint!(
                config,
                "To use vendored sources, add this to your .crabgo/config.toml for this project:\n\n"
            );
            crate::drop_print!(
                config,
                "{}",
...
fn sync(
    ...
    // First up attempt to work around rust-lang/crabgo#5956. Apparently build
    // artifacts sprout up in Crabgo's global cache for whatever reason, although
    // it's unsure what tool is causing these issues at this time. For now we
    // apply a heavy-hammer approach which is to delete Crabgo's unpacked version
    // of each crate to start off with. After we do this we'll re-resolve and
    // redownload again, which should trigger Crabgo to re-extract all the
    // crates.
    //
    ...
    for (id, pkg) in ids.iter() {
        ...
        let cksum = dst.join(".crabgo-checksum.json");
...
fn cp_sources(
    ...
    for p in paths {
        ...
        match relative.to_str() {
            ...
            // Temporary Crabgo files
            Some(".crabgo-ok") => continue,

            // Skip patch-style orig/rej files. Published crates on crates.io
            // have `Crabgo.toml.orig` which we don't want to use here and
        ...
        let cksum = if dst.file_name() == Some(OsStr::new("Crabgo.toml"))
