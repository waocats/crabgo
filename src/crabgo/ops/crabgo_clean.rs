...
pub fn clean(ws: &Workspace<'_>, opts: &CleanOptions<'_>) -> CrabgoResult<()> {
                ...
                for (compile_kind, layout) in &layouts {
                    ...
                    for file_type in file_types {
                        ...
                        if let Some(uplift_dir) = uplift_dir {
                            ...
                            // Dep-info generated by Crabgo itself.
...
fn escape_glob_path(pattern: &Path) -> CrabgoResult<String> {
    let pattern = pattern
        .to_str()
...
fn rm_rf(path: &Path, config: &Config, progress: &mut dyn CleaningProgressBar) -> CrabgoResult<()> {
...
fn clean_entire_folder(path: &Path, config: &Config) -> CrabgoResult<()> {
...
trait CleaningProgressBar {
    fn display_now(&mut self) -> CrabgoResult<()>;
    fn on_clean(&mut self) -> CrabgoResult<()>;
}
...
impl<'cfg> CleaningPackagesBar<'cfg> {
    ...
    fn on_cleaning_package(&mut self, package: &str) -> CrabgoResult<()> {
        self.cur += 1;
