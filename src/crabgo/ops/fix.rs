...
//!
//! The main goal is to run `crabgo check` to get rustc to emit JSON
...
//!
//! Crabgo begins by launching a `LockServer` thread in the background to
...
//!
//! Crabgo begins a normal `crabgo check` operation with itself set as a proxy
//! for rustc by setting `primary_unit_rustc` in the build config. When
//! crabgo launches rustc to check a crate, it is actually launching itself.
//! The `FIX_ENV_INTERNAL` environment variable is set so that crabgo knows it is in
//! fix-proxy-mode.
//!
//! Each proxied crabgo-as-rustc detects it is in fix-proxy-mode (via `FIX_ENV_INTERNAL`
//! environment variable in `main`) and does the following:
//!
//! - Acquire a lock from the `LockServer` from the master crabgo process.
...
use crabgo_util::{exit_status_to_string, is_simple_exit_code, paths, ProcessBuilder};
...
pub fn fix(ws: &Workspace<'_>, opts: &mut FixOptions) -> CrabgoResult<()> {
    ...
    *opts
    ...
    // This is calling rustc in crabgo fix-proxy-mode, so it also need to retry.
    // The argfile handling are located at `FixArgs::from_args`.
    wrapper.retry_with_argfile(true);

    // primary crates are compiled using a crabgo subprocess to do extra work of applying fixes and
...
fn check_version_control(config: &Config, opts: &FixOptions) -> CrabgoResult<()> {
    if opts.allow_no_vcs {
        return Ok(());
    }
    if !existing_vcs_repo(config.cwd(), config.cwd()) {
        bail!(
            "no VCS found for this package and `crabgo fix` can potentially \
    ...
    bail!(
        "the working directory of this package has uncommitted changes, and \
         `crabgo fix` can potentially perform destructive changes; if you'd \
...
fn check_resolver_change(ws: &Workspace<'_>, opts: &FixOptions) -> CrabgoResult<()> {
    ...
    let resolve_differences = |has_dev_units| -> CrabgoResult<(WorkspaceResolve<'_>, DiffMap)> {
    ...
    config.shell().note(
        "Switching to Edition 2021 will enable the use of the version 2 feature resolver in Crabgo.",
    )?;
    drop_eprintln!(
        config,
    ...
    drop_eprintln!(
        ...
        "More information about the resolver changes may be found \
         at https://doc.rust-lang.org/nightly/edition-guide/rust-2021/default-crabgo-resolver.html"
    );
    drop_eprintln!(
        config,
...
fn report_maybe_diesel(config: &Config, resolve: &Resolve) -> CrabgoResult<()> {
...
pub fn fix_get_proxy_lock_addr() -> Option<String> {
    // ALLOWED: For the internal mechanism of `crabgo fix` only.
    // Shouldn't be set directly by anyone.
    #[allow(clippy::disallowed_methods)]
    env::var(FIX_ENV_INTERNAL).ok()
}

/// Entry point for `crabgo` running as a proxy for `rustc`.
///
/// This is called every time `crabgo` is run to check if it is in proxy mode.
///
...
pub fn fix_exec_rustc(config: &Config, lock_addr: &str) -> CrabgoResult<()> {
    let args = FixArgs::get()?;
    trace!("crabgo-fix as rustc got file {:?}", args.file);
    ...
    for arg in args.format_args {
        // Add any json/error format arguments that Crabgo wants. This allows
...
fn rustfix_and_fix(
    ...
    for suggestion in suggestions {
        ...
        // Do not write into registry cache. See rust-lang/crabgo#9857.
...
/// Various command-line options and settings used when `crabgo` is running as
...
struct FixArgs {
    ...
    /// Crabgo normally passes when applying fixes. When displaying warnings or
    /// errors, it will use these flags.
    format_args: Vec<String>,
}

impl FixArgs {
    fn get() -> CrabgoResult<FixArgs> {
        Self::from_args(env::args_os())
    }

    // This is a separate function so that we can use it in tests.
    fn from_args(argv: impl IntoIterator<Item = OsString>) -> CrabgoResult<Self> {
        ...
        let mut handle_arg = |arg: OsString| -> CrabgoResult<()> {
            ...
            if let Some(s) = path.to_str() {
                ...
                if s.starts_with("--error-format=") || s.starts_with("--json=") {
                    // Crabgo may add error-format in some cases, but `crabgo
        ...
        if let Some(argfile_path) = rustc.to_str().unwrap_or_default().strip_prefix("@") {
            // Because crabgo in fix-proxy-mode might hit the command line size limit,
            // crabgo fix need handle `@path` argfile for this special case.
    ...
    fn apply(&self, cmd: &mut ProcessBuilder) {
        ...
        if self.prepare_for_edition.is_some() {
            ...
            // This allows `crabgo fix` to work even if the crate has #[deny(warnings)].
    ...
    fn can_run_rustfix(&self, config: &Config) -> CrabgoResult<bool> {
        ...
        // Unfortunately determining which crabgo targets are being built
        // isn't easy, and each target can be a different edition. The
        // crabgo-as-rustc fix wrapper doesn't know anything about the
        // workspace, so it can't check for the `crabgo-features` unstable
