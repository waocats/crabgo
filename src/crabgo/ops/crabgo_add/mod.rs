...
pub struct AddOptions<'a> {
    /// Configuration information for crabgo operations
    pub config: &'a Config,
...
pub fn add(workspace: &Workspace<'_>, options: &AddOptions<'_>) -> CrabgoResult<()> {
...
fn check_invalid_ws_keys(toml_key: &str, arg: &DepOp) -> CrabgoResult<()> {
...
fn get_existing_dependency(
    ...
    dep_key: &str,
    section: &DepTable,
) -> CrabgoResult<Option<Dependency>> {
...
fn infer_package_for_git_source(
    ...
    let package = match packages.len() {
        ...
        _ => {
            ...
            names.sort_unstable();
            anyhow::bail!(
                "multiple packages found at `{src}`:\n    {}\nTo disambiguate, run `crabgo add --git {src} <package>`",
                names
                    .iter()
                    .map(|s| s.to_string())
                    .coalesce(|x, y| if x.len() + y.len() < 78 {
...
fn print_action_msg(shell: &mut Shell, dep: &DependencyUI, section: &[String]) -> CrabgoResult<()> {
...
fn print_dep_table_msg(shell: &mut Shell, dep: &DependencyUI) -> CrabgoResult<()> {
    if matches!(shell.verbosity(), crate::core::shell::Verbosity::Quiet) {
        return Ok(());
    }
...
fn find_workspace_dep(toml_key: &str, root_manifest: &Path) -> CrabgoResult<Dependency> {
