...
//! the `Crabgo.lock` file and checkinf if it needs updating.
//!
...
//!   - [`Package`]
//!     Corresponds with `Crabgo.toml` manifest (deserialized as [`Manifest`]) and its associated files.
...
pub struct WorkspaceResolve<'cfg> {
    ...
    ///
    /// This may be `None` for things like `crabgo install` and `-Zavoid-dev-deps`.
...
what is locked in the Crabgo.lock file, run `crabgo update` to use the new
...
/// (unless it is an ephemeral workspace such as `crabgo install` or `crabgo
/// package`).
///
/// This is a simple interface used by commands like `clean`, `fetch`, and
/// `package`, which don't specify any options or features.
pub fn resolve_ws<'a>(ws: &Workspace<'a>) -> CrabgoResult<(PackageSet<'a>, Resolve)> {
...
///
/// This function will also write the result of resolution as a new lock file
/// (unless `Workspace::require_optional_deps` is false, such as `crabgo
/// install` or `-Z avoid-dev-deps`), or it is an ephemeral workspace (`crabgo
/// install` or `crabgo package`).
///
...
) -> CrabgoResult<WorkspaceResolve<'cfg>> {
    let mut registry = PackageRegistry::new(ws.config())?;
...
pub fn resolve_with_previous<'cfg>(
    ...
    // We only want one Crabgo at a time resolving a crate graph since this can
    ...
    if register_patches {
        for (url, patches) in ws.root_patch()?.iter() {
            ...
            // the raw `Dependency` which matches what's listed in `Crabgo.toml`.
            ...
            for dep in patches {
                ...
                let lock = match candidates().find(|id| dep.matches_id(*id)) {
                    ...
                    None => {
                        ...
                        // (which is now treated separately in Crabgo).
                        //
...
pub fn add_overrides<'a>(
    ...
    let paths = paths.val.iter().map(|(s, def)| {
        ...
        // key was located, so we want to pop off the `.crabgo/config` component
        // to get the directory containing the `.crabgo` folder.
...
fn register_previous_locks(
    ...
    let path_pkg = |id: SourceId| {
        ...
        if let Ok(path) = id.url().to_file_path() {
            if let Ok(pkg) = ws.load(&path.join("Crabgo.toml")) {
                return Some(pkg);
            }
        }
        None
    ...
    // line". That is, if we run `crabgo update -p foo` then `keep(bar)` will return
    ...
    //
    // Let's say we then run `crabgo update -p serde`. This may *also* want to
    ...
    // graph can come from two locations, the `crabgo update` command or
    ...
    for node in resolve.iter().filter(keep) {
        ...
        // directive. Nowadays in Crabgo, however, these two directives are
