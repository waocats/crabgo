...
pub fn run(
    ...
    if options.filter.contains_glob_patterns() {
        anyhow::bail!("`crabgo run` does not support glob patterns on target selection")
    }
    ...
    if bins.is_empty() {
        if !options.filter.is_specific() {
            anyhow::bail!("a bin target must be available for `crabgo run`")
        } else {
            // This will be verified in `crabgo_compile`.
        }
    }
    ...
    if bins.len() > 1 {
        if !options.filter.is_specific() {
            ...
            names.sort();
            anyhow::bail!(
                "`crabgo run` could not determine which binary to run. \
            ...
            anyhow::bail!(
                "`crabgo run` can run at most one executable, but \
                 multiple were specified"
            )
        }
    }

    // `crabgo run` is only compatible with one `--target` flag at most
