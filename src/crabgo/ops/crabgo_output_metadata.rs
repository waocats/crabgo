...
/// format to stdout.
pub fn output_metadata(ws: &Workspace<'_>, opt: &OutputMetadataOptions) -> CrabgoResult<ExportInfo> {
...
///
/// See crabgo-metadata.adoc for detailed documentation of the format.
#[derive(Serialize)]
...
struct Dep {
    ...
    // mark this field as deprecated in the help manual of crabgo-metadata
    name: InternedString,
    pkg: PackageId,
...
) -> CrabgoResult<(Vec<SerializedPackage>, MetadataResolve)> {
...
fn build_resolve_graph_r(
    ...
    // Crabgo generally only exposes PackageIds from the Package struct, and
    ...
    // are deserialized from Crabgo.lock. Crabgo.lock may have been generated by
    // an older (or newer!) version of Crabgo which uses a different style.
        ...
        for (dep_id, deps) in iter {
            ...
            for dep in deps.iter() {
                if let Some(target) = lib_target {
                    ...
                    // TODO(bindeps): Crabgo shouldn't have `extern_name` field
                ...
                let compile_target = match artifact_requirements.target() {
                    Some(t) => t
                        ...
                        // Given that Crabgo doesn't know which target it should resolve to,
