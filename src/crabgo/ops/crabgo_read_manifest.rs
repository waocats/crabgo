...
pub fn read_package(
    ...
    source_id: SourceId,
    config: &Config,
) -> CrabgoResult<(Package, Vec<PathBuf>)> {
    trace!(
...
pub fn read_packages(
    ...
    if all_packages.is_empty() {
        match errors.pop() {
            Some(err) => Err(err),
            None => {
                if find_project_manifest_exact(path, "crabgo.toml").is_ok() {
                    Err(anyhow::format_err!(
                "Could not find Crabgo.toml in `{}`, but found crabgo.toml please try to rename it to Crabgo.toml",
                path.display()
            ))
                } else {
                    Err(anyhow::format_err!(
                        "Could not find Crabgo.toml in `{}`",
                        path.display()
                    ))
                }
            }
...
fn walk(path: &Path, callback: &mut dyn FnMut(&Path) -> CrabgoResult<bool>) -> CrabgoResult<()> {
    if !callback(path)? {
...
fn has_manifest(path: &Path) -> bool {
    find_project_manifest_exact(path, "Crabgo.toml").is_ok()
}
...
fn read_nested_packages(
    ...
    let manifest_path = find_project_manifest_exact(path, "Crabgo.toml")?;
    ...
    if !source_id.is_registry() {
        for p in nested.iter() {
            ...
            //
            // See https://github.com/rust-lang/crabgo/issues/6822.
