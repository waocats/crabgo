...
fn compile_tests<'a>(ws: &Workspace<'a>, options: &TestOptions) -> CrabgoResult<Compilation<'a>> {
...
///
/// This is used when `crabgo test --no-run` is used.
...
fn cmd_builds(
    ...
    compilation: &Compilation<'_>,
    exec_type: &str,
) -> CrabgoResult<(String, ProcessBuilder)> {
...
///
/// Crabgo will return the error code from the test process itself. If some
/// other error happened (like a failure to launch the process), then it will
/// return a standard 101 error code.
///
/// When using `--no-fail-fast`, Crabgo always uses the 101 exit code (since
