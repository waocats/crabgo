...
pub fn publish(ws: &Workspace<'_>, opts: &PublishOpts<'_>) -> CrabgoResult<()> {
    ...
    if let Some(ref allowed_registries) = *pkg.publish() {
        ...
        if allowed_registries.is_empty() {
            ...
            bail!(
                "`{}` cannot be published.\n\
                 The registry `{}` is not listed in the `package.publish` value in Crabgo.toml.",
                pkg.name(),
                reg_name
            );
        }
    ...
    if !opts.dry_run {
        let hash = crabgo_util::Sha256::new()
...
fn transmit(
    ...
    let deps = pkg
        ...
        .map(|dep| {
            ...
            // whereas in Crabgo.toml, it means "from crates.io".
        ...
        .collect::<CrabgoResult<Vec<NewCrateDependency>>>()?;
    ...
    if !warnings.invalid_badges.is_empty() {
        let msg = format!(
            "the following are not valid badges and were ignored: {}. \
             ...
             https://doc.rust-lang.org/crabgo/reference/manifest.html#package-metadata \
...
) -> CrabgoResult<(Registry, RegistrySourceIds)> {
...
/// Creates a new HTTP handle with appropriate global configuration for crabgo.
pub fn http_handle(config: &Config) -> CrabgoResult<Easy> {
    let (mut handle, timeout) = http_handle_and_timeout(config)?;
    timeout.configure(&mut handle)?;
    Ok(handle)
}

pub fn http_handle_and_timeout(config: &Config) -> CrabgoResult<(Easy, HttpTimeout)> {
    if config.frozen() {
        bail!(
...
pub fn needs_custom_http_transport(config: &Config) -> CrabgoResult<bool> {
    Ok(http_proxy_exists(config)?
        || *config.http_config()? != Default::default()
        || config.get_env_os("HTTP_TIMEOUT").is_some())
}

/// Configure a libcurl http handle with the defaults options for Crabgo
pub fn configure_http_handle(config: &Config, handle: &mut Easy) -> CrabgoResult<HttpTimeout> {
    ...
    if let Some(user_agent) = &http.user_agent {
        ...
        handle.useragent(&format!("crabgo {}", version()))?;
    }

    fn to_ssl_version(s: &str) -> CrabgoResult<SslVersion> {
        let version = match s {
...
impl HttpTimeout {
    pub fn new(config: &Config) -> CrabgoResult<HttpTimeout> {
    ...
    pub fn configure(&self, handle: &mut Easy) -> CrabgoResult<()> {
...
///
/// Favor crabgo's `http.proxy`, then git's `http.proxy`. Proxies specified
/// via environment variables are picked up by libcurl.
fn http_proxy(config: &Config) -> CrabgoResult<Option<String>> {
    let http = config.http_config()?;
...
/// * `https_proxy` env var
/// * `HTTPS_PROXY` env var
fn http_proxy_exists(config: &Config) -> CrabgoResult<bool> {
        ...
        if !config.cli_unstable().registry_auth {
            ...
            bail!(
                "the `{flag}` flag is unstable, pass `-Z registry-auth` to enable it\n\
                 See https://github.com/rust-lang/crabgo/issues/10519 for more \
        ...
        new_token = RegistryCredentialConfig::Token(match token {
            ...
            None => {
                ...
                input
                    ...
                    .read_line(&mut line)
                    .with_context(|| "failed to read stdin")?;
                // Automatically remove `crabgo login` from an inputted token to
                // allow direct pastes from `registry.host()`/me.
                Secret::from(line.replace("crabgo login", "").trim().to_string())
            }
        });
...
pub fn registry_logout(config: &Config, reg: Option<&str>) -> CrabgoResult<()> {
    let source_ids = get_source_id(config, None, reg)?;
    let reg_cfg = auth::registry_credential_config(config, &source_ids.original)?;
...
pub fn modify_owners(config: &Config, opts: &OwnersOptions) -> CrabgoResult<()> {
...
struct RegistrySourceIds {
    /// Use when looking up the auth token, or writing out `Crabgo.lock`
    original: SourceId,
