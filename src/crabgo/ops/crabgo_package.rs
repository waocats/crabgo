...
const ORIGINAL_MANIFEST_FILE: &str = "Crabgo.toml.orig";
const VCS_INFO_FILE: &str = ".crabgo_vcs_info.json";
...
enum GeneratedFile {
    /// Generates `Crabgo.toml` by rewriting the original.
    Manifest,
    /// Generates `Crabgo.lock` in some cases (like if there is a binary).
    Lockfile,
    /// Adds a `.crabgo_vcs_info.json` file if in a (clean) git repo.
    VcsInfo(VcsInfo),
}
...
pub fn package(ws: &Workspace<'_>, opts: &PackageOpts<'_>) -> CrabgoResult<Option<Vec<FileLock>>> {
    ...
    if ws.root().join("Crabgo.lock").exists() {
        // Make sure the Crabgo.lock is up-to-date and valid.
        let _ = ops::resolve_ws(ws)?;
        // If Crabgo.lock does not exist, it will be generated by `build_lock`
...
fn build_ar_list(
    ...
    if pkg.include_lockfile() {
        result.push(ArchiveFile {
            rel_path: PathBuf::from("Crabgo.lock"),
            rel_str: "Crabgo.lock".to_string(),
...
/// Construct `Crabgo.lock` for the package to be published.
fn build_lock(ws: &Workspace<'_>, orig_pkg: &Package) -> CrabgoResult<String> {
    let config = ws.config();
    ...
    // Regenerate Crabgo.lock using the old one as a guide.
...
fn check_metadata(pkg: &Package, config: &Config) -> CrabgoResult<()> {
...
fn check_repo_state(
        ...
        if let Some(workdir) = repo.workdir() {
            ...
            if let Ok(status) = repo.status_file(path) {
                if (status & git2::Status::IGNORED).is_empty() {
                    debug!(
                        "found (git) Crabgo.toml at {:?} in workdir {:?}",
                        path, workdir
                    );
            ...
            config.shell().verbose(|shell| {
                shell.warn(format!(
                    "No (git) Crabgo.toml found at `{}` in workdir `{}`",
                    path.display(),
                    workdir.display()
                ))
            })?;
    ...
    // No VCS with a checked in `Crabgo.toml` found, so we don't know if the
    // directory is dirty or not, thus we have to assume that it's clean.
    return Ok(None);

    fn git(p: &Package, src_files: &[PathBuf], repo: &git2::Repository) -> CrabgoResult<GitVcsInfo> {
    ...
    fn collect_statuses(
        ...
        let this_dirty = repo_statuses.iter().filter_map(|entry| {
            ...
            if path.ends_with("Crabgo.lock") && entry.status() == git2::Status::IGNORED {
                // It is OK to include Crabgo.lock even if it is ignored.
                return None;
            }
    ...
    for ar_file in ar_files {
        ...
        match contents {
            ...
            FileContents::Generated(generated_kind) => {
                ...
                // use something nonzero to avoid rust-lang/crabgo#9512
                header.set_mtime(1);
                header.set_cksum();
...
/// Generate warnings when packaging Crabgo.lock, and the resolve have changed.
fn compare_resolve(
    config: &Config,
    current_pkg: &Package,
    ...
    for pkg_id in added {
        ...
        let extra = match removed_candidates.len() {
            ...
            1 => {
                ...
                // `[replace]` is not captured in Crabgo.lock.
                format!(
        ...
        let msg = format!(
            "package `{}` added to the packaged Crabgo.lock file{}",
            pkg_id, extra
        );
...
pub fn check_yanked(
    ...
    for (pkg_id, is_yanked) in results {
        if is_yanked? {
            config.shell().warn(format!(
                "package `{}` in Crabgo.lock is yanked in registry `{}`, {}",
                pkg_id,
...
fn hash_all(path: &Path) -> CrabgoResult<HashMap<PathBuf, u64>> {
    fn wrap(path: &Path) -> CrabgoResult<HashMap<PathBuf, u64>> {
...
fn check_filename(file: &Path, shell: &mut Shell) -> CrabgoResult<()> {
