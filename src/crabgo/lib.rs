...
//! # Crabgo as a library
//!
//! There are two places you can find API documentation of crabgo-the-library,
//!
//! - <https://docs.rs/crabgo>: targeted at external tool developers using crabgo-the-library
//!   - Released with every rustc release
//! - <https://doc.rust-lang.org/nightly/nightly-rustc/crabgo>: targeted at crabgo contributors
//!   - Updated on each update of the `crabgo` submodule in `rust-lang/rust`
//!
//! **WARNING:** Using Crabgo as a library has drawbacks, particulary the API is unstable,
//! and there is no clear path to stabilize it soon at the time of writing.  See [The Crabgo Book:
//! External tools] for more on this topic.
//!
//! ## Overview
//!
//! Major components of crabgo include:
//!
//! - [`ops`]:
...
//! - [`util::toml`]:
//!   This directory contains the code for parsing `Crabgo.toml` files.
//!   - [`ops::lockfile`]:
//!     This is where `Crabgo.lock` files are loaded and saved.
//!
//! Related crates:
//! - [`crabgo-platform`](https://crates.io/crates/crabgo-platform)
//!   ([nightly docs](https://doc.rust-lang.org/nightly/nightly-rustc/crabgo_platform)):
//!   This library handles parsing `cfg` expressions.
//! - [`crabgo-util`](https://crates.io/crates/crabgo-util)
//!   ([nightly docs](https://doc.rust-lang.org/nightly/nightly-rustc/crabgo_util)):
//!   This contains general utility code that is shared between crabgo and the testsuite
...
//!   This library is shared between crabgo and rustup and is used for finding their home directories.
//!   This is not directly depended upon with a `path` dependency; crabgo uses the version from crates.io.
//!   It is intended to be versioned and published independently of Rust's release system.
//!   Whenever a change needs to be made, bump the version in Crabgo.toml and `crabgo publish` it manually, and then update crabgo's `Crabgo.toml` to depend on the new version.
//! - [`crabgo-test-support`](https://github.com/rust-lang/crabgo/tree/master/crates/crabgo-test-support)
//!   ([nightly docs](https://doc.rust-lang.org/nightly/nightly-rustc/crabgo_test_support/index.html)):
//!   This contains a variety of code to support writing tests
//! - [`crabgo-test-macro`](https://github.com/rust-lang/crabgo/tree/master/crates/crabgo-test-macro)
//!   ([nightly docs](https://doc.rust-lang.org/nightly/nightly-rustc/crabgo_test_macro/index.html)):
//!   This is the `#[crabgo_test]` proc-macro used by the test suite to define tests.
//! - [`credential`](https://github.com/rust-lang/crabgo/tree/master/credential)
//!   This subdirectory contains several packages for implementing the
//!   experimental
//!   [credential-process](https://doc.rust-lang.org/nightly/crabgo/reference/unstable.html#credential-process)
//!   feature.
...
//!   This is a utility for generating crabgo's man pages. See [Building the man
//!   pages](https://github.com/rust-lang/crabgo/tree/master/src/doc#building-the-man-pages)
...
//! ### File Overview
//!
//! Files that interact with crabgo include
//!
//! - Package
//!   - `Crabgo.toml`: User-written project manifest, loaded with [`util::toml::TomlManifest`] and then
...
//!   - `Crabgo.lock`: Generally loaded with [`ops::resolve_ws`] or a variant of it into a [`core::resolver::Resolve`]
...
//! - `**/.crabgo/config.toml`: Environment dependent (env variables, files) configuration.  See
//!   [`util::config`]
//!
//! ## Contribute to Crabgo documentations
//!
//! The Crabgo team always continues improving all external and internal documentations.
//! If you spot anything could be better, don't hesitate to discuss with the team on
//! Zulip [`t-crabgo` stream], or [submit an issue] right on GitHub.
//! There is also an issue label [`A-documenting-crabgo-itself`],
//! which is generally for documenting user-facing [The Crabgo Book],
//! but the Crabgo team is welcome any form of enhancement for the [Crabgo Contributor Guide]
//! and this API documentation as well.
//!
//! [The Crabgo Book: External tools]: https://doc.rust-lang.org/stable/crabgo/reference/external-tools.html
//! [Crabgo Architecture Overview]: https://doc.crates.io/contrib/architecture
//! [`t-crabgo` stream]: https://rust-lang.zulipchat.com/#narrow/stream/246057-t-crabgo
//! [submit an issue]: https://github.com/rust-lang/crabgo/issues/new/choose
//! [`A-documenting-crabgo-itself`]: https://github.com/rust-lang/crabgo/labels/A-documenting-crabgo-itself
//! [The Crabgo Book]: https://doc.rust-lang.org/crabgo/
//! [Crabgo Contributor Guide]: https://doc.crates.io/contrib/
...
pub fn display_error(err: &Error, shell: &mut Shell) {
    ...
    {
        drop(shell.note("this is an unexpected crabgo internal error"));
        drop(
            shell.note(
                "we would appreciate a bug report: https://github.com/rust-lang/crabgo/issues/",
            ),
        );
        drop(shell.note(format!("crabgo {}", version())));
