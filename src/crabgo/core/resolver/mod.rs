...
//!   dependency in Crabgo (e.g., when you write `foo = "0.1.2"`) is
...
//! 1. With all features enabled (this is what gets saved to `Crabgo.lock`)
...
//! ## Performance
//!
//! Note that this is a relatively performance-critical portion of Crabgo. The
...
impl RemainingCandidates {
    ...
    fn next(
        ...
        for b in self.remaining.by_ref() {
            ...
            // semver. Crabgo dictates that you can't duplicate multiple
...
/// `conflicting_activations`.
///
/// Read <https://github.com/rust-lang/crabgo/pull/4834>
...
fn check_cycles(resolve: &Resolve) -> CrabgoResult<()> {
...
fn check_duplicate_pkgs_in_lockfile(resolve: &Resolve) -> CrabgoResult<()> {
