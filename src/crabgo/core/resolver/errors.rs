...
pub(super) fn activation_error(
    ...
    if !candidates.is_empty() {
        ...
        // (see https://github.com/rust-lang/crabgo/pull/5037) and I don't feel like changing it.
        ...
        for (p, r) in &conflicting_activations {
            match r {
                ...
                ConflictReason::Links(link) => {
                    ...
                    msg.push_str(&*dep.package_name());
                    msg.push_str("' value. For more information, see https://doc.rust-lang.org/crabgo/reference/resolver.html#links.");
                }
    ...
    let mut msg =
        if !candidates.is_empty() {
            ...
            // indicate that we updated a sub-package and forgot to run `crabgo
            // update`. In this case try to print a helpful error!
            if dep.source_id().is_path() && dep.version_req().is_locked() {
                msg.push_str(
                    "\nconsider running `crabgo update` to update \
