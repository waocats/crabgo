...
//!
//! [new feature resolver]: https://doc.rust-lang.org/nightly/crabgo/reference/resolver.html#feature-resolver-version-2
...
/// Flag to indicate if Crabgo is building *any* dev units (tests, examples, etc.).
///
...
/// `crabgo test` because the lib may need to be built 3 times instead of
/// twice.
...
impl ResolvedFeatures {
    ...
    /// not know which packages are activated (like `crabgo clean`).
    ...
    fn activated_features_int(
        ...
        pkg_id: PackageId,
        features_for: FeaturesFor,
    ) -> CrabgoResult<Vec<InternedString>> {
    ...
    ///
    /// Used by `crabgo fix --edition` to display any differences.
    ...
    fn do_resolve(
        ...
        for (member, cli_features) in &member_features {
            ...
            let fk = if self.track_for_host && self.is_proc_macro(member.package_id()) {
                ...
                // or running in `crabgo test`. Note that in a workspace, if
