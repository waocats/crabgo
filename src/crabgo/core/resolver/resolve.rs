...
pub struct Resolve {
    ...
    /// from `Crabgo.toml`. We need a `HashSet` here because the same package
    ...
    /// found in the `[metadata]` section of `Crabgo.lock`, preserved for
    /// forwards compatibility.
    metadata: Metadata,
    /// `[patch]` entries that did not match anything, preserved in
    /// `Crabgo.lock` as the `[[patch.unused]]` table array. Tracking unused
    /// patches helps prevent Crabgo from being forced to re-update the
    ...
    /// Version of the `Crabgo.lock` format, see
    /// `crabgo::core::resolver::encode` for more.
    version: ResolveVersion,
    summaries: HashMap<PackageId, Summary>,
}

/// A version to indicate how a `Crabgo.lock` should be serialized.
///
...
/// those generated by `crabgo update` (update everything) or building after
/// a `crabgo new` (where no lock file previously existed). This is also used
/// for *updated* lock files such as when a dependency is added or when a
/// version requirement changes. In this situation Crabgo's updating the lock
...
/// to the `Crabgo.lock` format, but we've yet to see how that strategy pans out.
...
impl Resolve {
    ...
    pub fn merge_from(&mut self, previous: &Resolve) -> CrabgoResult<()> {
...
",
                ...
                // that future Crabgo figured out how to checksum something or
...
",
    ...
    pub fn query(&self, spec: &str) -> CrabgoResult<PackageId> {
        PackageIdSpec::query_str(spec, self.iter())
    }

    pub fn specs_to_ids(&self, specs: &[PackageIdSpec]) -> CrabgoResult<Vec<PackageId>> {
    ...
    pub fn extern_crate_name_and_dep_name(
        ...
        to: PackageId,
        to_target: &Target,
    ) -> CrabgoResult<(InternedString, Option<InternedString>)> {
