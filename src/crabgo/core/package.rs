...
#
# When uploading crates to the registry Crabgo will automatically
# \"normalize\" Crabgo.toml files for maximal compatibility
# with all versions of Crabgo and also rewrite `path` dependencies
# to registry (e.g., crates.io) dependencies.
#
# If you are reading this file be aware that the original Crabgo.toml
# will likely look very different (and much more reasonable).
# See Crabgo.toml.orig for the original contents.
";

/// Information about a package that is available somewhere in the file system.
///
/// A package is a `Crabgo.toml` file plus all the files that are part of it.
#[derive(Clone)]
pub struct Package {
...
impl Package {
    ...
    pub fn to_registry_toml(&self, ws: &Workspace<'_>) -> CrabgoResult<String> {
        let manifest = self
            .manifest()
            .original()
    ...
    /// Returns if package should include `Crabgo.lock`.
...
pub struct Downloads<'a, 'cfg> {
    ...
    /// Global filesystem lock to ensure only one Crabgo is downloading at a
    /// time.
...
impl<'cfg> PackageSet<'cfg> {
    ...
    pub fn enable_download<'a>(&'a self) -> CrabgoResult<Downloads<'a, 'cfg>> {
    ...
    pub fn get_one(&self, id: PackageId) -> CrabgoResult<&Package> {
    ...
    pub fn get_many(&self, ids: impl IntoIterator<Item = PackageId>) -> CrabgoResult<Vec<&Package>> {
    ...
    pub fn download_accessible(
        ...
        // messages for Crabgo's testsuite. Perhaps there is a better ordering
...
impl<'a, 'cfg> Downloads<'a, 'cfg> {
    ...
    pub fn start(&mut self, id: PackageId) -> CrabgoResult<Option<&'a Package>> {
        self.start_inner(id)
            .with_context(|| format!("failed to download `{}`", id))
    }

    fn start_inner(&mut self, id: PackageId) -> CrabgoResult<Option<&'a Package>> {
        ...
        //
        // Currently Crabgo requests the `http2` feature of the `curl` crate
        // which means it should always be built in. On OSX, however, we ship
        // crabgo still linked against the system libcurl. Building curl with
        // ALPN support for HTTP/2 requires newer versions of OSX (the
        // SecureTransport API) than we want to ship Crabgo for. By linking Crabgo
    ...
    pub fn wait(&mut self) -> CrabgoResult<&'a Package> {
        let (dl, data) = loop {
    ...
    fn enqueue(&mut self, dl: Download<'cfg>, handle: Easy) -> CrabgoResult<()> {
    ...
    fn wait_for_curl(&mut self) -> CrabgoResult<(usize, Result<(), curl::Error>)> {
    ...
    fn tick(&self, why: WhyTick<'_>) -> CrabgoResult<()> {
