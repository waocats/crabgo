...
//!
//! If you consider [`ops::crabgo_compile::compile`] as a `rustc` driver but on
//! Crabgo side, this module is kinda the `rustc_interface` for that merits.
//! It contains all the interaction between Crabgo and the rustc compiler,
...
//! like what rustc has done[^1]. Also, no one knows if Crabgo really needs that.
...
//!   machinary happen between Crabgo and the compiler.
...
//!
//! [^1]: Maybe [`-Zbuild-plan`](https://doc.rust-lang.org/nightly/crabgo/reference/unstable.html#build-plan)
//!   was designed to serve that purpose but still [in flux](https://github.com/rust-lang/crabgo/issues/7614).
//!
//! [`ops::crabgo_compile::compile`]: crate::ops::compile
...
use crate::util::errors::{CrabgoResult, VerboseError};
use crate::util::interning::InternedString;
use crate::util::machine_message::{self, Message};
use crate::util::toml::TomlDebugInfo;
use crate::util::{add_path_args, internal, iter_join_onto, profile};
use crabgo_util::{paths, ProcessBuilder, ProcessError};
use rustfix::diagnostics::Applicability;

const RUSTDOC_CRATE_VERSION_FLAG: &str = "--crate-version";

// TODO: Rename this to `ExtraLinkArgFor` or else, and move to compiler/custom_build.rs?
/// Represents one of the instruction from `crabgo:rustc-link-arg-*` build script
/// instruction family.
///
/// In other words, indicates targets that custom linker arguments applies to.
#[derive(Clone, Hash, Debug, PartialEq, Eq)]
pub enum LinkType {
    /// Represents `crabgo:rustc-link-arg=FLAG`.
    All,
    /// Represents `crabgo:rustc-cdylib-link-arg=FLAG`.
    Cdylib,
    /// Represents `crabgo:rustc-link-arg-bins=FLAG`.
    Bin,
    /// Represents `crabgo:rustc-link-arg-bin=BIN=FLAG`.
    SingleBin(String),
    /// Represents `crabgo:rustc-link-arg-tests=FLAG`.
    Test,
    /// Represents `crabgo:rustc-link-arg-benches=FLAG`.
    Bench,
    /// Represents `crabgo:rustc-link-arg-examples=FLAG`.
    Example,
}
...
pub trait Executor: Send + Sync + 'static {
    ...
    /// In case of an `Err`, Crabgo will not continue with the build process for
    /// this package.
    fn exec(
        &self,
        cmd: &ProcessBuilder,
        ...
        target: &Target,
        mode: CompileMode,
        on_stdout_line: &mut dyn FnMut(&str) -> CrabgoResult<()>,
        on_stderr_line: &mut dyn FnMut(&str) -> CrabgoResult<()>,
    ) -> CrabgoResult<()>;
...
/// A `DefaultExecutor` calls rustc without doing anything else. It is Crabgo's
/// default behaviour.
#[derive(Copy, Clone)]
...
fn rustc(cx: &mut Context<'_, '_>, unit: &Unit, exec: &Arc<dyn Executor>) -> CrabgoResult<Work> {
    ...
    return Ok(Work::new(move |state| {
        ...
        fn verbose_if_simple_exit_code(err: Error) -> Error {
            ...
            {
                Some(n) if crabgo_util::is_simple_exit_code(n) => VerboseError::new(err).into(),
                _ => err,
            }
        }
        ...
        if rustc_dep_info_loc.exists() {
            ...
            // This mtime shift allows Crabgo to detect if a source file was
...
fn link_targets(cx: &mut Context<'_, '_>, unit: &Unit, fresh: bool) -> CrabgoResult<Work> {
    let bcx = cx.bcx;
    let outputs = cx.outputs(unit)?;
    ...
    Ok(Work::new(move |state| {
        ...
        // above. This means that `crabgo build` will produce binaries in
        // `target/debug` which one probably expects.
        let mut destinations = vec![];
        for output in outputs.iter() {
            let src = &output.path;
            // This may have been a `crabgo rustc` command which changes the
...
fn rustdoc(cx: &mut Context<'_, '_>, unit: &Unit) -> CrabgoResult<Work> {
    let bcx = cx.bcx;
    ...
    let scrape_output_path = |unit: &Unit| -> CrabgoResult<PathBuf> {
    ...
    let scrape_outputs = if should_include_scrape_units {
        ...
        Some(
            cx.bcx
                ...
                .collect::<CrabgoResult<HashMap<_, _>>>()?,
        )
    } else {
        None
    };
...
/// Forwards [`-Zallow-features`] if it is set for crabgo.
///
/// [`-Zallow-features`]: https://doc.rust-lang.org/nightly/crabgo/reference/unstable.html#allow-features
...
///
/// Crabgo always uses JSON output. This has several benefits, such as being
...
/// which Crabgo will extract and display to the user.
///
...
fn build_base_args(
    ...
    if unit.mode.is_check() {
        ...
        // future Crabgo session as part of a pipelined compile.
    ...
    if test && unit.target.harness() {
        ...
        // Crabgo has historically never compiled `--test` binaries with
        ...
        // passed when compiling the test. We require, in Crabgo, an unstable
...
///
/// [`check-cfg`]: https://doc.rust-lang.org/nightly/crabgo/reference/unstable.html#check-cfg
        ...
        if let Some(dep) = deps.iter().find(|dep| {
            ...
            bcx.config.shell().warn(format!(
                "The package `{}` \
                 ...
                 Crabgo.toml. This warning might turn into a hard error in the future.",
...
) -> CrabgoResult<Vec<OsString>> {
    let mut result = Vec::new();
    let deps = cx.unit_deps(unit);

    // Closure to add one dependency to `result`.
    let mut link_to =
        |dep: &UnitDep, extern_crate_name: InternedString, noprelude: bool| -> CrabgoResult<()> {
...
struct OutputOptions {
    /// What format we're emitting from Crabgo itself.
...
fn on_stderr_line_inner(
    ...
    // Depending on what we're emitting from Crabgo itself, we figure out what to
    // do with this JSON message.
    match options.format {
        // In the "human" output formats (human/short) or if diagnostic messages
        // from rustc aren't being included in the output of Crabgo's JSON
        ...
        | MessageFormat::Json {
            ...
            // if we turn that off. Crabgo only cares about the 1 field listed
            // here.
            #[derive(serde::Deserialize)]
        ...
        // enabled. Crabgo always asks for ANSI colors from rustc. This allows
    ...
    // information.
    //
    // Look for a matching directive and inform Crabgo internally that a
    ...
    // from the compiler, so wrap it in an external Crabgo JSON message
    ...
    // instead. We want the stdout of Crabgo to always be machine parseable as
...
fn apply_env_config(config: &crate::Config, cmd: &mut ProcessBuilder) -> CrabgoResult<()> {
    for (key, value) in config.env_config()?.iter() {
        // never override a value that has already been set by crabgo
