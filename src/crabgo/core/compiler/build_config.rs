...
use crate::util::{CrabgoResult, Config, RustfixDiagnosticServer};
...
pub struct BuildConfig {
    ...
    /// Force Crabgo to do a full rebuild and treat each target as changed.
    ...
    /// A thread used by `crabgo fix` to receive messages on a socket regarding
...
fn default_parallelism() -> CrabgoResult<u32> {
...
impl BuildConfig {
    ...
    pub fn single_requested_kind(&self) -> CrabgoResult<CompileKind> {
...
pub enum MessageFormat {
    Human,
    Json {
        /// Whether rustc diagnostics are rendered by crabgo or included into the
        /// output stream.
