...
//!
//! Rust code is typically organized as a set of Crabgo packages. The
...
fn deps_of_roots(roots: &[Unit], state: &mut State<'_, '_>) -> CrabgoResult<()> {
    for unit in roots.iter() {
        // Dependencies of tests/benches should not have `panic` set.
        // We check the global test mode to see if we are running in `crabgo
...
///
/// The result is stored in `state.unit_dependencies`.
fn deps_of(unit: &Unit, state: &mut State<'_, '_>, unit_for: UnitFor) -> CrabgoResult<()> {
        ...
        && (unit.target.is_test() || unit.target.is_bench())
    {
        ...
        ret.extend(
            unit.pkg
                ...
                .collect::<CrabgoResult<Vec<UnitDep>>>()?,
        );
    }
