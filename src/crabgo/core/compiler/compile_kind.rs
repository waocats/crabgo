    ...
    pub fn from_requested_targets(
        ...
        let dedup = |targets: &[String]| {
            Ok(targets
                ...
                // more than once...
                .collect::<CrabgoResult<BTreeSet<_>>>()?
...
/// Abstraction for the representation of a compilation target that Crabgo has.
///
...
/// here in Crabgo. Note that for JSON target files this `CompileTarget` stores a
/// full canonicalized path to the target.
///
/// The main reason for this existence is to handle JSON target files where when
/// we call rustc we pass full paths but when we use it for Crabgo's purposes
...
impl CompileTarget {
    pub fn new(name: &str) -> CrabgoResult<CompileTarget> {
        let name = name.trim();
        if name.is_empty() {
    ...
    /// Crabgo for configuration and such.
    ///
