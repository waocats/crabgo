...
use crate::util::{self, CrabgoResult, StableHasher};
...
/// output files.
///
/// Normally crabgo updates ship with rustc updates which will
/// cause a new hash due to the rustc version changing, but this allows
/// crabgo to be extra careful to deal with different versions of crabgo that
...
///
/// This also acts as the main layer of caching provided by Crabgo.
/// For example, we want to cache `crabgo build` and `crabgo doc` separately, so that running one
...
impl<'a, 'cfg: 'a> CompilationFiles<'a, 'cfg> {
    ...
    ) -> CrabgoResult<Arc<Vec<OutputFile>>> {
        self.outputs[unit]
    ...
    fn uplift_to(&self, unit: &Unit, file_type: &FileType, from_path: &Path) -> Option<PathBuf> {
        ...
        //   latest copies (even if the dylib was built from a previous crabgo
        ...
        //   historically how Crabgo has operated. This is primarily useful to
    ...
    fn calc_outputs(
        ...
        let ret = match unit.mode {
            ...
            CompileMode::Doctest => {
                ...
                // but Crabgo does not know about that.
                vec![]
            }
            CompileMode::Docscrape => {
                // The file name needs to be stable across Crabgo sessions.
    ...
    fn calc_outputs_rustc(
        ...
        for file_type in file_types {
            ...
            // be the name of the binary provided by the user in `Crabgo.toml`.
...
fn should_use_metadata(bcx: &BuildContext<'_, '_>, unit: &Unit) -> bool {
    ...
    // See https://github.com/rust-lang/crabgo/issues/3005
