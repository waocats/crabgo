...
impl<'a, 'cfg> Context<'a, 'cfg> {
    pub fn new(bcx: &'a BuildContext<'a, 'cfg>) -> CrabgoResult<Self> {
    ...
    ///
    /// See [`ops::crabgo_compile`] for a higher-level view of the compile process.
    ///
    /// [`ops::crabgo_compile`]: ../../../ops/crabgo_compile/index.html
    pub fn compile(mut self, exec: &Arc<dyn Executor>) -> CrabgoResult<Compilation<'cfg>> {
        ...
        // any versioning (See https://github.com/rust-lang/crabgo/issues/8461).
    ...
    pub fn get_executable(&mut self, unit: &Unit) -> CrabgoResult<Option<PathBuf>> {
    ...
    pub fn prepare_units(&mut self) -> CrabgoResult<()> {
    ...
    /// place.
    pub fn prepare(&mut self) -> CrabgoResult<()> {
    ...
    pub fn outputs(&self, unit: &Unit) -> CrabgoResult<Arc<Vec<OutputFile>>> {
    ...
    /// Returns the list of filenames read by crabgo to generate the [`BuildContext`]
    /// (all `Crabgo.toml`, etc.).
    pub fn build_plan_inputs(&self) -> CrabgoResult<Vec<PathBuf>> {
    ...
    /// See <https://github.com/rust-lang/crabgo/issues/6313> for more.
    fn check_collisions(&self) -> CrabgoResult<()> {
        ...
        let suggestion =
            "Consider changing their names to be unique or compiling them separately.\n\
             ...
             <https://github.com/rust-lang/crabgo/issues/6313>.";
         ...
         -> CrabgoResult<()> {
            if unit.target.name() == other_unit.target.name() {
                ...
                self.bcx.config.shell().warn(format!(
                    ...
                    {}\n\
                    If this looks unexpected, it may be a bug in Crabgo. Please file a bug report at\n\
                    https://github.com/rust-lang/crabgo/issues/ with as much information as you\n\
                    can provide.\n\
                    crabgo {} running on `{}` target `{}`\n\
                    First unit: {:?}\n\
                    Second unit: {:?}",
                    describe_collision(unit, other_unit, path),
                    suggestion,
        ...
        fn doc_collision_error(unit: &Unit, other_unit: &Unit) -> CrabgoResult<()> {
            bail!(
                "document output filename collision\n\
                 ...
                 or marking one with `doc = false` in Crabgo.toml.",
                unit.target.kind().description(),
                unit.target.name(),
                unit.pkg,
                other_unit.target.kind().description(),
    ...
    /// will be the metadata of the Crabgo unit that generated the current library's rmeta file,
