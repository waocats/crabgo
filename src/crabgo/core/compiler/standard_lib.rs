...
pub fn parse_unstable_flag(value: Option<&str>) -> Vec<String> {
    ...
    // declare dependencies in Crabgo.toml.
...
pub fn resolve_std<'cfg>(
    ...
    crates: &[String],
) -> CrabgoResult<(PackageSet<'cfg>, Resolve, ResolvedFeatures)> {
    ...
    let current_manifest = src_path.join("library/test/Crabgo.toml");
...
fn detect_sysroot_src_path(target_data: &RustcTargetData<'_>) -> CrabgoResult<PathBuf> {
    ...
    let src_path = target_data
        ...
        .join("src")
        .join("rust");
    let lock = src_path.join("Crabgo.lock");
    if !lock.exists() {
        let msg = format!(
