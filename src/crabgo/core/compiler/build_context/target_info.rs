...
use crate::util::{CrabgoResult, Rustc};
use anyhow::Context as _;
use crabgo_platform::{Cfg, CfgExpr};
...
pub struct TargetInfo {
    ...
    /// `Some((prefix, suffix))`, for example `libcrabgo.so` would be
...
impl FileType {
    ...
    /// The filename for this FileType that Crabgo should use when "uplifting"
...
impl TargetInfo {
    ...
    pub fn new(
        ...
        loop {
            ...
            // `cfg` flags define which `.crabgo/config` sections apply, and they
    ...
    fn not_user_specific_cfg(cfg: &CrabgoResult<Cfg>) -> bool {
    ...
    fn file_types(
        ...
        flavor: FileFlavor,
        target_triple: &str,
    ) -> CrabgoResult<Option<Vec<FileType>>> {
        ...
        if target_triple.starts_with("wasm32-") && crate_type == CrateType::Bin && suffix == ".js" {
            ...
            ret.push(FileType {
                ...
                // `foo_bar.wasm`. Crabgo will translate the underscore and
    ...
    fn discover_crate_type(&self, crate_type: &CrateType) -> CrabgoResult<Option<(String, String)>> {
    ...
    pub fn rustc_outputs(
        ...
        target_triple: &str,
    ) -> CrabgoResult<(Vec<FileType>, Vec<CrateType>)> {
        match mode {
...
/// Rust library like `libcrabgo.rlib`, we have prefix "lib" and suffix "rlib").
///
...
) -> CrabgoResult<Option<(String, String)>> {
...
///
///  - `target.*.rustflags` from the config (.crabgo/config)
...
/// necessary to retain backwards compatibility with older versions of Crabgo.
fn extra_args(
    config: &Config,
    requested_kinds: &[CompileKind],
    host_triple: &str,
    ...
    // particular (inconsistent) past Crabgo behavior where host artifacts _do_ pick up rustflags
    // set elsewhere when `--target` isn't passed.
    if kind.is_host() {
        if target_applies_to_host && requested_kinds == [CompileKind::Host] {
            // This is the past Crabgo behavior where we fall back to the same logic as for other
...
fn rustflags_from_target(
    ...
    kind: CompileKind,
    flag: Flags,
) -> CrabgoResult<Option<Vec<String>>> {
...
/// See [`extra_args`] for more.
fn rustflags_from_build(config: &Config, flag: Flags) -> CrabgoResult<Option<Vec<String>>> {
...
impl<'cfg> RustcTargetData<'cfg> {
    pub fn new(
        ws: &Workspace<'cfg>,
        requested_kinds: &[CompileKind],
    ) -> CrabgoResult<RustcTargetData<'cfg>> {
    ...
    fn merge_compile_kind(&mut self, kind: CompileKind) -> CrabgoResult<()> {
        if let CompileKind::Target(target) = kind {
    ...
    /// configuration in Crabgo or presenting to users.
...
impl RustDocFingerprint {
    ...
    /// `Workspace`'s docs was the same as the one is currently being used in this `crabgo doc`
    /// call.
    ///
    ...
    /// any versioning.
    pub fn check_rustdoc_fingerprint(cx: &Context<'_, '_>) -> CrabgoResult<()> {
        ...
        let write_fingerprint = || -> CrabgoResult<()> {
            paths::write(
                &fingerprint_path,
        ...
        let rustdoc_data = match paths::read(&fingerprint_path) {
            ...
            // `crabgo doc` in a way that deleting it would break it.
        ...
        fn clean_doc(path: &Path) -> CrabgoResult<()> {
            let entries = path
                .read_dir()
