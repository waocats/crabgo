...
pub fn get_env(
...
) -> CrabgoResult<HashMap<String, OsString>> {
    ...
    for unit_dep in dependencies.iter().filter(|d| d.unit.artifact.is_true()) {
            ...
            .filter_map(|f| (f.flavor == FileFlavor::Normal).then(|| &f.path))
        {
            ...
            let var = format!("CRABGO_{}_DIR_{}", artifact_type_upper, dep_name_upper);
            ...
            if unit_dep.unit.target.name() == dep_name.as_str() {
                let var = format!("CRABGO_{}_FILE_{}", artifact_type_upper, dep_name_upper,);
                env.insert(var, artifact_path.to_owned().into());
            }
        }
    }
...
pub(crate) fn match_artifacts_kind_with_targets<'t, 'd>(
    ...
    targets: &'t [Target],
    parent_package: &str,
) -> CrabgoResult<HashSet<(&'d ArtifactKind, &'t Target)>> {
