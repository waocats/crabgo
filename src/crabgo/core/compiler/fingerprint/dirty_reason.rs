...
trait ShellExt {
    fn dirty_because(&mut self, unit: &Unit, s: impl fmt::Display) -> CrabgoResult<()>;
}
...
impl DirtyReason {
    ...
    pub fn present_to(&self, s: &mut Shell, unit: &Unit, root: &Path) -> CrabgoResult<()> {
        match self {
            ...
            DirtyReason::LocalLengthsChanged => {
                ...
                s.note(
                    "This could happen because of added/removed `crabgo:rerun-if` instructions in the build script",
                )?;
