//! # Fingerprints
//!
//! This module implements change-tracking so that Crabgo can know whether or
//! not something needs to be recompiled. A Crabgo [`Unit`] can be either "dirty"
...
//! ### dep-info files
//!
//! Crabgo has several kinds of "dep info" files:
//!
...
//!
//! Crabgo passes the `--emit=dep-info` flag to `rustc` so that `rustc` will
...
//! the crate. This file is used by Crabgo to know which files to check to see
//! if the crate will need to be rebuilt. Example:
//!
//! ```makefile
//! /path/to/target/debug/deps/crabgo-b6219d178925203d: src/bin/main.rs src/bin/crabgo/cli.rs # … etc.
//! ```
//!
//! #### Fingerprint dep-info files
//!
//! After `rustc` exits successfully, Crabgo will read the first kind of dep
...
//!
//! Note that Crabgo parses the special `# env-var:...` comments in dep-info
//! files to learn about environment variables that the rustc compile depends on.
//! Crabgo then later uses this to trigger a recompile if a referenced env var
//! changes (even if the source didn't change).
//!
//! #### dep-info files for build system integration.
//!
//! There is also a third dep-info file. Crabgo will extend the file created by
...
//! primarily to support rustc development, so that Crabgo can check the
//! implicit dependency to the standard library (which lives in the sysroot).
//! We want Crabgo to recompile whenever the standard library rlib/dylibs
...
//! running. Crabgo can't know whether or not the file was included in the
...
//!
//! Rustdoc does not emit a dep-info file, so Crabgo currently has a relatively
...
//! this mode, Crabgo will use [`LocalFingerprint::Precalculated`]. See the
...
//!   a project directory Crabgo will continue to preserve all build artifacts
...
//! test suite nowadays and lord knows Crabgo gets enough testing in the wild!
//!
//! ## Build scripts
//!
...
//! output directory in a file called `output`. Crabgo parses this file when
...
//! used, the Package ID will change, causing a rebuild). Crabgo currently
...
//! number of problems, particularly with Crabgo's testsuite which does rapid
...
//! [`A-rebuild-detection`]: https://github.com/rust-lang/crabgo/issues?q=is%3Aissue+is%3Aopen+label%3AA-rebuild-detection
...
use crabgo_util::{paths, ProcessBuilder};
...
pub fn prepare_target(cx: &mut Context<'_, '_>, unit: &Unit, force: bool) -> CrabgoResult<Job> {
    ...
    // fingerprint at step 3 ensures that Crabgo never mistakes a partially
...
/// `DependencyQueue`, but it also needs to be retained here because Crabgo can
...
pub struct Fingerprint {
    ...
    /// `crabgo rustc` or `crabgo rustdoc`.
    ///
    /// [`Profile`]: crate::core::profiles::Profile
    /// [`CompileMode`]: crate::core::compiler::CompileMode
    profile: u64,
...
enum LocalFingerprint {
    ...
    /// that's overall a mistake and causes bugs in Crabgo. We shouldn't use this
    /// for build scripts.
    Precalculated(String),

    /// This is used for crate compilations. The `dep_info` file is a relative
    /// path anchored at `target_root(...)` to the dep-info file that Crabgo
                    ...
                    let current = if key == CARGO_ENV {
                        Some(
                            crabgo_exe
                                ...
                                .ok_or_else(|| {
                                    format_err!(
                                        "crabgo exe path {} must be valid UTF-8",
                                        crabgo_exe.display()
                                    )
                                })?
                                .to_string(),
...
impl Fingerprint {
    ...
    fn check_filesystem(
        ...
        for local in self.local.get_mut().unwrap().iter() {
            if let Some(item) =
                local.find_stale_item(mtime_cache, pkg_root, target_root, crabgo_exe, config)?
            {
                item.log();
...
impl DepFingerprint {
    fn new(cx: &mut Context<'_, '_>, parent: &Unit, dep: &UnitDep) -> CrabgoResult<DepFingerprint> {
...
///
/// This fingerprint is used by Crabgo to learn about when information such as:
///
...
/// dependencies.
fn calculate(cx: &mut Context<'_, '_>, unit: &Unit) -> CrabgoResult<Arc<Fingerprint>> {
    ...
    let crabgo_exe = cx.bcx.config.crabgo_exe()?;
...
fn calculate_normal(cx: &mut Context<'_, '_>, unit: &Unit) -> CrabgoResult<Fingerprint> {
    let deps = {
...
fn calculate_run_custom_build(cx: &mut Context<'_, '_>, unit: &Unit) -> CrabgoResult<Fingerprint> {
...
By default, if your project contains a build script, crabgo scans all files in
...
See https://doc.rust-lang.org/crabgo/reference/build-scripts.html#rerun-if-changed for more information.";
...
/// overridden via `.crabgo/config.toml`. The closure is much more complicated.
///
...
/// `&BuildDeps`. This is the parsed version of a build script, and when Crabgo
...
fn build_script_local_fingerprints(
    ...
    Box<
        dyn FnOnce(
                &BuildDeps,
                Option<&dyn Fn() -> CrabgoResult<String>>,
            ) -> CrabgoResult<Option<Vec<LocalFingerprint>>>
            + Send,
    >,
    bool,
) {
    ...
    if let Some(fingerprint) = build_script_override_fingerprint(cx, unit) {
        ...
        return (
            Box::new(
                move |_: &BuildDeps, _: Option<&dyn Fn() -> CrabgoResult<String>>| {
    ...
    // longstanding bug, in Crabgo. Recent refactorings just made it painfully
    // obvious.
    let pkg_root = unit.pkg.root().to_path_buf();
    let target_dir = target_root(cx);
    let calculate =
        move |deps: &BuildDeps, pkg_fingerprint: Option<&dyn Fn() -> CrabgoResult<String>>| {
...
fn write_fingerprint(loc: &Path, fingerprint: &Fingerprint) -> CrabgoResult<()> {
...
pub fn prepare_init(cx: &mut Context<'_, '_>, unit: &Unit) -> CrabgoResult<()> {
...
fn compare_old_fingerprint(
    ...
    mtime_on_use: bool,
) -> CrabgoResult<Option<DirtyReason>> {
...
fn log_compare(unit: &Unit, compare: &CrabgoResult<Option<DirtyReason>>) {
    match compare {
        Ok(None) => {}
        Ok(Some(reason)) => {
            info!(
...
/// Parses Crabgo's internal [`EncodedDepInfo`] structure that was previously
/// serialized to disk.
///
/// Note that this is not rustc's `*.d` files.
///
/// Also note that rustc's `*.d` files are translated to Crabgo-specific
...
pub fn parse_dep_info(
    ...
    target_root: &Path,
    dep_info: &Path,
) -> CrabgoResult<Option<RustcDepInfo>> {
    ...
    let info = match EncodedDepInfo::parse(&data) {
        Some(info) => info,
        None => {
            log::warn!("failed to parse crabgo's dep-info at {:?}", dep_info);
            return Ok(None);
        }
    };
...
fn pkg_fingerprint(bcx: &BuildContext<'_, '_>, pkg: &Package) -> CrabgoResult<String> {
    ...
    I::Item: AsRef<Path>,
{
    ...
    for path in paths {
        ...
        // a file is updated at most 10ms after a build starts then Crabgo may not
        ...
        // Unfortunately this became problematic when (in #6484) crabgo switch to more accurately
...
/// Parses the dep-info file coming out of rustc into a Crabgo-specific format.
///
...
/// re-serialized into the `crabgo_dep_info` path in a Crabgo-specific format.
///
/// The `pkg_root` argument here is the absolute path to the directory
/// containing `Crabgo.toml` for this crate that was compiled. The paths listed
...
///
/// The serialized Crabgo format will contain a list of files, all of which are
/// relative if they're under `root`. or absolute if they're elsewhere.
pub fn translate_dep_info(
    rustc_dep_info: &Path,
    crabgo_dep_info: &Path,
    rustc_cwd: &Path,
    pkg_root: &Path,
    target_root: &Path,
    ...
    // variables are synthesized by Crabgo and/or the build script, and the
    ...
    paths::write(crabgo_dep_info, on_disk_info.serialize()?)?;
    Ok(())
}
...
/// Crabgo will read it for crates on all future compilations.
#[derive(Default)]
...
impl EncodedDepInfo {
    ...
    fn serialize(&self) -> CrabgoResult<Vec<u8>> {
        let mut ret = Vec::new();
        let dst = &mut ret;
...
pub fn parse_rustc_dep_info(rustc_dep_info: &Path) -> CrabgoResult<RustcDepInfo> {
    ...
    fn unescape_env(s: &str) -> CrabgoResult<String> {
