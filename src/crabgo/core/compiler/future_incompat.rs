...
//!
//! Here is an overview of how Crabgo handles future-incompatible reports.
//!
//! ## Receive reports from the compiler
//!
//! When receiving a compiler message during a build, if it is effectively
//! a [`FutureIncompatReport`], Crabgo gathers and forwards it as a
...
//! Crabgo saves the report on disk, and displays it directly if requested
//! via command line or configuration. The information of the on-disk file can
//! be found in [`FUTURE_INCOMPAT_FILE`].
//!
//! During the persistent process, Crabgo will attempt to query the source of
...
//!
//! Users can run `crabgo report future-incompat` to retrieve a report. This is
//! done by [`OnDiskReports::load`]. Crabgo simply prints reports to the
//! standard output.
//!
//! [1]: https://doc.rust-lang.org/nightly/crabgo/reference/future-incompat-report.html
...
use crate::util::{iter_join, CrabgoResult, Config};
...
pub struct OnDiskReports {
    /// A schema version number, to handle older crabgo's from trying to read
...
struct OnDiskReport {
    ...
    /// is stable across multiple runs of `crabgo`
...
impl OnDiskReports {
    ...
    pub fn load(ws: &Workspace<'_>) -> CrabgoResult<OnDiskReports> {
        ...
        if on_disk_reports.version != ON_DISK_VERSION {
            bail!("unable to read reports; reports were saved from a future version of Crabgo");
        }
        Ok(on_disk_reports)
    }
...
{updated_versions}\n",
  ...
  - {package_spec}
  - Repository: {url}
  - Detailed warning command: `crabgo report future-incompatibilities --id {id} --package {package_spec}`",
...
section in `Crabgo.toml` to use your own version of the dependency. For more
information, see:
https://doc.rust-lang.org/crabgo/reference/overriding-dependencies.html#the-patch-section
        ",
    ...
    if bcx.build_config.future_incompat_report {
        ...
        drop(bcx.config.shell().note(&format!(
            "this report can be shown with `crabgo report \
        ...
        drop(bcx.config.shell().note(&format!(
            "to see what the problems were, use the option \
             `--future-incompat-report`, or run `crabgo report \
             future-incompatibilities --id {}`",
            saved_report_id
        )));
    }
