...
use crate::util::{CrabgoResult, Config};
...
struct UnitTime {
    unit: Unit,
    /// A string describing the crabgo target.
    target: String,
...
impl<'cfg> Timings<'cfg> {
    ...
    fn report_html(&self, cx: &Context<'_, '_>, error: &Option<anyhow::Error>) -> CrabgoResult<()> {
        let duration = self.start.elapsed().as_secs_f64();
        let timestamp = self.start_str.replace(&['-', ':'][..], "");
        let timings_path = cx.files().host_root().join("crabgo-timings");
        paths::create_dir_all(&timings_path)?;
        let filename = timings_path.join(format!("crabgo-timing-{}.html", timestamp));
        ...
        let unstamped_filename = timings_path.join("crabgo-timing.html");
...
"#,
    ...
    fn write_js_data(&self, f: &mut impl Write) -> CrabgoResult<()> {
    ...
    fn write_unit_table(&self, f: &mut impl Write) -> CrabgoResult<()> {
        write!(
            f,
            r#"
...
<html>
<head>
  <title>Crabgo Build Timings — {ROOTS}</title>
  <meta charset="utf-8">
<style type="text/css">
html {
...
<h1>Crabgo Build Timings</h1>
See <a href="https://doc.rust-lang.org/nightly/crabgo/reference/timings.html">Documentation</a>
"#;
