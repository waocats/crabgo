...
use crate::util::{config, CrabgoResult, Config};
...
impl<'cfg> Compilation<'cfg> {
    pub fn new<'a>(bcx: &BuildContext<'a, 'cfg>) -> CrabgoResult<Compilation<'cfg>> {
    ...
    /// Returns a [`ProcessBuilder`] appropriate for running a process for the
    /// target platform. This is typically used for `crabgo run` and `crabgo
    /// test`.
    ///
    ...
    fn fill_env(
        ...
        if cfg!(target_os = "macos") && dylib_path_is_empty {
            ...
            // set or set to an empty string. Since Crabgo is explicitly setting
...
) -> CrabgoResult<HashMap<CompileKind, PathBuf>> {
    bcx.all_kinds
        .iter()
        .map(|&kind| {
...
fn target_runner(
    bcx: &BuildContext<'_, '_>,
    kind: CompileKind,
) -> CrabgoResult<Option<(PathBuf, Vec<String>)>> {
