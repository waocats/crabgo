...
use crate::util::{internal, CrabgoResult};
use crabgo_util::paths;
use log::debug;

/// Bacially just normalizes a given path and converts it to a string.
fn render_filename<P: AsRef<Path>>(path: P, basedir: Option<&str>) -> CrabgoResult<String> {
    fn wrap_path(path: &Path) -> CrabgoResult<String> {
        path.to_str()
...
///
/// Crabgo emits its own dep-info files in the output directory. This is
...
pub fn output_depinfo(cx: &mut Context<'_, '_>, unit: &Unit) -> CrabgoResult<()> {
    let bcx = cx.bcx;
