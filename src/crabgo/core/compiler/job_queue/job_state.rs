...
pub struct JobState<'a, 'cfg> {
    ...
    // Historical versions of Crabgo made use of the `'a` argument here, so to
...
impl<'a, 'cfg> JobState<'a, 'cfg> {
    ...
    pub fn stdout(&self, stdout: String) -> CrabgoResult<()> {
    ...
    pub fn stderr(&self, stderr: String) -> CrabgoResult<()> {
        if let Some(dedupe) = self.output {
    ...
    pub fn emit_diag(&self, level: String, diag: String, fixable: bool) -> CrabgoResult<()> {
        if let Some(dedupe) = self.output {
    ...
    /// See [`Message::Warning`].
    pub fn warning(&self, warning: String) -> CrabgoResult<()> {
    ...
    pub(super) fn run_to_finish(self, job: Job) {
        ...
        // 3. We forgot to call `rmeta_produced` and there's a bug in Crabgo.
        //
        ...
        // shouldn't panic unless there's a bug in Crabgo, so we just need
        // to make sure nothing hangs by accident.
        struct FinishOnDrop<'a> {
            messages: &'a Queue<Message>,
            id: JobId,
            result: Option<CrabgoResult<()>>,
        }
