...
pub struct Work {
    inner: Box<dyn FnOnce(&JobState<'_, '_>) -> CrabgoResult<()> + Send>,
}
...
impl Work {
    ...
    where
        F: FnOnce(&JobState<'_, '_>) -> CrabgoResult<()> + Send + 'static,
    {
    ...
    pub fn call(self, tx: &JobState<'_, '_>) -> CrabgoResult<()> {
        (self.inner)(tx)
    }
...
impl Job {
    ...
    /// computation.
    pub fn run(self, state: &JobState<'_, '_>) -> CrabgoResult<()> {
