...
pub struct BuildScripts {
    ...
    ///
    /// Crabgo will use this `to_link` vector to add `-L` flags to compiles as we
...
pub fn prepare(cx: &mut Context<'_, '_>, unit: &Unit) -> CrabgoResult<Job> {
...
fn build_work(cx: &mut Context<'_, '_>, unit: &Unit) -> CrabgoResult<Job> {
...
impl BuildOutput {
    ...
    pub fn parse(
        ...
        for line in input.split(|b| *b == b'\n') {
            ...
            if iter.next() != Some("crabgo") {
                // skip this line since it doesn't start with "crabgo:"
                continue;
            }
            ...
            let (key, value) = match (key, value) {
                ...
                // Line started with `crabgo:` but didn't match `key=value`.
                _ => bail!("invalid output in {}: `{}`\n\
                    Expected a line with `crabgo:key=value` with an `=` character, \
                    but none was found.\n\
            ...
            macro_rules! check_and_add_target {
                ($target_kind: expr, $is_target_kind: expr, $link_type: expr) => {
                    if !targets.iter().any(|target| $is_target_kind(target)) {
                        bail!(
                            "invalid instruction `crabgo:{}` from {}\n\
            ...
            match key {
                ...
                "rustc-link-arg-cdylib" | "rustc-cdylib-link-arg" => {
                    if !targets.iter().any(|target| target.is_cdylib()) {
                        warnings.push(format!(
                            "crabgo:{} was specified in the build script of {}, \
                             ...
                             <https://github.com/rust-lang/crabgo/issues/9562>.",
                            key, pkg_descr
                        ));
                    }
                ...
                "rustc-link-arg-bin" => {
                    ...
                    let arg = parts.next().ok_or_else(|| {
                        anyhow::format_err!(
                            "invalid instruction `crabgo:{}={}` from {}\n\
                                The instruction should have the form crabgo:{}=BIN=ARG",
                            key,
                            value,
                            whence,
                            key
                ...
                "rustc-check-cfg" => {
                    if extra_check_cfg {
                        ...
                        warnings.push(format!("crabgo:{} requires -Zcheck-cfg=output flag", key));
                    }
                }
                "rustc-env" => {
                    let (key, val) = BuildOutput::parse_rustc_env(&value, &whence)?;
                    // Build scripts aren't allowed to set RUSTC_BOOTSTRAP.
                    // See https://github.com/rust-lang/crabgo/issues/7088.
                    if key == "RUSTC_BOOTSTRAP" {
                        // If RUSTC_BOOTSTRAP is already set, the user of Crabgo knows about
                        ...
                        // NOTE: crabgo only allows nightly features on RUSTC_BOOTSTRAP=1, but we
                            ...
                            || rustc_bootstrap_allows(library_name.as_deref())
                        {
                            ...
                            bail!("Cannot set `RUSTC_BOOTSTRAP={}` from {}.\n\
                                note: Crates cannot set `RUSTC_BOOTSTRAP` themselves, as doing so would subvert the stability guarantees of Rust for your project.\n\
                                help: If you're sure you want to do this in your project, set the environment variable `RUSTC_BOOTSTRAP={}` before running crabgo instead.",
                                val,
                                whence,
    ...
    pub fn parse_rustc_flags(
        value: &str,
        whence: &str,
    ) -> CrabgoResult<(Vec<PathBuf>, Vec<String>)> {
    ...
    pub fn parse_rustc_env(value: &str, whence: &str) -> CrabgoResult<(String, String)> {
...
fn prepare_metabuild(cx: &Context<'_, '_>, unit: &Unit, deps: &[String]) -> CrabgoResult<()> {
...
pub fn build_map(cx: &mut Context<'_, '_>) -> CrabgoResult<()> {
