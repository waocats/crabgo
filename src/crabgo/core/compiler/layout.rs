...
//!     # Crabgo-specific directories added in the future should use some prefix
//!     # like `.` to avoid name collisions.
//!     debug/  # or release/
//!
//!         # File used to lock the directory to prevent multiple crabgo processes
...
//!     doc/
//!
//!     # Used by `crabgo package` and `crabgo publish` to build a `.crate` file.
//!     package/
//!
...
use crate::util::{CrabgoResult, FileLock};
...
pub struct Layout {
    ...
    tmp: PathBuf,
    /// The lockfile for a build (`.crabgo-lock`). Will be unlocked when this
    /// struct is `drop`ped.
    _lock: FileLock,
}
...
impl Layout {
    ...
    pub fn new(
        ...
        // compile.
        let lock = dest.open_rw(".crabgo-lock", ws.config(), "build directory")?;
    ...
    pub fn prepare_tmp(&self) -> CrabgoResult<&Path> {
