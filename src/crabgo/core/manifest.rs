...
/// Contains all the information about a package, as loaded from a `Crabgo.toml`.
///
...
/// When parsing `Crabgo.toml`, some warnings should silenced
...
/// validated by crabgo itself, but rather it is up to the registry when uploaded
/// to validate these fields. Crabgo will itself accept any valid TOML
...
struct TargetInner {
    kind: TargetKind,
    name: String,
    // Note that `bin_name` is used for the crabgo-feature `different_binary_name`
...
struct SerializedTarget<'a> {
    ...
    /// Whether docs should be built for the target via `crabgo doc`
    /// See <https://doc.rust-lang.org/crabgo/commands/crabgo-doc.html#target-selection>
    doc: bool,
    doctest: bool,
    /// Whether tests should be run for the target (`test` field in `Crabgo.toml`)
    test: bool,
}
...
impl Manifest {
    ...
    // It is used by crabgo-c, please do not remove it
    ...
    pub fn feature_gate(&self) -> CrabgoResult<()> {
    ...
    // Just a helper function to test out `-Z` flags on Crabgo
