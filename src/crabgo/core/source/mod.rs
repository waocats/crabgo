...
pub trait Source {
    ...
    fn download_now(self: Box<Self>, package: PackageId, config: &Config) -> CrabgoResult<Package>
    where
    ...
    fn finish_download(&mut self, package: PackageId, contents: Vec<u8>) -> CrabgoResult<Package>;
    ...
    fn verify(&self, _pkg: PackageId) -> CrabgoResult<()> {
        Ok(())
    }
...
impl<'a, T: Source + ?Sized + 'a> Source for Box<T> {
    ...
    fn verify(&self, pkg: PackageId) -> CrabgoResult<()> {
        (**self).verify(pkg)
    }
