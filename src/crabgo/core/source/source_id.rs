...
use crate::util::{config, CanonicalUrl, CrabgoResult, Config, IntoUrl};
use log::trace;
use serde::de;
use serde::ser;
...
impl SourceId {
    ...
    fn new(kind: SourceKind, url: Url, name: Option<&str>) -> CrabgoResult<SourceId> {
        if kind == SourceKind::SparseRegistry {
            ...
            // file that defines the registry, or whenever Crabgo displays it to the user.
    ...
    /// ```
    pub fn from_url(string: &str) -> CrabgoResult<SourceId> {
    ...
    pub fn for_path(path: &Path) -> CrabgoResult<SourceId> {
        let url = path.into_url()?;
        SourceId::new(SourceKind::Path, url, None)
    }

    /// Creates a `SourceId` from a Git reference.
    pub fn for_git(url: &Url, reference: GitReference) -> CrabgoResult<SourceId> {
    ...
    /// generates better messages for crabgo.
    pub fn for_registry(url: &Url) -> CrabgoResult<SourceId> {
    ...
    pub fn for_alt_registry(url: &Url, name: &str) -> CrabgoResult<SourceId> {
        let kind = Self::remote_source_kind(url);
        SourceId::new(kind, url.to_owned(), Some(name))
    }

    /// Creates a SourceId from a local registry path.
    pub fn for_local_registry(path: &Path) -> CrabgoResult<SourceId> {
        let url = path.into_url()?;
        SourceId::new(SourceKind::LocalRegistry, url, None)
    }

    /// Creates a `SourceId` from a directory path.
    pub fn for_directory(path: &Path) -> CrabgoResult<SourceId> {
        let url = path.into_url()?;
    ...
    ///
    /// This is the main crabgo registry by default, but it can be overridden in
    /// a `.crabgo/config.toml`.
    pub fn crates_io(config: &Config) -> CrabgoResult<SourceId> {
    ...
    pub fn crates_io_maybe_sparse_http(config: &Config) -> CrabgoResult<SourceId> {
    ...
    pub fn crates_io_is_sparse(config: &Config) -> CrabgoResult<bool> {
    ...
    pub fn alt_registry(config: &Config, key: &str) -> CrabgoResult<SourceId> {
    ...
    /// WARNING: alt registries that come from Crabgo.lock, or --index will
    /// not have a name.
    ...
    ) -> CrabgoResult<Box<dyn super::Source + 'a>> {
...
// The hash of SourceId is used in the name of some Crabgo folders, so shouldn't
...
// `SourceKind` makes its way into `~/.crabgo/registry/index/github.com-XXXX`
...
mod tests {
    ...
    // versions of Crabgo because changing it means that users will need to
    // redownload the index and all crates they use when using a new Crabgo version.
    //
