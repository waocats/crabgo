...
use crate::util::{closest_msg, config, CrabgoResult, Config};
...
impl Profiles {
    pub fn new(ws: &Workspace<'_>, requested_profile: InternedString) -> CrabgoResult<Profiles> {
        ...
        // This simplifies the API (no need for CrabgoResult), and enforces
    ...
    pub fn get_profile(
        ...
        //
        // (see also https://github.com/rust-lang/crabgo/issues/3972)
        if !is_local {
    ...
    fn get_profile_maker(&self, name: InternedString) -> CrabgoResult<&ProfileMaker> {
        self.by_name
            .get(&name)
...
///
/// - Profiles in `.crabgo/config` files (using same order as below).
...
struct ProfileMaker {
    ...
    default: Profile,
    /// The TOML profile defined in `Crabgo.toml` or config.
    ///
...
pub struct UnitFor {
    ...
    host_features: bool,
    /// How Crabgo processes the `panic` setting or profiles.
...
///
/// [`-Z panic-abort-tests`]: https://doc.rust-lang.org/nightly/crabgo/reference/unstable.html#panic-abort-tests
#[derive(Copy, Clone, Debug, Eq, PartialEq, Hash, Ord, PartialOrd)]
enum PanicSetting {
...
fn merge_config_profiles(
    ws: &Workspace<'_>,
    requested_profile: InternedString,
) -> CrabgoResult<BTreeMap<InternedString, TomlProfile>> {
    ...
    // Add the built-in profiles. This is important for things like `crabgo
...
fn get_config_profile(ws: &Workspace<'_>, name: &str) -> CrabgoResult<Option<TomlProfile>> {
...
fn validate_packages_unique(
    ...
    name: &str,
    toml: &Option<TomlProfile>,
) -> CrabgoResult<HashSet<PackageIdSpec>> {
    let toml = match toml {
        Some(ref toml) => toml,
