...
pub trait Registry {
    ...
    fn query_vec(&mut self, dep: &Dependency, kind: QueryKind) -> Poll<CrabgoResult<Vec<Summary>>> {
        let mut ret = Vec::new();
...
/// `Package` from.
///
/// The resolution phase of Crabgo uses this to drive knowledge about new
...
impl<'cfg> PackageRegistry<'cfg> {
    pub fn new(config: &'cfg Config) -> CrabgoResult<PackageRegistry<'cfg>> {
    ...
    pub fn get(self, package_ids: &[PackageId]) -> CrabgoResult<PackageSet<'cfg>> {
        trace!("getting packages; sources={}", self.sources.len());
        PackageSet::new(package_ids, self.sources, self.config)
    }

    fn ensure_loaded(&mut self, namespace: SourceId, kind: Kind) -> CrabgoResult<()> {
    ...
    pub fn add_sources(&mut self, ids: impl IntoIterator<Item = SourceId>) -> CrabgoResult<()> {
        for id in ids {
    ...
    /// a specific version found in Crabgo.lock. This will be `None` if
    /// `Crabgo.lock` doesn't exist, or the patch did not match any existing
    /// entries in `Crabgo.lock`.
    ///
    ...
    ) -> CrabgoResult<Vec<(Dependency, PackageId)>> {
    ...
    fn load(&mut self, source_id: SourceId, kind: Kind) -> CrabgoResult<()> {
        ...
        // `Some("locked")` as other `Some` values indicate a `crabgo update
        // --precise` request
    ...
    fn query_overrides(&mut self, dep: &Dependency) -> Poll<CrabgoResult<Option<Summary>>> {
...
) -> Poll<CrabgoResult<(Summary, Option<PackageId>)>> {
    if summaries.len() == 1 {
