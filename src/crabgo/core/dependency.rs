...
/// Information about a dependency requested by a Crabgo manifest.
/// Cheap to copy.
...
impl Dependency {
    ...
    /// This is the name of this `Dependency` as listed in `Crabgo.toml`.
    ///
    ...
    ///
    /// If the `package` key is used in `Crabgo.toml` then this returns the same
...
impl ArtifactTarget {
    pub fn parse(target: &str) -> CrabgoResult<ArtifactTarget> {
        Ok(match target {
...
impl ArtifactKind {
    ...
    fn parse(kind: &str) -> CrabgoResult<Self> {
        Ok(match kind {
    ...
    fn validate(kinds: Vec<ArtifactKind>) -> CrabgoResult<Vec<ArtifactKind>> {
