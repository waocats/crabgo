//! Support for nightly features in Crabgo itself.
//!
//! This file is the version of `feature_gate.rs` in upstream Rust for Crabgo
//! itself and is intended to be the avenue for which new features in Crabgo are
...
//! Crabgo, and the good news is that it shouldn't be too hard! First determine
//! how the feature should be gated:
//!
//! * New syntax in Crabgo.toml should use `crabgo-features`.
...
//!   parts of Crabgo should use a new `-Z` flag.
//!
...
//!
//! ## New Crabgo.toml syntax
//!
//! The steps for adding new Crabgo.toml syntax are:
//!
//! 1. Add the crabgo-features unstable gate. Search below for "look here" to
//!    find the [`features!`] macro invocation and add your feature to the list.
//!
//! 2. Update the Crabgo.toml parsing code to handle your new feature.
//!
...
//!    in `crabgo-features` or this is not the nightly channel.
//!
//! ## `-Z unstable-options`
//!
...
//! `crabgo-features` or `-Z unstable-options`.
//!
...
//!
//! ### `-Z` vs `crabgo-features`
//!
//! In some cases there might be some changes that `crabgo-features` is unable
//! to sufficiently encompass. An example would be a syntax change in
//! `Crabgo.toml` that also impacts the index or resolver. The resolver doesn't
//! know about `crabgo-features`, so it needs a `-Z` flag to enable the
//! experimental functionality.
//!
//! In those cases, you usually should introduce both a `-Z` flag (to enable
//! the changes outside of the manifest) and a `crabgo-features` entry (to
//! enable the new syntax in `Crabgo.toml`). The `crabgo-features` entry ensures
...
//! Crabgo should behave roughly similar to how it behaved *before* the
//! unstable feature was introduced. If Crabgo would normally have ignored or
//! warned about the introduction of something, then it probably should
//! continue to do so.
//!
//! For example, Crabgo generally ignores (or warns) about `config.toml`
//! entries it doesn't know about. This allows a limited degree of
//! forwards-compatibility with future versions of Crabgo that add new entries.
//!
...
//! that are not critical to Crabgo's operation in order to reduce the
...
//!   1. `crabgo-features`: Change the feature to `stable` in the [`features!`]
...
//!      necessary.
//! 2. Remove `masquerade_as_nightly_crabgo` from any tests, and remove
//!    `crabgo-features` from `Crabgo.toml` test files if any. You can
...
/// [this example]: https://github.com/rust-lang/crabgo/blob/3ebb5f15a940810f250b68821149387af583a79e/src/doc/src/reference/unstable.md?plain=1#L1238-L1264
    ...
    fn from_str(s: &str) -> Result<Self, Error> {
        match s {
            ...
            s if s.parse().map_or(false, |y: u16| y > 2021 && y < 2050) => bail!(
                "this version of Crabgo is older than the `{}` edition, \
...
// A listing of all features in Crabgo.
//
// "look here"
//
// This is the macro that lists all stable and unstable features in Crabgo.
// You'll want to add to this macro whenever you add a feature to Crabgo, also
// following the directions above.
//
// Note that all feature names here are valid Rust identifiers, but the `_`
// character is translated to `-` when specified in the `crabgo-features`
// manifest entry in `Crabgo.toml`.
features! {
    ...
    (stable, rename_dependency, "1.31", "reference/specifying-dependencies.html#renaming-dependencies-in-crabgotoml"),
...
impl Features {
    ...
    fn add(
        ...
        let (slot, feature) = match self.status(feature_name) {
            Some(p) => p,
            None => bail!("unknown crabgo feature `{}`", feature_name),
        };

        if *slot {
            bail!(
                "the crabgo feature `{}` has already been activated",
                feature_name
            );
        }
        ...
        let see_docs = || {
            ...
            format!(
                "See https://doc.rust-lang.org/{}crabgo/{} for more information \
        ...
        match feature.stability {
            Status::Stable => {
                ...
                if is_local {
                    let warning = format!(
                        "the crabgo feature `{}` has been stabilized in the {} \
            ...
            Status::Unstable if !nightly_features_allowed => bail!(
                "the crabgo feature `{}` requires a nightly version of \
                 Crabgo, but this is the `{}` channel\n\
                 {}\n{}",
                feature_name,
                channel(),
                SEE_CHANNELS,
            ...
            Status::Removed => {
                let mut msg = format!(
                    "the crabgo feature `{}` has been removed in the {} release\n\n",
                ...
                if self.is_local {
                    ...
                    drop(writeln!(
                        msg,
                        "This package cannot be used with this version of Crabgo, \
    ...
    pub fn require(&self, feature: &Feature) -> CrabgoResult<()> {
        ...
        let mut msg = format!(
            "feature `{}` is required\n\
             \n\
             The package requires the Crabgo feature called `{}`, but \
             that feature is not stabilized in this version of Crabgo ({}).\n\
            ",
            feature_name,
            feature_name,
            crate::version(),
        ...
        if self.nightly_features_allowed {
            if self.is_local {
                drop(writeln!(
                    msg,
                    "Consider adding `crabgo-features = [\"{}\"]` \
                     to the top of Crabgo.toml (above the [package] table) \
                     to tell Crabgo you are opting in to use this unstable feature.",
                    feature_name
                ));
            } else {
                drop(writeln!(
            ...
            drop(writeln!(
                msg,
                "Consider trying a newer version of Crabgo \
                 (this may require the nightly release)."
            ));
        }
        drop(writeln!(
            msg,
            "See https://doc.rust-lang.org/nightly/crabgo/{} for more information \
...
macro_rules! unstable_cli_options {
    (
        ...
        /// A parsed representation of all unstable flags that Crabgo accepts.
        ///
        /// Crabgo, like `rustc`, accepts a suite of `-Z` flags which are intended for
        /// gating unstable functionality to Crabgo. These flags are only available on
        /// the nightly channel of Crabgo.
...
unstable_cli_options!(
    ...
    bindeps: bool = ("Allow Crabgo packages to depend on bin, cdylib, and staticlib crates, and use the artifacts built by those crates"),
    #[serde(deserialize_with = "deserialize_build_std")]
    build_std: Option<Vec<String>>  = ("Enable Crabgo to compile the standard library itself as part of a crate graph compilation"),
    build_std_features: Option<Vec<String>>  = ("Configure features enabled for the standard library itself when building the standard library"),
    codegen_backend: bool = ("Enable the `codegen-backend` option in profiles in .crabgo/config.toml file"),
    ...
    mtime_on_use: bool = ("Configure Crabgo to update the mtime of used files"),
    no_index_update: bool = ("Do not update the registry index even if the cache is outdated"),
    panic_abort_tests: bool = ("Enable support to run tests with -Cpanic=abort"),
    profile_rustflags: bool = ("Enable the `rustflags` option in profiles in .crabgo/config.toml file"),
    host_config: bool = ("Enable the [host] section in the .crabgo/config.toml file"),
    ...
    target_applies_to_host: bool = ("Enable the `target-applies-to-host` key in the .crabgo/config.toml file"),
    rustdoc_map: bool = ("Allow passing external documentation mappings to rustdoc"),
    separate_nightlies: bool = (HIDDEN),
    publish_timeout: bool = ("Enable the `publish.timeout` key in .crabgo/config.toml file"),
    unstable_options: bool = ("Allow the usage of unstable options"),
    skip_rustdoc_fingerprint: bool = (HIDDEN),
    rustdoc_scrape_examples: bool = ("Allows Rustdoc to scrape code examples from reverse-dependencies"),
    msrv_policy: bool = ("Enable rust-version aware policy within crabgo"),
);

const STABILIZED_COMPILE_PROGRESS: &str = "The progress bar is now always \
    enabled when used on an interactive console.\n\
    See https://doc.rust-lang.org/crabgo/reference/config.html#termprogresswhen \
...
const STABILIZED_INSTALL_UPGRADE: &str = "Packages are now always upgraded if \
    ...
    See https://doc.rust-lang.org/crabgo/commands/crabgo-install.html for more \
    information on how upgrading works.";

const STABILIZED_CONFIG_PROFILE: &str = "See \
    https://doc.rust-lang.org/crabgo/reference/config.html#profile for more \
...
const STABILIZED_PACKAGE_FEATURES: &str = "Enhanced feature flag behavior is now \
    ...
    Crabgo.toml.\n\
    See https://doc.rust-lang.org/nightly/crabgo/reference/features.html#resolver-version-2-command-line-flags \
    for more information.";

const STABILIZED_FEATURES: &str = "The new feature resolver is now available \
    by specifying `resolver = \"2\"` in Crabgo.toml.\n\
    See https://doc.rust-lang.org/nightly/crabgo/reference/features.html#feature-resolver-version-2 \
    for more information.";
...
const STABILIZED_NAMED_PROFILES: &str = "The named-profiles feature is now always enabled.\n\
    See https://doc.rust-lang.org/nightly/crabgo/reference/profiles.html#custom-profiles \
    for more information";
...
) -> CrabgoResult<Option<GitoxideFeatures>> {
...
fn parse_check_cfg(
    it: impl Iterator<Item = impl AsRef<str>>,
) -> CrabgoResult<Option<(bool, bool, bool, bool)>> {
...
impl CliUnstable {
    pub fn parse(
        ...
        flags: &[String],
        nightly_features_allowed: bool,
    ) -> CrabgoResult<Vec<String>> {
        if !flags.is_empty() && !nightly_features_allowed {
            bail!(
                "the `-Z` flag is only accepted on the nightly channel of Crabgo, \
        ...
        if self.gitoxide.is_none() && crabgo_use_gitoxide_instead_of_git2() {
            self.gitoxide = GitoxideFeatures::safe().into();
        }
        Ok(warnings)
    }

    fn add(&mut self, flag: &str, warnings: &mut Vec<String>) -> CrabgoResult<()> {
        let mut parts = flag.splitn(2, '=');
        let k = parts.next().unwrap();
        let v = parts.next();

        fn parse_bool(key: &str, value: Option<&str>) -> CrabgoResult<bool> {
            match value {
        ...
        fn parse_empty(key: &str, value: Option<&str>) -> CrabgoResult<bool> {
            if let Some(v) = value {
        ...
        match k {
            ...
            // can also be set in .crabgo/config or with and ENV
    ...
    pub fn fail_if_stable_opt(&self, flag: &str, issue: u32) -> CrabgoResult<()> {
        if !self.unstable_options {
            let see = format!(
                "See https://github.com/rust-lang/crabgo/issues/{issue} for more \
            ...
            if channel == "nightly" || channel == "dev" {
                ...
                bail!(
                    "the `{flag}` flag is unstable, and only available on the nightly channel \
                     of Crabgo, but this is the `{channel}` channel\n\
                     {SEE_CHANNELS}\n\
                     {see}"
                );
            }
    ...
    pub fn fail_if_stable_command(
        ...
        let see = format!(
            "See https://github.com/rust-lang/crabgo/issues/{} for more \
            information about the `crabgo {}` command.",
            issue, command
        );
        if config.nightly_features_allowed {
            bail!(
                "the `crabgo {}` command is unstable, pass `-Z unstable-options` to enable it\n\
                 {}",
                command,
                see
            );
            ...
            bail!(
                "the `crabgo {}` command is unstable, and only available on the \
                 nightly channel of Crabgo, but this is the `{}` channel\n\
                 {}\n\
                 {}",
                command,
                channel(),
