...
use crabgo_util::paths::normalize_path;
use pathdiff::diff_paths;

/// The core abstraction in Crabgo for working with a workspace of crates.
///
...
pub struct Workspace<'cfg> {
    ...
    // This path is a path to where the current crabgo subcommand was invoked
    // from. That is the `--manifest-path` argument to Crabgo, and
    ...
    // missing, `package.workspace` is missing, and no `Crabgo.toml` above
    ...
    // `crabgo install` or `crabgo package` commands.
    is_ephemeral: bool,
    ...
    // needed by the current configuration (such as in crabgo install). In some
    ...
    // If `true`, then the resolver will ignore any existing `Crabgo.lock`
    // file. This is set for `crabgo install` without `--locked`.
    ignore_lock: bool,
...
impl<'cfg> Workspace<'cfg> {
    ...
    pub fn new(manifest_path: &Path, config: &'cfg Config) -> CrabgoResult<Workspace<'cfg>> {
    ...
    pub fn new_virtual(
        ...
        config: &'cfg Config,
    ) -> CrabgoResult<Workspace<'cfg>> {
        let mut ws = Workspace::new_default(current_manifest, config);
        ws.root_manifest = Some(root_path.join("Crabgo.toml"));
        ws.target_dir = config.target_dir()?;
        ws.packages
            .packages
    ...
    ///
    /// This is currently only used in niche situations like `crabgo install` or
    /// `crabgo package`.
    pub fn ephemeral(
        package: Package,
        config: &'cfg Config,
    ...
    /// actually a "virtual Crabgo.toml", in which case an error is returned
    /// indicating that something else should be passed.
    pub fn current(&self) -> CrabgoResult<&Package> {
    ...
    pub fn current_mut(&mut self) -> CrabgoResult<&mut Package> {
    ...
    /// `Crabgo.toml` which is the root of this workspace.
    pub fn root(&self) -> &Path {
        self.root_manifest().parent().unwrap()
    }

    /// Returns the path of the `Crabgo.toml` which is the root of this
    /// workspace.
    ...
    fn config_patch(&self) -> CrabgoResult<HashMap<Url, Vec<Dependency>>> {
        ...
        for message in warnings {
            self.config
                .shell()
                .warn(format!("[patch] in crabgo config: {}", message))?
        }
    ...
    ///
    /// This may be from a virtual crate or an actual crate.
    pub fn root_patch(&self) -> CrabgoResult<HashMap<Url, Vec<Dependency>>> {
    ...
    pub fn load_workspace_config(&mut self) -> CrabgoResult<Option<WorkspaceRootConfig>> {
    ...
    /// at `manifest_path`.
    ///
    /// This will parse the `Crabgo.toml` at `manifest_path` and then interpret
    ...
    fn find_root(&mut self, manifest_path: &Path) -> CrabgoResult<Option<PathBuf>> {
    ...
    /// the workspace.
    fn find_members(&mut self) -> CrabgoResult<()> {
        ...
        for path in &members_paths {
            self.find_path_deps(&path.join("Crabgo.toml"), &root_manifest_path, false)
                .with_context(|| {
                    format!(
        ...
        if let Some(default) = default_members_paths {
            for path in default {
                ...
                let manifest_path = normalized_path.join("Crabgo.toml");
                if !self.members.contains(&manifest_path) {
                    ...
                    // include `/Crabgo.toml`, and because excluded paths may not
                    // be crates.
    ...
    /// Returns the unstable nightly-only features enabled via `crabgo-features` in the manifest.
    ...
    fn validate(&mut self) -> CrabgoResult<()> {
    ...
    fn validate_unique_names(&self) -> CrabgoResult<()> {
    ...
    fn validate_workspace_roots(&self) -> CrabgoResult<()> {
    ...
    fn validate_members(&mut self) -> CrabgoResult<()> {
    ...
    fn error_if_manifest_not_in_members(&mut self) -> CrabgoResult<()> {
    ...
    fn validate_manifest(&mut self) -> CrabgoResult<()> {
        if let Some(ref root_manifest) = self.root_manifest {
                ...
                .filter(|p| p.manifest_path() != root_manifest)
            {
                ...
                let emit_warning = |what| -> CrabgoResult<()> {
                    let msg = format!(
    ...
    pub fn load(&self, manifest_path: &Path) -> CrabgoResult<Package> {
    ...
    pub fn preload(&self, registry: &mut PackageRegistry<'cfg>) {
        ...
        // `crabgo install`. Things like git repositories will actually have a
        // `PathSource` with multiple entries in it, so the logic below is
        // mostly just an optimization for normal `crabgo build` in workspaces
        // during development.
    ...
    /// `true`. This is used for generating `Crabgo.lock`, which must include
    /// all members with all features enabled.
    pub fn members_with_features(
        &self,
        specs: &[PackageIdSpec],
        cli_features: &CliFeatures,
    ) -> CrabgoResult<Vec<(&Package, CliFeatures)>> {
        assert!(
    ...
    fn members_with_features_new(
        ...
        if members.is_empty() {
            // `crabgo build -p foo`, where `foo` is not a member.
    ...
    pub fn unit_needs_doc_scrape(&self, unit: &Unit) -> bool {
        ...
        // Crabgo to panic, see issue #10545.
...
impl<'cfg> Packages<'cfg> {
    ...
    fn load(&mut self, manifest_path: &Path) -> CrabgoResult<&MaybePackage> {
...
impl WorkspaceRootConfig {
    ...
    fn members_paths(&self, globs: &[String]) -> CrabgoResult<Vec<PathBuf>> {
    ...
    fn expand_member_path(path: &Path) -> CrabgoResult<Vec<PathBuf>> {
...
pub fn find_workspace_root(manifest_path: &Path, config: &Config) -> CrabgoResult<Option<PathBuf>> {
...
fn find_workspace_root_with_loader(
    manifest_path: &Path,
    config: &Config,
    mut loader: impl FnMut(&Path) -> CrabgoResult<Option<PathBuf>>,
        ...
        for current in manifest_path.ancestors().skip(1) {
            if let Some(ws_config) = roots.get(current) {
                if !ws_config.is_excluded(manifest_path) {
                    // Add `Crabgo.toml` since ws_root is the root and not the file
                    return Ok(Some(current.join("Crabgo.toml")));
                }
            }
        }
    }
...
fn find_root_iter<'a>(
    ...
    LookBehind::new(paths::ancestors(manifest_path, None).skip(2))
        ...
        .map(|path| path.curr.join("Crabgo.toml"))
