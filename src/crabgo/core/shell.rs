...
impl Shell {
    ...
    /// output.
    pub fn new() -> Shell {
        let auto_clr = ColorChoice::CrabgoAuto;
        Shell {
            output: ShellOut::Stream {
                ...
                color_choice: ColorChoice::CrabgoAuto,
    ...
    pub fn status<T, U>(&mut self, status: T, message: U) -> CrabgoResult<()>
    where
        T: fmt::Display,
        U: fmt::Display,
    {
    ...
    pub fn status_header<T>(&mut self, status: T) -> CrabgoResult<()>
    where
        T: fmt::Display,
    {
    ...
    pub fn verbose<F>(&mut self, mut callback: F) -> CrabgoResult<()>
    where
        F: FnMut(&mut Shell) -> CrabgoResult<()>,
    {
        match self.verbosity {
    ...
    pub fn concise<F>(&mut self, mut callback: F) -> CrabgoResult<()>
    where
        F: FnMut(&mut Shell) -> CrabgoResult<()>,
    {
        match self.verbosity {
    ...
    pub fn error<T: fmt::Display>(&mut self, message: T) -> CrabgoResult<()> {
        if self.needs_clear {
            self.err_erase_line();
        }
        self.output
    ...
    pub fn warn<T: fmt::Display>(&mut self, message: T) -> CrabgoResult<()> {
        match self.verbosity {
            Verbosity::Quiet => Ok(()),
    ...
    pub fn note<T: fmt::Display>(&mut self, message: T) -> CrabgoResult<()> {
    ...
    pub fn set_color_choice(&mut self, color: Option<&str>) -> CrabgoResult<()> {
        ...
        } = self.output
        {
            let cfg = match color {
                ...
                Some("auto") | None => ColorChoice::CrabgoAuto,
    ...
    pub fn print_ansi_stderr(&mut self, message: &[u8]) -> CrabgoResult<()> {
        if self.needs_clear {
            self.err_erase_line();
        }
        #[cfg(windows)]
    ...
    pub fn print_ansi_stdout(&mut self, message: &[u8]) -> CrabgoResult<()> {
        if self.needs_clear {
            self.err_erase_line();
        }
        #[cfg(windows)]
    ...
    pub fn print_json<T: serde::ser::Serialize>(&mut self, obj: &T) -> CrabgoResult<()> {
...
impl ShellOut {
    ...
    /// Write a styled fragment
    fn write_stdout(&mut self, fragment: impl fmt::Display, color: &ColorSpec) -> CrabgoResult<()> {
        match *self {
    ...
    /// Write a styled fragment
    fn write_stderr(&mut self, fragment: impl fmt::Display, color: &ColorSpec) -> CrabgoResult<()> {
        match *self {
            ShellOut::Stream { ref mut stderr, .. } => {
                stderr.reset()?;
