...
use crate::util::errors::{CrabgoResult, HttpNotSuccessful, DEBUG_HEADERS};
...
struct Downloads<'cfg> {
    ...
    results: HashMap<PathBuf, CrabgoResult<CompletedDownload>>,
...
impl<'cfg> HttpRegistry<'cfg> {
    ...
    fn start_fetch(&mut self) -> CrabgoResult<()> {
        ...
        // We've enabled the `http2` feature of `curl` in Crabgo, so treat
    ...
    fn handle_completed_downloads(&mut self) -> CrabgoResult<()> {
        assert_eq!(
    ...
    fn config_cached(&mut self) -> CrabgoResult<Option<&RegistryConfig>> {
    ...
    fn config(&mut self) -> Poll<CrabgoResult<&RegistryConfig>> {
    ...
    fn add_sleepers(&mut self) -> CrabgoResult<()> {
...
impl<'cfg> RegistryData for HttpRegistry<'cfg> {
    ...
    fn load(
        ...
        if let Some(result) = self.downloads.results.remove(path) {
            ...
            match result.response_code {
                        ...
                        && self.config.cli_unstable().registry_auth =>
                {
                    ...
                    // Look for a `www-authenticate` header with the `Crabgo` scheme.
                    for header in &result.header_map.www_authenticate {
                        for challenge in http_auth::ChallengeParser::new(header) {
                            match challenge {
                                Ok(challenge) if challenge.scheme.eq_ignore_ascii_case("Crabgo") => {
                                ...
                                Ok(challenge) => {
                                    debug!("ignoring non-Crabgo challenge: {}", challenge.scheme)
                                }
        ...
        // know that Crabgo is attempting to use the sparse protocol.
        headers.append("crabgo-protocol: version=1")?;
