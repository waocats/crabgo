...
impl<'cfg> RemoteRegistry<'cfg> {
    ...
    fn repo(&self) -> CrabgoResult<&git2::Repository> {
    ...
    fn head(&self) -> CrabgoResult<git2::Oid> {
    ...
    fn tree(&self) -> CrabgoResult<Ref<'_, git2::Tree<'_>>> {
        {
...
impl<'cfg> RegistryData for RemoteRegistry<'cfg> {
    ...
    // Older versions of Crabgo used the single value of the hash of the HEAD commit as a `index_version`.
    ...
    // However if an old crabgo has written such a file we still know how to read it, as long as we check for that hash value.
    //
    // Crabgo now uses a hash of the file's contents as provided by git.
    fn load(
        &mut self,
        _root: &Path,
        path: &Path,
        ...
        if index_version.is_some() && index_version == git_commit_hash.as_deref() {
            // This file was written by an old version of crabgo, but it is still up-to-date.
    ...
    fn block_until_ready(&mut self) -> CrabgoResult<()> {
        ...
        // is run multiple times, such as during `crabgo publish`.
        if self.is_updated() {
            return Ok(());
        }
        self.mark_updated();
        ...
        // problems with `.crabgo/config` configuration related to HTTP.
        //
