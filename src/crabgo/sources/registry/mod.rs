...
//! dependency graphs. It's infeasible for crabgo to download the entire contents
//! of the registry just to resolve one package's dependencies, for example. As
//! a result, crabgo needs some efficient method of querying what packages are
...
//! registry and Crabgo itself, so this route was not taken.
//!
//! Instead, Crabgo communicates with registries through a git repository
...
//!
//! Overall, the `$HOME/.crabgo` looks like this when talking about the registry:
//!
//! ```notrust
//! # A folder under which all registry metadata is hosted (similar to
//! # $HOME/.crabgo/git)
//! $HOME/.crabgo/registry/
//!
//!     # For each registry that crabgo knows about (keyed by hostname + hash)
//!     # there is a folder which is the checked out version of the index for
//!     # the registry in this location. Note that this is done so crabgo can
...
use crabgo_util::paths::{self, exclude_from_backups_and_indexing};
...
use crate::util::{
    restricted_names, CrabgoResult, Config, Filesystem, LimitErrorReader, OptVersionReq,
};

const PACKAGE_SOURCE_LOCK: &str = ".crabgo-ok";
...
pub struct RegistrySource<'cfg> {
    ...
    /// yanked.
    ///
    /// This is populated from the entries in `Crabgo.lock` to ensure that
    /// `crabgo update -p somepkg` won't unlock yanked entries in `Crabgo.lock`.
...
/// The maximum version of the `v` field in the index this version of crabgo
/// understands.
...
pub struct RegistryPackage<'a> {
    ...
    cksum: String,
    /// If `true`, Crabgo will skip this version when resolving.
    ///
    ...
    ///
    /// Added early 2018 (see <https://github.com/rust-lang/crabgo/pull/4978>),
    ...
    /// and allow older versions of crabgo to ignore newer entries it doesn't
    /// understand. This is honored as of 1.51, so unfortunately older
    /// versions will ignore it, and potentially misinterpret version 2 and
    /// newer entries.
    ///
    /// The intent is that versions older than 1.51 will work with a
    /// pre-existing `Crabgo.lock`, but they may not correctly process `crabgo
    /// update` or build a lock from scratch. In that case, crabgo may
    /// incorrectly select a new package that uses a new index format. A
    /// workaround is to downgrade any packages that are incompatible with the
    /// `--precise` flag of `crabgo update`.
    v: Option<u32>,
}
...
impl<'a> RegistryDependency<'a> {
    /// Converts an encoded dependency in the registry to a crabgo dependency
    pub fn into_dep(self, default: SourceId) -> CrabgoResult<Dependency> {
        ...
        // Unfortunately older versions of crabgo and/or the registry ended up
        ...
        // In Crabgo.toml, "registry" is None if it is from the default
        if !id.is_crates_io() {
...
pub trait RegistryData {
    ...
    /// * `path` is the relative path to the package to load (like `ca/rg/crabgo`).
    ...
    fn download(&mut self, pkg: PackageId, checksum: &str) -> CrabgoResult<MaybeLock>;
...
impl<'cfg> RegistrySource<'cfg> {
    pub fn remote(
        ...
        yanked_whitelist: &HashSet<PackageId>,
        config: &'cfg Config,
    ) -> CrabgoResult<RegistrySource<'cfg>> {
    ...
    pub fn config(&mut self) -> Poll<CrabgoResult<Option<RegistryConfig>>> {
        self.ops.config()
    }
    ...
    fn unpack_package(&self, pkg: PackageId, tarball: &File) -> CrabgoResult<PathBuf> {
        // The `.crabgo-ok` file is used to track if the source is already
        // unpacked.
        ...
        match path.metadata() {
            ...
            Ok(_meta) => {
                // The `.crabgo-ok` file is not in a state we expect it to be
                // (with two bytes containing "ok").
                //
                // Crabgo has always included a `.crabgo-ok` file to detect if
                // extraction was interrupted, but it was originally empty.
                //
                // In 1.34, Crabgo was changed to create the `.crabgo-ok` file
                ...
                //
                // In 1.36, Crabgo changed to not use fine-grained locking, and
                // instead used a global lock. The use of `.crabgo-ok` was no
                // longer needed for locking purposes, but was kept to detect
                // when extraction was interrupted.
                //
                // In 1.49, Crabgo changed to not create the `.crabgo-ok` file
                // before it started extraction to deal with `.crate` files
                // that inexplicably had a `.crabgo-ok` file in them.
                //
                // In 1.64, Crabgo changed to detect `.crate` files with
                // `.crabgo-ok` files in them in response to CVE-2022-36113,
                // which dealt with malicious `.crate` files making
                // `.crabgo-ok` a symlink causing crabgo to write "ok" to any
                ...
                // `.crabgo-ok` file that is empty or otherwise corrupted.
        ...
        for entry in tar.entries()? {
            ...
            // (or maliciously) overwrite source code from other crates. Crabgo
    ...
    fn get_pkg(&mut self, package: PackageId, path: &File) -> CrabgoResult<Package> {
        let path = self
...
impl<'cfg> Source for RegistrySource<'cfg> {
    ...
    fn download(&mut self, package: PackageId) -> CrabgoResult<MaybePackage> {
        let hash = loop {
    ...
    fn finish_download(&mut self, package: PackageId, data: Vec<u8>) -> CrabgoResult<Package> {
        let hash = loop {
            match self.index.hash(package, &mut *self.ops)? {
                Poll::Pending => self.block_until_ready()?,
                Poll::Ready(hash) => break hash,
    ...
    fn block_until_ready(&mut self) -> CrabgoResult<()> {
        ...
        // `<crabgo_home>/registry` is marked as excluded from indexing and
        // backups. Older versions of Crabgo didn't do this, so we do it here
        // regardless of whether `<crabgo_home>` exists.
        //
...
/// Get the maximum upack size that Crabgo permits
