...
//! path" as much as possible. Whenever you type `crabgo build` Crabgo will
//! *always* reparse the registry and learn about dependency information. This
//! is done because Crabgo needs to learn about the upstream crates.io crates
//! that you're using and ensure that the preexisting `Crabgo.lock` still matches
//! the current state of the world.
//!
//! Consequently, Crabgo "null builds" (the index that Crabgo adds to each build
...
//!
//! To solve all these issues a strategy is employed here where Crabgo basically
//! creates an index into the index. The first time a package is queried about
//! (first time being for an entire computer) Crabgo will load the contents
//! (slowly via libgit2) from the registry. It will then (slowly) parse every
//! single line to learn about its versions. Afterwards, however, Crabgo will
...
//! version extracted from each JSON blob. That way Crabgo can quickly and easily
...
//! Crabgo null build performance. It's expected that the improvements earned
...
//! implemented a null build of Crabgo itself would parse 3700 JSON blobs from
//! the registry and load 150 blobs from git. Afterwards it parses 150 JSON
//! blobs and loads 0 files git. Removing 200ms or more from Crabgo's startup
...
use crate::util::{internal, CrabgoResult, Config, Filesystem, OptVersionReq, ToSemver};
use anyhow::bail;
use crabgo_util::{paths, registry::make_dep_path};
use log::{debug, info};
use semver::Version;
...
/// Crates.io treats hyphen and underscores as interchangeable, but the index and old Crabgo do not.
/// Therefore, the index must store uncanonicalized version of the name so old Crabgo's can find it.
...
///
/// 1. Primarily Crabgo will parse the corresponding file for a crate in the
///    upstream crates.io registry. That's just a JSON blob per line which we
///    can parse, extract the version, and then store here.
///
/// 2. Alternatively, if Crabgo has previously run, we'll have a cached index of
///    dependencies for the upstream index. This is a file that Crabgo maintains
...
/// A representation of the cache on disk that Crabgo maintains of summaries.
/// Crabgo will initially parse all summaries in the registry and will then
...
impl<'cfg> RegistryIndex<'cfg> {
    ...
    pub fn hash(&mut self, pkg: PackageId, load: &mut dyn RegistryData) -> Poll<CrabgoResult<&str>> {
    ...
    /// though since this method is called quite a lot on null builds in Crabgo.
    pub fn summaries<'a, 'b>(
        &'a mut self,
        name: &str,
        req: &'b OptVersionReq,
        load: &mut dyn RegistryData,
    ) -> Poll<CrabgoResult<impl Iterator<Item = &'a IndexSummary> + 'b>>
    where
        'a: 'b,
    {
        ...
        // First up actually parse what summaries we have available. If Crabgo
        // has run previously this will parse a Crabgo-specific cache file rather
    ...
    fn load_summaries(
        ...
        name: InternedString,
        load: &mut dyn RegistryData,
    ) -> Poll<CrabgoResult<&mut Summaries>> {
    ...
    pub fn query_inner(
        ...
        if self.config.offline() {
            ...
            // then crabgo will fail to download and an error message
    ...
    fn query_inner_with_online(
        ...
        let summaries = summaries
            ...
            // previously in `Crabgo.lock`
            .filter(|s| !s.yanked || yanked_whitelist.contains(&s.summary.package_id()))
            .map(|s| s.summary.clone());

        // Handle `crabgo update --precise` here. If specified, our own source
...
impl Summaries {
    ...
    /// exists from a previous invocation of Crabgo (aka you're typing `crabgo
    ...
    pub fn parse(
        ...
        load: &mut dyn RegistryData,
        config: &Config,
    ) -> Poll<CrabgoResult<Option<Summaries>>> {
        ...
        match response {
            ...
            LoadResponse::Data {
                ...
                for line in split(&ret.raw_data, b'\n') {
                    ...
                    // allow future crabgo implementations to break the
                    // interpretation of each line here and older crabgo will simply
                    // ignore the new lines.
                    let summary = match IndexSummary::parse(config, line, source_id) {
                        Ok(summary) => summary,
                        Err(e) => {
                            // This should only happen when there is an index
                            // entry from a future version of crabgo that this
                            // version doesn't understand. Hopefully, those future
                            // versions of crabgo correctly set INDEX_V_MAX and
                ...
                if let Some(index_version) = index_version {
                    ...
                    // about to return, write that back out to disk so future Crabgo
    ...
    /// Crabgo.
    pub fn parse_cache(contents: Vec<u8>) -> CrabgoResult<(Summaries, InternedString)> {
...
// +----------------+---+------------+---+
//
// The idea is that this is a very easy file for Crabgo to parse in future
...
// versions of Crabgo share the same cache they don't get too confused. The git
...
//   changes, different versions of crabgo won't get confused reading each
//   other's caches.
...
impl<'a> SummariesCache<'a> {
    fn parse(data: &'a [u8]) -> CrabgoResult<SummariesCache<'a>> {
        ...
        if *first_byte != CURRENT_CACHE_VERSION {
            bail!("looks like a different Crabgo's cache, bailing out");
        }
...
impl IndexSummary {
    ...
    fn parse(config: &Config, line: &[u8], source_id: SourceId) -> CrabgoResult<IndexSummary> {
        // ****CAUTION**** Please be extremely careful with returning errors
        // from this function. Entries that error are not included in the
        // index cache, and can cause crabgo to get confused when switching
