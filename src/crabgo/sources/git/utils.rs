...
use crate::sources::git::oxide::crabgo_config_to_gitoxide_overrides;
...
impl GitRemote {
    ...
    pub fn rev_for(&self, path: &Path, reference: &GitReference) -> CrabgoResult<git2::Oid> {
    ...
    pub fn checkout(
        ...
        crabgo_config: &Config,
    ) -> CrabgoResult<(GitDatabase, git2::Oid)> {
    ...
    pub fn db_at(&self, db_path: &Path) -> CrabgoResult<GitDatabase> {
...
impl GitDatabase {
    pub fn copy_to(
        ...
        {
            Some(co) => co,
            None => GitCheckout::clone_into(dest, self, rev, crabgo_config)?,
        };
        checkout.update_submodules(crabgo_config, parent_remote_url)?;
        Ok(checkout)
    }

    pub fn to_short_id(&self, revision: git2::Oid) -> CrabgoResult<GitShortID> {
    ...
    pub fn resolve(&self, r: &GitReference) -> CrabgoResult<git2::Oid> {
        r.resolve(&self.repo)
    }
}

impl GitReference {
    pub fn resolve(&self, repo: &git2::Repository) -> CrabgoResult<git2::Oid> {
        let id = match self {
            // Note that we resolve the named tag here in sync with where it's
            // fetched into via `fetch` below.
            GitReference::Tag(s) => (|| -> CrabgoResult<git2::Oid> {
...
impl<'a> GitCheckout<'a> {
    ...
    fn is_fresh(&self) -> bool {
        match self.repo.revparse_single("HEAD") {
            Ok(ref head) if head.id() == self.revision => {
                ...
                self.location.join(".crabgo-ok").exists()
            }
            _ => false,
        }
    }

    fn reset(&self, config: &Config) -> CrabgoResult<()> {
        // If we're interrupted while performing this reset (e.g., we die because
        // of a signal) Crabgo needs to be sure to try to check out this repo
        // again on the next go-round.
        //
        // To enable this we have a dummy file in our checkout, .crabgo-ok, which
        // if present means that the repo has been successfully reset and is
        // ready to go. Hence if we start to do a reset, we make sure this file
        // *doesn't* exist, and then once we're done we create the file.
        let ok_file = self.location.join(".crabgo-ok");
    ...
    fn update_submodules(&self, crabgo_config: &Config, parent_remote_url: &Url) -> CrabgoResult<()> {
        return update_submodules(&self.repo, crabgo_config, parent_remote_url);
        ...
        fn update_submodules(
            ...
            for mut child in repo.submodules()? {
                update_submodule(repo, &mut child, crabgo_config, parent_remote_url).with_context(
                    || {
                        format!(
        ...
        fn update_submodule(
            ...
            let mut repo = match head_and_repo {
                Ok((head, repo)) => {
                    if child.head_id() == head {
                        return update_submodules(&repo, crabgo_config, parent_remote_url);
                    }
                    repo
                }
                Err(..) => {
            ...
            reset(&repo, &obj, crabgo_config)?;
            update_submodules(&repo, crabgo_config, parent_remote_url)
        }
    }
}
...
where
    F: FnMut(&mut git2::Credentials<'_>) -> CrabgoResult<T>,
{
    ...
    let mut res = f(&mut |url, username, allowed| {
        ...
        // process or from a raw in-memory SSH key. Crabgo only supports using
        // ssh-agent currently.
        //
        ...
        // is where Crabgo would have an interactive prompt if we supported it,
    ...
    if ssh_username_requested {
        ...
        if let Ok(s) = crabgo_config
            .get_env("USER")
            .or_else(|_| crabgo_config.get_env("USERNAME"))
        {
            attempts.push(s);
        }
    ...
    if any_attempts {
        ...
        msg.push_str("https://doc.rust-lang.org/crabgo/reference/config.html#netgit-fetch-with-cli");
        err = err.context(msg);
...
fn reset(repo: &git2::Repository, obj: &git2::Object<'_>, config: &Config) -> CrabgoResult<()> {
...
pub fn with_fetch_options(
    ...
    url: &str,
    config: &Config,
    cb: &mut dyn FnMut(git2::FetchOptions<'_>) -> CrabgoResult<()>,
) -> CrabgoResult<()> {
...
pub fn fetch(
    ...
    {
        let git2_repo = repo;
        let config_overrides = crabgo_config_to_gitoxide_overrides(config)?;
        ...
        let res = oxide::with_retry_and_progress(
            ...
            &|repo_path,
              ...
              url_for_authentication: &mut dyn FnMut(&gix::bstr::BStr)| {
                ...
                // reasons (aka network related reasons). We want Crabgo to automatically
                // recover from corrupt repositories, but we don't want Crabgo to stomp
...
/// files atomically.
/// Crabgo has its own lock files and doesn't need that mechanism for race protection, so a stray lock means
...
fn fetch_with_cli(
    ...
    cmd.arg("--force") // handle force pushes
        ...
        .arg(url)
        .args(refspecs)
        // If crabgo is run by git (for example, the `exec` command in `git
...
/// Crabgo has a bunch of long-lived git repositories in its global cache and
...
fn maybe_gc_repo(repo: &mut git2::Repository, config: &Config) -> CrabgoResult<()> {
...
fn reinitialize(repo: &mut git2::Repository) -> CrabgoResult<()> {
...
fn init(path: &Path, bare: bool) -> CrabgoResult<git2::Repository> {
