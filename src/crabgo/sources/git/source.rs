...
use anyhow::Context;
use crabgo_util::paths::exclude_from_backups_and_indexing;
use log::trace;
...
impl<'cfg> GitSource<'cfg> {
    pub fn new(source_id: SourceId, config: &'cfg Config) -> CrabgoResult<GitSource<'cfg>> {
    ...
    pub fn read_packages(&mut self) -> CrabgoResult<Vec<Package>> {
...
/// crabgo's aren't necessarily handling shallow clones correctly.
...
impl<'cfg> Source for GitSource<'cfg> {
    ...
    fn block_until_ready(&mut self) -> CrabgoResult<()> {
        ...
        // Before getting a checkout, make sure that `<crabgo_home>/git` is
        // marked as excluded from indexing and backups. Older versions of Crabgo
        // didn't do this, so we do it here regardless of whether `<crabgo_home>`
        // exists.
        //
    ...
    fn fingerprint(&self, _pkg: &Package) -> CrabgoResult<String> {
...
mod test {
    ...
    pub fn test_url_to_path_ident_with_path() {
        let ident = ident(&src("https://github.com/carlhuda/crabgo"));
        assert!(ident.starts_with("crabgo-"));
    }
