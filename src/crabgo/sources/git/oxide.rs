...
/// Convert `git` related crabgo configuration into the respective `git` configuration which can be
/// used when opening new repositories.
pub fn crabgo_config_to_gitoxide_overrides(config: &Config) -> CrabgoResult<Vec<BString>> {
    ...
    values.push(if let Some(user_agent) = &http.user_agent {
        ...
        Http::USER_AGENT.validated_assignment_fmt(&format!("crabgo {}", crate::version()))
    }?);
    if let Some(ssl_version) = &http.ssl_version {
        ...
        // This text is copied from https://github.com/rust-lang/crabgo/blob/39c13e67a5962466cc7253d41bc1099bbcb224c3/src/crabgo/ops/registry.rs#L658-L674 .
...
pub fn reinitialize(git_dir: &Path) -> CrabgoResult<()> {
    fn init(path: &Path, bare: bool) -> CrabgoResult<()> {
