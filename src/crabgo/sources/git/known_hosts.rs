...
/// These are host keys that are hard-coded in crabgo to provide convenience.
///
...
enum KnownHostError {
    ...
    /// @cert-authority marker (which Crabgo doesn't yet support).
...
enum KnownHostLocation {
    ...
    /// Loaded from crabgo's config system.
    Config { definition: Definition },
    /// Part of the hard-coded bundled keys in Crabgo.
    Bundled,
}
...
impl Display for KnownHostLocation {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let loc = match self {
            ...
            KnownHostLocation::Bundled => format!("bundled with crabgo"),
        };
        f.write_str(&loc)
    }
}
    ...
    let err_msg = match check_ssh_known_hosts(
        ...
        Err(KnownHostError::HostKeyHasChanged {
            ...
            let old_key_resolution = match old_known_host.location {
                ...
                KnownHostLocation::Config { definition } => {
                    format!(
                        "removing the old {key_type_name} key for `{hostname}` \
                        loaded from Crabgo's config at {definition}, \
                        and adding the new key to {known_hosts_location}"
                    )
                }
                KnownHostLocation::Bundled => {
                    format!(
                        ...
                        The current host key is bundled as part of Crabgo."
                    )
                }
            };
        ...
        Err(KnownHostError::HostHasOnlyCertAuthority { hostname, location }) => {
            format!("error: Found a `@cert-authority` marker for `{hostname}`\n\
                \n\
                Crabgo doesn't support certificate authorities for host key verification. It is\n\
                recommended that the command line Git client is used instead. This can be achieved\n\
                by setting `net.git-fetch-with-cli` to `true` in the Crabgo config.\n\
                \n
...
fn check_ssh_known_hosts_loaded(
    ...
    // support SHA256.
    let mut remote_fingerprint = crabgo_util::Sha256::new();
...
fn user_known_host_location_to_add(diagnostic_home_config: &str) -> String {
    ...
    format!(
        "the `net.ssh.known-hosts` array in your Crabgo configuration \
...
fn load_hostfile(path: &Path) -> Result<Vec<KnownHost>, anyhow::Error> {
    let contents = crabgo_util::paths::read(path)?;
