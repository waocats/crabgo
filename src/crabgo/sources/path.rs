...
use crate::ops;
use crate::util::{internal, CrabgoResult, Config};
...
impl<'cfg> PathSource<'cfg> {
    /// Invoked with an absolute path to a directory that contains a `Crabgo.toml`.
    ///
    ...
    pub fn root_package(&mut self) -> CrabgoResult<Package> {
    ...
    pub fn read_packages(&self) -> CrabgoResult<Vec<Package>> {
        if self.updated {
            Ok(self.packages.clone())
        } else if self.recursive {
            ops::read_packages(&self.path, self.source_id, self.config)
        } else {
            let path = self.path.join("Crabgo.toml");
    ...
    pub fn list_files(&self, pkg: &Package) -> CrabgoResult<Vec<PathBuf>> {
    ...
    fn _list_files(&self, pkg: &Package) -> CrabgoResult<Vec<PathBuf>> {
        let root = pkg.root();
        ...
        if no_include_option && git_repo.is_none() {
            // no include option and not git repo discovered (see rust-lang/crabgo#7183).
        ...
        // Attempt Git-prepopulate only if no `include` (see rust-lang/crabgo#4135).
        if no_include_option {
    ...
    /// Returns `Some(git2::Repository)` if found sibling `Crabgo.toml` and `.git`
    /// directory; otherwise, caller should fall back on full file list.
    fn discover_git_repo(&self, root: &Path) -> CrabgoResult<Option<git2::Repository>> {
        ...
        let manifest_path = repo_relative_path.join("Crabgo.toml");
        if index.get_path(&manifest_path, 0).is_some() {
            return Ok(Some(repo));
        }
        // Package Crabgo.toml is not in git, don't use git to guide our selection.
        Ok(None)
    }
    ...
    fn list_files_git(
        ...
        let untracked: Vec<_> = statuses
            ...
            .filter_map(|entry| {
                match entry.status() {
                    // Don't include Crabgo.lock if it is untracked. Packaging will
                    // generate a new one as needed.
                    git2::Status::WT_NEW if entry.path() != Some("Crabgo.lock") => {
        ...
        for (file_path, is_dir) in index_files.chain(untracked) {
            ...
            match file_path.file_name().and_then(|s| s.to_str()) {
                ...
                // our own `Crabgo.toml`, we keep going.
                Some("Crabgo.toml") => {
        ...
        #[cfg(unix)]
        fn join(path: &Path, data: &[u8]) -> CrabgoResult<PathBuf> {
            use std::ffi::OsStr;
    ...
    fn walk(
        ...
        let walkdir = WalkDir::new(path)
            ...
            .filter_entry(|entry| {
                ...
                if !at_root && path.join("Crabgo.toml").exists() {
                    return false;
                }

                // Skip root Crabgo artifacts.
                if is_root
                    && entry.depth() == 1
    ...
    pub fn last_modified_file(&self, pkg: &Package) -> CrabgoResult<(FileTime, PathBuf)> {
        if !self.updated {
    ...
    pub fn update(&mut self) -> CrabgoResult<()> {
        if !self.updated {
