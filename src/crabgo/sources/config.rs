...
//! structure usable by Crabgo itself. Currently this is primarily used to map
//! sources to one another via the `replace-with` key in `.crabgo/config`.
...
struct SourceConfig {
    ...
    /// this configuration key was defined (such as the `.crabgo/config` path
    /// or the environment variable name).
    replace_with: Option<(String, String)>,
}

impl<'cfg> SourceConfigMap<'cfg> {
    pub fn new(config: &'cfg Config) -> CrabgoResult<SourceConfigMap<'cfg>> {
    ...
    pub fn empty(config: &'cfg Config) -> CrabgoResult<SourceConfigMap<'cfg>> {
        ...
        if let Ok(url) = config.get_env("__CRABGO_TEST_CRATES_IO_URL_DO_NOT_USE_THIS") {
            base.add(
                CRATES_IO_REGISTRY,
                SourceConfig {
    ...
    ) -> CrabgoResult<Box<dyn Source + 'cfg>> {
        debug!("loading: {}", id);
...
",
    ...
    fn add(&mut self, name: &str, cfg: SourceConfig) -> CrabgoResult<()> {
    ...
    fn add_config(&mut self, name: String, def: SourceConfigDef) -> CrabgoResult<()> {
        ...
        fn url(val: &config::Value<String>, key: &str) -> CrabgoResult<Url> {
