...
impl FileLock {
    ...
    pub fn remove_siblings(&self) -> CrabgoResult<()> {
        let path = self.path();
...
/// concurrent invocations across multiple instances of Crabgo.
#[derive(Clone, Debug)]
...
impl Filesystem {
    ...
    ///
    /// Handles errors where other Crabgo processes are also attempting to
    /// concurrently create this directory.
    pub fn create_dir(&self) -> CrabgoResult<()> {
    ...
    pub fn open_rw<P>(&self, path: P, config: &Config, msg: &str) -> CrabgoResult<FileLock>
    where
        P: AsRef<Path>,
    {
        self.open(
    ...
    /// error.
    pub fn open_ro<P>(&self, path: P, config: &Config, msg: &str) -> CrabgoResult<FileLock>
    where
        P: AsRef<Path>,
    {
        self.open(
...
///
/// Almost all long-running blocking actions in Crabgo have a status message
...
fn acquire(
    ...
    // there anyway.
    //
    // [1]: https://github.com/rust-lang/crabgo/issues/2615
