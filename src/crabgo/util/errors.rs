...
pub type CrabgoResult<T> = anyhow::Result<T>;
...
#[derive(Debug)]
/// The CLI error is the error type used at Crabgo's CLI-layer.
///
/// All errors from the lib side of Crabgo will get wrapped with this error.
...
pub struct CliError {
    ...
    /// code without displaying a message. For example `crabgo run -q` where
