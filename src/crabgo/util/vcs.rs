...
impl GitRepo {
    pub fn init(path: &Path, _: &Path) -> CrabgoResult<GitRepo> {
...
impl HgRepo {
    pub fn init(path: &Path, cwd: &Path) -> CrabgoResult<HgRepo> {
    ...
    pub fn discover(path: &Path, cwd: &Path) -> CrabgoResult<HgRepo> {
        ProcessBuilder::new("hg")
            .cwd(cwd)
            .arg("--cwd")
            .arg(path)
...
impl PijulRepo {
    pub fn init(path: &Path, cwd: &Path) -> CrabgoResult<PijulRepo> {
...
impl FossilRepo {
    pub fn init(path: &Path, cwd: &Path) -> CrabgoResult<FossilRepo> {
