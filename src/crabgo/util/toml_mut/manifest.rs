...
impl Manifest {
    ...
    pub fn package_name(&self) -> CrabgoResult<&str> {
        self.data
            .as_table()
            .get("package")
    ...
    pub fn get_table<'a>(&'a self, table_path: &[String]) -> CrabgoResult<&'a toml_edit::Item> {
        /// Descend into a manifest until the required table is found.
        fn descend<'a>(
            input: &'a toml_edit::Item,
            path: &[String],
        ) -> CrabgoResult<&'a toml_edit::Item> {
    ...
    pub fn get_table_mut<'a>(
        &'a mut self,
        table_path: &[String],
    ) -> CrabgoResult<&'a mut toml_edit::Item> {
        /// Descend into a manifest until the required table is found.
        fn descend<'a>(
...
/// An editable Crabgo manifest that is available locally.
#[derive(Debug, Clone)]
...
impl LocalManifest {
    ...
    pub fn try_new(path: &Path) -> CrabgoResult<Self> {
        if !path.is_absolute() {
            anyhow::bail!("can only edit absolute paths, got {}", path.display());
        }
        let data = crabgo_util::paths::read(&path)?;
        let manifest = data.parse().context("Unable to parse Crabgo.toml")?;
        Ok(LocalManifest {
            manifest,
            path: path.to_owned(),
        })
    ...
    pub fn write(&self) -> CrabgoResult<()> {
        ...
        crabgo_util::paths::write(&self.path, new_contents_bytes)
    }
    ...
    pub fn get_dependency_versions<'s>(
        &'s self,
        dep_key: &'s str,
    ) -> impl Iterator<Item = (DepTable, CrabgoResult<Dependency>)> + 's {
    ...
    /// Remove entry from a Crabgo.toml.
    pub fn remove_from_table(&mut self, table_path: &[String], name: &str) -> CrabgoResult<()> {
...
fn parse_manifest_err() -> anyhow::Error {
    anyhow::format_err!("unable to parse external Crabgo.toml")
}
