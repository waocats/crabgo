...
/// A dependency handled by Crabgo.
///
/// `None` means the field will be blank in TOML.
#[derive(Debug, PartialEq, Eq, Clone)]
#[non_exhaustive]
pub struct Dependency {
    /// The name of the dependency (as it is set in its `Crabgo.toml` and known
    /// to crates.io).
    pub name: String,
...
impl Dependency {
    ...
    pub fn source_id(&self, config: &Config) -> CrabgoResult<MaybeWorkspace<SourceId>> {
    ...
    pub fn query(
        &self,
        config: &Config,
    ) -> CrabgoResult<MaybeWorkspace<crate::core::dependency::Dependency>> {
...
impl Dependency {
    ...
    pub fn from_toml(crate_root: &Path, key: &str, item: &toml_edit::Item) -> CrabgoResult<Self> {
            ...
            let features = if let Some(value) = table.get("features") {
                Some(
                    value
                        ...
                        .collect::<CrabgoResult<IndexSet<String>>>()?,
                )
            } else {
                None
            };
...
impl RegistrySource {
    ...
    pub fn new(version: impl AsRef<str>) -> Self {
        ...
        // store in the crabgo toml files.  This would cause a warning upon compilation
...
impl PathSource {
    ...
    /// Get the SourceID for this dependency.
    pub fn source_id(&self) -> CrabgoResult<SourceId> {
