//! Crabgo's config system.
//!
//! The `Config` object contains general information about the environment,
//! and provides access to Crabgo's configuration files.
//!
//! ## Config value API
//!
...
use crate::util::{internal, toml as crabgo_toml};
...
enum WhyLoad {
    ...
    ///
    /// [`config-include`]: https://doc.rust-lang.org/nightly/crabgo/reference/unstable.html#config-include
    Cli,
...
/// Configuration information for crabgo. This is not specific to a build, it is information
/// relating to crabgo itself.
#[derive(Debug)]
pub struct Config {
    /// The location of the user's Crabgo home directory. OS-dependent.
    ...
    /// The current working directory of crabgo
    cwd: PathBuf,
    /// Directory where config file searching should stop (inclusive).
    search_stop_path: Option<PathBuf>,
    /// The location of the crabgo executable (path to current process)
    crabgo_exe: LazyCell<PathBuf>,
    ...
    /// Cached configuration parsed by Crabgo
    http_config: LazyCell<CrabgoHttpConfig>,
    future_incompat_config: LazyCell<CrabgoFutureIncompatConfig>,
    net_config: LazyCell<CrabgoNetConfig>,
    build_config: LazyCell<CrabgoBuildConfig>,
    ...
    ///      that does not opt in with `masquerade_as_nightly_crabgo`
    ...
    /// - this is a crabgo executable that was built from source
    /// - this is an `#[test]` that called `enable_nightly_features`
    /// - this is an integration test that uses `ProcessBuilder`
    ///       that called `masquerade_as_nightly_crabgo`
...
impl Config {
    ...
    /// This does only minimal initialization. In particular, it does not load
    /// any config files from disk. Those will be loaded lazily as-needed.
    pub fn default() -> CrabgoResult<Config> {
        let shell = Shell::new();
        let cwd = env::current_dir()
            .with_context(|| "couldn't get the current directory of the process")?;
        let homedir = homedir(&cwd).ok_or_else(|| {
            anyhow!(
                "Crabgo couldn't find your home directory. \
    ...
    /// Gets the user's Crabgo home directory (OS-dependent).
    ...
    /// Gets the Crabgo Git directory (`<crabgo_home>/git`).
    pub fn git_path(&self) -> Filesystem {
        self.home_path.join("git")
    }

    /// Gets the Crabgo base directory for all registry information (`<crabgo_home>/registry`).
    pub fn registry_base_path(&self) -> Filesystem {
        self.home_path.join("registry")
    }

    /// Gets the Crabgo registry index directory (`<crabgo_home>/registry/index`).
    pub fn registry_index_path(&self) -> Filesystem {
        self.registry_base_path().join("index")
    }

    /// Gets the Crabgo registry cache directory (`<crabgo_home>/registry/path`).
    pub fn registry_cache_path(&self) -> Filesystem {
        self.registry_base_path().join("cache")
    }

    /// Gets the Crabgo registry source directory (`<crabgo_home>/registry/src`).
    pub fn registry_source_path(&self) -> Filesystem {
        self.registry_base_path().join("src")
    }

    /// Gets the default Crabgo registry.
    pub fn default_registry(&self) -> CrabgoResult<Option<String>> {
        Ok(self
    ...
    pub fn rustdoc(&self) -> CrabgoResult<&Path> {
        self.rustdoc
    ...
    pub fn load_global_rustc(&self, ws: Option<&Workspace<'_>>) -> CrabgoResult<Rustc> {
    ...
    /// Gets the path to the `crabgo` executable.
    pub fn crabgo_exe(&self) -> CrabgoResult<&Path> {
        self.crabgo_exe
            .try_borrow_with(|| {
                ...
                fn from_current_exe() -> CrabgoResult<PathBuf> {
                    // Try fetching the path to `crabgo` using `env::current_exe()`.
                ...
                fn from_argv() -> CrabgoResult<PathBuf> {
                    ...
                    // - a relative path (e.g., `./crabgo`, `target/debug/crabgo`), or
                    // - an absolute path (e.g., `/usr/local/bin/crabgo`).
                ...
                let exe = from_env()
                    ...
                    .or_else(|_| from_argv())
                    .with_context(|| "couldn't get the path to crabgo executable")?;
                Ok(exe)
            })
            .map(AsRef::as_ref)
    }
    ...
    pub fn values(&self) -> CrabgoResult<&HashMap<String, ConfigValue>> {
    ...
    /// currently only exists for `crabgo vendor` to remove the `source`
    /// entries. This doesn't respect environment variables. You should avoid
    /// using this if possible.
    pub fn values_mut(&mut self) -> CrabgoResult<&mut HashMap<String, ConfigValue>> {
        let _ = self.values()?;
        Ok(self
            .values
            .borrow_mut()
    ...
    // Note: this is used by RLS, not Crabgo.
    pub fn set_values(&self, values: HashMap<String, ConfigValue>) -> CrabgoResult<()> {
    ...
    pub fn reload_rooted_at<P: AsRef<Path>>(&mut self, path: P) -> CrabgoResult<()> {
    ...
    fn get_cv(&self, key: &ConfigKey) -> CrabgoResult<Option<ConfigValue>> {
    ...
    ) -> CrabgoResult<Option<ConfigValue>> {
        log::trace!("get cv {:?}", key);
        if key.is_root() {
            // Returning the entire root table (for example `crabgo config get`
    ...
    pub(crate) fn get_cv_with_env(&self, key: &ConfigKey) -> CrabgoResult<Option<CV>> {
    ...
    pub fn get_env(&self, key: impl AsRef<OsStr>) -> CrabgoResult<String> {
    ...
    fn has_key(&self, key: &ConfigKey, env_prefix_ok: bool) -> CrabgoResult<bool> {
    ...
    pub fn get_string(&self, key: &str) -> CrabgoResult<OptValue<String>> {
    ...
    /// more details.
    pub fn get_path(&self, key: &str) -> CrabgoResult<OptValue<PathBuf>> {
    ...
    /// if you want that.
    pub fn get_list(&self, key: &str) -> CrabgoResult<OptValue<Vec<(String, Definition)>>> {
        let key = ConfigKey::from_str(key);
        self._get_list(&key)
    }

    fn _get_list(&self, key: &ConfigKey) -> CrabgoResult<OptValue<Vec<(String, Definition)>>> {
    ...
    fn get_list_or_string(
        ...
        key: &ConfigKey,
        merge: bool,
    ) -> CrabgoResult<Vec<(String, Definition)>> {
        let mut res = Vec::new();
    ...
    fn get_table(&self, key: &ConfigKey) -> CrabgoResult<OptValue<HashMap<String, CV>>> {
        match self.get_cv(key)? {
    ...
    fn expected<T>(&self, ty: &str, key: &ConfigKey, val: &CV) -> CrabgoResult<T> {
    ...
    pub fn configure(
        ...
        // commands like `crabgo version` to error out due to config file
        // problems.
    ...
    fn load_unstable_flags_from_config(&mut self) -> CrabgoResult<()> {
    ...
    pub fn load_values(&self) -> CrabgoResult<HashMap<String, ConfigValue>> {
        self.load_values_from(&self.cwd)
    }

    /// Like [`load_values`](Config::load_values) but without merging config values.
    ///
    /// This is primarily crafted for `crabgo config` command.
    pub(crate) fn load_values_unmerged(&self) -> CrabgoResult<Vec<ConfigValue>> {
        ...
        .with_context(|| "could not load Crabgo configuration")?;
        Ok(result)
    }
    ...
    fn load_values_from(&self, path: &Path) -> CrabgoResult<HashMap<String, ConfigValue>> {
    ...
    fn load_file(&self, path: &Path) -> CrabgoResult<ConfigValue> {
    ...
    fn _load_file(
        ...
        let toml = crabgo_toml::parse_document(&contents, path, self).with_context(|| {
    ...
    fn include_paths(
        ...
        cv: &mut CV,
        remove: bool,
    ) -> CrabgoResult<Vec<(String, PathBuf, Definition)>> {
    ...
    pub(crate) fn cli_args_as_table(&self) -> CrabgoResult<ConfigValue> {
    ...
    fn merge_cli_args(&mut self) -> CrabgoResult<()> {
    ...
    /// .toml extensions on Crabgo's config files, which were historically not used.
    ...
    fn get_file_path(
        ...
        if possible.exists() {
            if warn && possible_with_extension.exists() {
                ...
                // support multiple Crabgo versions at once and not
                // get a warning.
    ...
    fn walk_tree<F>(&self, pwd: &Path, home: &Path, mut walk: F) -> CrabgoResult<()>
    where
        F: FnMut(&Path) -> CrabgoResult<()>,
    {
        let mut stash: HashSet<PathBuf> = HashSet::new();

        for current in paths::ancestors(pwd, self.search_stop_path.as_deref()) {
            if let Some(path) = self.get_file_path(&current.join(".crabgo"), "config", true)? {
                walk(&path)?;
                stash.insert(path);
            }
        }
    ...
    pub fn get_registry_index(&self, registry: &str) -> CrabgoResult<Url> {
    ...
    pub fn check_registry_index_not_set(&self) -> CrabgoResult<()> {
    ...
    fn resolve_registry_index(&self, index: &Value<String>) -> CrabgoResult<Url> {
    ...
    pub fn load_credentials(&self) -> CrabgoResult<()> {
        ...
        // Backwards compatibility for old `.crabgo/credentials` layout.
        {
    ...
    pub fn http(&self) -> CrabgoResult<&RefCell<Easy>> {
        let http = self
            .easy
    ...
    pub fn http_config(&self) -> CrabgoResult<&CrabgoHttpConfig> {
        self.http_config
            .try_borrow_with(|| self.get::<CrabgoHttpConfig>("http"))
    }

    pub fn future_incompat_config(&self) -> CrabgoResult<&CrabgoFutureIncompatConfig> {
        self.future_incompat_config
            .try_borrow_with(|| self.get::<CrabgoFutureIncompatConfig>("future-incompat-report"))
    }

    pub fn net_config(&self) -> CrabgoResult<&CrabgoNetConfig> {
        self.net_config
            .try_borrow_with(|| self.get::<CrabgoNetConfig>("net"))
    }

    pub fn build_config(&self) -> CrabgoResult<&CrabgoBuildConfig> {
        self.build_config
            .try_borrow_with(|| self.get::<CrabgoBuildConfig>("build"))
    }
    ...
    pub fn env_config(&self) -> CrabgoResult<&EnvConfig> {
        let env_config = self
            .env_config
    ...
    /// early, and in some situations (like `crabgo version`) we don't want to
    /// fail if there are problems with the config file.
    pub fn validate_term_config(&self) -> CrabgoResult<()> {
    ...
    pub fn target_cfgs(&self) -> CrabgoResult<&Vec<(String, TargetCfgConfig)>> {
        self.target_cfgs
            .try_borrow_with(|| target::load_target_cfgs(self))
    }

    pub fn doc_extern_map(&self) -> CrabgoResult<&RustdocExternMap> {
    ...
    pub fn target_applies_to_host(&self) -> CrabgoResult<bool> {
        target::get_target_applies_to_host(self)
    }

    /// Returns the `[host]` table definition for the given target triple.
    pub fn host_cfg_triple(&self, target: &str) -> CrabgoResult<TargetConfig> {
        target::load_host_triple(self, target)
    }

    /// Returns the `[target]` table definition for the given target triple.
    pub fn target_cfg_triple(&self, target: &str) -> CrabgoResult<TargetConfig> {
        target::load_target_triple(self, target)
    }

    pub fn crates_io_source_id<F>(&self, f: F) -> CrabgoResult<SourceId>
    where
        F: FnMut() -> CrabgoResult<SourceId>,
    {
    ...
    pub fn get<'de, T: serde::de::Deserialize<'de>>(&self, key: &str) -> CrabgoResult<T> {
        let d = Deserializer {
            config: self,
    ...
    pub fn assert_package_cache_locked<'a>(&self, f: &'a Filesystem) -> &'a Path {
        ...
        assert!(
            ...
            "package cache lock is not currently held, Crabgo forgot to call \
    ...
    pub fn acquire_package_cache_lock(&self) -> CrabgoResult<PackageCacheLock<'_>> {
        ...
        match *slot {
            ...
            None => {
                ...
                // in subfolders, so they're assumed by Crabgo to also be
...
impl ConfigValue {
    fn from_toml(def: Definition, toml: toml::Value) -> CrabgoResult<ConfigValue> {
        match toml {
    ...
    fn merge(&mut self, from: ConfigValue, force: bool) -> CrabgoResult<()> {
        match (self, from) {
    ...
    pub fn i64(&self, key: &str) -> CrabgoResult<(i64, &Definition)> {
        match self {
    ...
    pub fn string(&self, key: &str) -> CrabgoResult<(&str, &Definition)> {
        match self {
    ...
    pub fn table(&self, key: &str) -> CrabgoResult<(&HashMap<String, ConfigValue>, &Definition)> {
        match self {
    ...
    pub fn list(&self, key: &str) -> CrabgoResult<&[(String, Definition)]> {
        match self {
    ...
    pub fn boolean(&self, key: &str) -> CrabgoResult<(bool, &Definition)> {
        match self {
    ...
    fn expected<T>(&self, wanted: &str, key: &str) -> CrabgoResult<T> {
        bail!(
...
pub fn save_credentials(
    ...
    let mut toml = crabgo_toml::parse_document(&contents, file.path(), cfg)?;
    ...
    #[cfg(unix)]
    fn set_permissions(file: &File, mode: u32) -> CrabgoResult<()> {
...
#[derive(Debug, Default, Deserialize, PartialEq)]
#[serde(rename_all = "kebab-case")]
pub struct CrabgoFutureIncompatConfig {
    frequency: Option<CrabgoFutureIncompatFrequencyConfig>,
}

#[derive(Debug, Default, Deserialize, PartialEq)]
#[serde(rename_all = "kebab-case")]
pub enum CrabgoFutureIncompatFrequencyConfig {
    #[default]
    Always,
    Never,
}

impl CrabgoFutureIncompatConfig {
    pub fn should_display_message(&self) -> bool {
        use CrabgoFutureIncompatFrequencyConfig::*;
...
#[serde(rename_all = "kebab-case")]
pub struct CrabgoNetConfig {
    pub retry: Option<u32>,
    pub offline: Option<bool>,
    pub git_fetch_with_cli: Option<bool>,
    pub ssh: Option<CrabgoSshConfig>,
}

#[derive(Debug, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub struct CrabgoSshConfig {
    pub known_hosts: Option<Vec<Value<String>>>,
}

#[derive(Debug, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub struct CrabgoBuildConfig {
...
impl BuildTargetConfig {
    ...
    pub fn values(&self, config: &Config) -> CrabgoResult<Vec<String>> {
        let map = |s: &String| {
            if s.ends_with(".json") {
