...
pub struct TargetConfig {
    /// Process to run as a wrapper for `crabgo run`, `test`, and `bench` commands.
    pub runner: OptValue<PathAndArgs>,
...
pub(super) fn load_target_cfgs(config: &Config) -> CrabgoResult<Vec<(String, TargetCfgConfig)>> {
...
pub(super) fn get_target_applies_to_host(config: &Config) -> CrabgoResult<bool> {
...
pub(super) fn load_host_triple(config: &Config, triple: &str) -> CrabgoResult<TargetConfig> {
...
pub(super) fn load_target_triple(config: &Config, triple: &str) -> CrabgoResult<TargetConfig> {
    load_config_table(config, &format!("target.{}", triple))
}

/// Loads a single table for the given prefix.
fn load_config_table(config: &Config, prefix: &str) -> CrabgoResult<TargetConfig> {
...
fn parse_links_overrides(
    ...
    config: &Config,
) -> CrabgoResult<BTreeMap<String, BuildOutput>> {
...
fn extra_link_args<'a>(
    ...
    key: &str,
    value: &'a CV,
) -> CrabgoResult<impl Iterator<Item = (LinkType, String)> + 'a> {
