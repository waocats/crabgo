...
//! from.
//!
//! Often Crabgo wants to report semantic error information or other sorts of
...
pub(crate) const VALUE_FIELD: &str = "$__crabgo_private_value";
pub(crate) const DEFINITION_FIELD: &str = "$__crabgo_private_definition";
pub(crate) const NAME: &str = "$__crabgo_private_Value";
pub(crate) static FIELDS: [&str; 2] = [VALUE_FIELD, DEFINITION_FIELD];

/// Location where a config value is defined.
#[derive(Clone, Debug, Eq)]
pub enum Definition {
    /// Defined in a `.crabgo/config`, includes the path to the file.
    Path(PathBuf),
...
impl Definition {
    ...
    ///
    /// If from a file, it is the directory above `.crabgo/config`.
