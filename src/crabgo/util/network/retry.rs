...
impl<'a> Retry<'a> {
    pub fn new(config: &'a Config) -> CrabgoResult<Retry<'a>> {
        Ok(Retry {
            config,
            retries: 0,
    ...
    pub fn r#try<T>(&mut self, f: impl FnOnce() -> CrabgoResult<T>) -> RetryResult<T> {
        match f() {
...
///
/// Closure must return a `CrabgoResult`.
///
/// # Examples
///
/// ```
/// # use crate::crabgo::util::{CrabgoResult, Config};
/// # let download_something = || return Ok(());
/// # let config = Config::default().unwrap();
/// use crabgo::util::network;
/// let crabgo_result = network::retry::with_retry(&config, || download_something());
/// ```
pub fn with_retry<T, F>(config: &Config, mut callback: F) -> CrabgoResult<T>
where
    F: FnMut() -> CrabgoResult<T>,
{
...
fn with_retry_repeats_the_call_then_works() {
    ...
    .into();
    let mut results: Vec<CrabgoResult<()>> = vec![Ok(()), Err(error1), Err(error2)];
    let config = Config::default().unwrap();
