//! This module implements Crabgo conventions for directory layout:
//!
...
//!
//! It is a bit tricky because we need match explicit information from `Crabgo.toml`
...
fn clean_bins(
    ...
    for bin in &bins {
        ...
        // check if the corresponding crabgo feature has been activated.
        ...
        if restricted_names::is_conflicting_artifact_name(&name) {
            anyhow::bail!(
                "the binary target name `{}` is forbidden, \
                 it conflicts with with crabgo's build directory names",
                name
            )
        }
    }
...
) -> CrabgoResult<Vec<(PathBuf, TomlTarget)>> {
    clean_targets_with_legacy_path(
        target_kind_human,
        target_kind,
        toml_targets,
                ...
                None => {
                    if edition == Edition::Edition2015 {
                        if !rem_targets.is_empty() {
                            ...
                            warnings.push(format!(
                                "\
An explicit [[{section}]] section is specified in Crabgo.toml which currently
disables Crabgo from automatically inferring other {target_kind_human} targets.
...
fn validate_unique_names(targets: &[TomlTarget], target_kind: &str) -> CrabgoResult<()> {
...
fn configure(toml: &TomlTarget, target: &mut Target) -> CrabgoResult<()> {
    let t2 = target.clone();
    target
...
Crabgo doesn't know which to use because multiple target files found at `{}` and `{}`.",
                target.name(),
                target_kind,
