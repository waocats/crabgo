...
use crate::util::errors::{CrabgoResult, ManifestError};
...
/// Loads a `Crabgo.toml` from a file on disk.
///
...
/// Parse an already-loaded `Crabgo.toml` as a Crabgo manifest.
///
/// This could result in a real or virtual manifest being returned.
///
/// A list of nested paths is also returned, one for each path dependency
...
pub fn read_manifest_from_str(
    ...
    source_id: SourceId,
    config: &Config,
) -> CrabgoResult<(EitherManifest, Vec<PathBuf>)> {
    ...
    if let Some(package) = toml.get("package").or_else(|| toml.get("project")) {
        if let Some(feats) = package.get("crabgo-features") {
            bail!(
                "crabgo-features = {} was found in the wrong location: it \
                 should be set at the top of Crabgo.toml before any tables",
                feats
            );
        }
    }
...
pub fn parse_document(toml: &str, _file: &Path, _config: &Config) -> CrabgoResult<toml::Table> {
...
pub struct DetailedTomlDependency<P: Clone = String> {
    ...
    /// This is an internal implementation detail. When Crabgo creates a
    ...
    /// registry names configured, so Crabgo can't rely on just the name for
    /// crates published by other users.
    registry_index: Option<String>,
    // `path` is relative to the file it appears in. If that's a `Crabgo.toml`, it'll be relative to
    // that TOML file, and if it's a `.crabgo/config` file, it'll be relative to that file.
    path: Option<P>,
    git: Option<String>,
...
/// This type is used to deserialize `Crabgo.toml` files.
#[derive(Debug, Deserialize, Serialize)]
#[serde(rename_all = "kebab-case")]
pub struct TomlManifest {
    crabgo_features: Option<Vec<String>>,
...
impl TomlProfiles {
    ...
    ///
    /// It's a bit unfortunate both `-Z` flags and `crabgo-features` are required,
    /// because profiles can now be set in either `Crabgo.toml` or `config.toml`.
    pub fn validate(
        &self,
...
pub struct TomlProfile {
    ...
    // Note that `rustflags` is used for the crabgo-feature `profile_rustflags`
...
impl TomlProfile {
    ...
    pub fn validate_name(name: &str) -> CrabgoResult<()> {
        if let Some(ch) = name
            .chars()
        ...
        const SEE_DOCS: &str = "See https://doc.rust-lang.org/crabgo/reference/profiles.html \
        ...
        // syntax like crabgo:foo if needed. But it is unlikely these will ever
        // be used.
        if matches!(
            lower_name.as_str(),
            "build"
    ...
    fn validate_override(&self, which: &str) -> CrabgoResult<()> {
...
/// `workspace` is not `false` in a `Crabgo.toml`
...
/// An enum that allows for inheriting keys from a workspace in a Crabgo.toml.
...
impl<T, W: WorkspaceInherit> MaybeWorkspace<T, W> {
    fn resolve<'a>(
        self,
        label: &str,
        get_ws_inheritable: impl FnOnce() -> CrabgoResult<T>,
    ) -> CrabgoResult<T> {
        match self {
    ...
    fn resolve_with_self<'a>(
        self,
        label: &str,
        get_ws_inheritable: impl FnOnce(&W) -> CrabgoResult<T>,
    ) -> CrabgoResult<T> {
        match self {
            MaybeWorkspace::Defined(value) => Ok(value),
...
impl TomlWorkspaceDependency {
    fn resolve<'a>(
        &self,
        name: &str,
        inheritable: impl FnOnce() -> CrabgoResult<&'a InheritableFields>,
        cx: &mut Context<'_, '_>,
    ) -> CrabgoResult<TomlDependency> {
...
/// Represents the `package`/`project` sections of a `Crabgo.toml`.
///
...
impl InheritableFields {
    ...
    pub fn dependencies(&self) -> CrabgoResult<BTreeMap<String, TomlDependency>> {
    ...
    pub fn get_dependency(&self, name: &str, package_root: &Path) -> CrabgoResult<TomlDependency> {
        self.dependencies.clone().map_or(
            Err(anyhow!("`workspace.dependencies` was not defined")),
            |deps| {
    ...
    pub fn version(&self) -> CrabgoResult<semver::Version> {
    ...
    pub fn authors(&self) -> CrabgoResult<Vec<String>> {
    ...
    pub fn description(&self) -> CrabgoResult<String> {
    ...
    pub fn homepage(&self) -> CrabgoResult<String> {
    ...
    pub fn documentation(&self) -> CrabgoResult<String> {
    ...
    pub fn readme(&self, package_root: &Path) -> CrabgoResult<StringOrBool> {
    ...
    pub fn keywords(&self) -> CrabgoResult<Vec<String>> {
    ...
    pub fn categories(&self) -> CrabgoResult<Vec<String>> {
    ...
    pub fn license(&self) -> CrabgoResult<String> {
    ...
    pub fn license_file(&self, package_root: &Path) -> CrabgoResult<String> {
    ...
    pub fn repository(&self) -> CrabgoResult<String> {
    ...
    pub fn publish(&self) -> CrabgoResult<VecStringOrBool> {
    ...
    pub fn edition(&self) -> CrabgoResult<String> {
    ...
    pub fn rust_version(&self) -> CrabgoResult<String> {
    ...
    pub fn badges(&self) -> CrabgoResult<BTreeMap<String, BTreeMap<String, String>>> {
    ...
    pub fn exclude(&self) -> CrabgoResult<Vec<String>> {
    ...
    pub fn include(&self) -> CrabgoResult<Vec<String>> {
...
impl TomlManifest {
    ...
    pub fn prepare_for_publish(
        ...
        if ws.resolve_behavior() != current_resolver {
            // This ensures the published crate if built as a root (e.g. `crabgo install`) will
        ...
        if let Some(license_file) = &package.license_file {
            ...
            if abs_license_path.strip_prefix(package_root).is_err() {
                // This path points outside of the package root. `crabgo package`
        ...
        return Ok(TomlManifest {
            ...
            crabgo_features: self.crabgo_features.clone(),
        });
        ...
        fn map_deps(
        ...
        ) -> CrabgoResult<Option<BTreeMap<String, MaybeWorkspaceDependency>>> {
            let deps = match deps {
                Some(deps) => deps,
                None => return Ok(None),
            };
            let deps = deps
                ...
                .collect::<CrabgoResult<BTreeMap<_, _>>>()?;
            Ok(Some(deps))
        }
        ...
        ) -> CrabgoResult<MaybeWorkspaceDependency> {
            let dep = match dep {
    ...
    pub fn to_real_manifest(
        ...
        package_root: &Path,
        config: &Config,
    ) -> CrabgoResult<(Manifest, Vec<PathBuf>)> {
        fn get_ws(
            config: &Config,
            resolved_path: &Path,
            workspace_config: &WorkspaceConfig,
        ) -> CrabgoResult<InheritableFields> {
        ...
        let empty = Vec::new();
        let crabgo_features = me.crabgo_features.as_ref().unwrap_or(&empty);
        let features = Features::new(crabgo_features, config, &mut warnings, source_id.is_path())?;
        ...
        let resolved_path = package_root.join("Crabgo.toml");
        ...
        fn process_dependencies(
            ...
            let inheritable = || {
                inherit_cell.try_borrow_with(|| {
                    get_ws(cx.config, &cx.root.join("Crabgo.toml"), &workspace_config)
                })
            };
        ...
        let resolved_toml = TomlManifest {
            crabgo_features: me.crabgo_features.clone(),
        ...
        if package.license_file.is_some() && package.license.is_some() {
            manifest.warnings_mut().add_warning(
                "only one of `license` or `license-file` is necessary\n\
                 ...
                 See https://doc.rust-lang.org/crabgo/reference/manifest.html#the-license-and-license-file-fields \
    ...
    fn to_virtual_manifest(
        ...
        root: &Path,
        config: &Config,
    ) -> CrabgoResult<(VirtualManifest, Vec<PathBuf>)> {
    ...
    fn replace(&self, cx: &mut Context<'_, '_>) -> CrabgoResult<Vec<(PackageIdSpec, Dependency)>> {
    ...
    fn patch(&self, cx: &mut Context<'_, '_>) -> CrabgoResult<HashMap<Url, Vec<Dependency>>> {
    ...
    fn maybe_custom_build(
        ...
        match *build {
            ...
            None => {
                // If there is a `build.rs` file next to the `Crabgo.toml`, assume it is
                // a build script.
                if build_rs.is_file() {
                    Some(build_rs)
                } else {
...
fn inheritable_from_path(
    ...
    // Workspace path should have Crabgo.toml at the end
...
struct TomlTarget {
    ...
    // versions of Crabgo also accepted `crate_type`, so look for both.
    crate_type: Option<Vec<String>>,
    #[serde(rename = "crate_type")]
    crate_type2: Option<Vec<String>>,

    path: Option<PathValue>,
    // Note that `filename` is used for the crabgo-feature `different_binary_name`
