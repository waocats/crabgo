...
//! cross-platform way for the `crabgo fix` command.
...
impl<'a> DiagnosticPrinter<'a> {
    ...
    pub fn print(&mut self, msg: &Message) -> CrabgoResult<()> {
        match msg {
            Message::Migrating {
                file,
                from_edition,
...
1. Start with `edition = \"{prev_edition}\"` in `Crabgo.toml`
2. Run `crabgo fix --edition`
3. Modify `Crabgo.toml` to set `edition = \"{this_edition}\"`
