...
use crate::util::{profile, CrabgoResult, Config, StableHasher};
...
impl Rustc {
    ...
    pub fn new(
        ...
        let extract = |field: &str| -> CrabgoResult<&str> {
            verbose_version
                .lines()
...
/// current toolchain.
///
/// <https://github.com/rust-lang/crabgo/issues/5315>
    ...
    fn load(
        ...
        match (
            ...
            (Some(cache_location), Ok(rustc_fingerprint)) => {
                ...
                fn read(path: &Path) -> CrabgoResult<CacheData> {
            ...
            self.data.outputs.insert(
                ...
                Output {
                    ...
                    status: if output.status.success() {
                        ...
                        crabgo_util::exit_status_to_string(output.status)
                    },
...
fn rustc_fingerprint(
    ...
    let hash_exe = |hasher: &mut _, path| -> CrabgoResult<()> {
    ...
    // the rustup shim at `~/.crabgo/bin/rustup`), because `RUSTUP_TOOLCHAIN`
