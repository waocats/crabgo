...
pub trait CommandExt: Sized {
    ...
    fn arg_manifest_path(self) -> Self {
        self._arg(opt("manifest-path", "Path to Crabgo.toml").value_name("PATH"))
    }
    ...
    fn arg_quiet(self) -> Self {
        self._arg(flag("quiet", "Do not print crabgo log messages").short('q'))
    }
...
pub enum ProfileChecking {
    /// `crabgo rustc` historically has allowed "test", "bench", and "check". This
    /// variant explicitly allows those.
    LegacyRustc,
    /// `crabgo check` and `crabgo fix` historically has allowed "test". This variant
...
pub trait ArgMatchesExt {
    fn value_of_u32(&self, name: &str) -> CrabgoResult<Option<u32>> {
    ...
    fn value_of_i32(&self, name: &str) -> CrabgoResult<Option<i32>> {
        let arg = match self._value_of(name) {
            None => None,
    ...
    fn root_manifest(&self, config: &Config) -> CrabgoResult<PathBuf> {
        if let Some(path) = self.value_of_path("manifest-path", config) {
            // In general, we try to avoid normalizing paths in Crabgo,
            // but in this particular case we need it to fix #3586.
            let path = paths::normalize_path(&path);
            if !path.ends_with("Crabgo.toml") {
                anyhow::bail!("the manifest-path must be a path to a Crabgo.toml file")
            }
            if !path.exists() {
                anyhow::bail!(
    ...
    fn workspace<'a>(&self, config: &'a Config) -> CrabgoResult<Workspace<'a>> {
    ...
    fn jobs(&self) -> CrabgoResult<Option<i32>> {
    ...
    fn get_profile_name(
        ...
        match (specified_profile, profile_checking) {
            // `crabgo rustc` has legacy handling of these names
            (Some(name @ ("dev" | "test" | "bench" | "check")), ProfileChecking::LegacyRustc)
            // `crabgo fix` and `crabgo check` has legacy handling of this profile name
        ...
        let name = match (self.flag("release"), self.flag("debug"), specified_profile) {
            ...
            // warning in Crabgo.toml, but since `--profile` is new, we can
    ...
    fn packages_from_flags(&self) -> CrabgoResult<Packages> {
        Packages::from_flags(
    ...
    fn compile_options(
        ...
        if let Some(ws) = workspace {
            ...
            // As for crabgo 0.50.0, this won't occur but if someone sneaks in
    ...
    fn cli_features(&self) -> CrabgoResult<CliFeatures> {
    ...
    fn new_options(&self, config: &Config) -> CrabgoResult<NewOptions> {
    ...
    fn registry(&self, config: &Config) -> CrabgoResult<Option<String>> {
    ...
    fn index(&self) -> CrabgoResult<Option<String>> {
