...
use crate::util::{config, config::ConfigKey, CanonicalUrl, CrabgoResult, Config, IntoUrl};
...
pub fn registry_credential_config(
    config: &Config,
    sid: &SourceId,
) -> CrabgoResult<RegistryCredentialConfig> {
    #[derive(Deserialize)]
    ...
    if !sid.is_remote_registry() {
        bail!(
            "{} does not support API commands.\n\
             Check for a source-replacement in .crabgo/config.",
            sid
        );
    }
    ...
    // command line argument, or from a Crabgo.lock file. For this reason, we always
    ...
    // It's possible to have a registry configured in a Crabgo config file,
...
impl fmt::Display for AuthorizationError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if self.sid.is_crates_io() {
            ...
            write!(
                f,
                "{} for `{}`, please run `crabgo login --registry {name}`\nor use environment variable {}",
                self.reason,
...
) -> CrabgoResult<Option<Secret<String>>> {
...
pub fn login(config: &Config, sid: &SourceId, token: RegistryCredentialConfig) -> CrabgoResult<()> {
...
pub fn logout(config: &Config, sid: &SourceId) -> CrabgoResult<()> {
    match registry_credential_config(config, sid)? {
        RegistryCredentialConfig::Process(process) => {
...
fn run_command(
    ...
    sid: &SourceId,
    action: Action,
) -> CrabgoResult<Option<(bool, String)>> {
    let index_url = sid.url().as_str();
    let cred_proc;
    let (exe, args) = if process.0.to_str().unwrap_or("").starts_with("crabgo:") {
...
fn sysroot_credential(
    config: &Config,
    process: &(PathBuf, Vec<String>),
) -> CrabgoResult<(PathBuf, Vec<String>)> {
    let cred_name = process.0.to_str().unwrap().strip_prefix("crabgo:").unwrap();
    let crabgo = config.crabgo_exe()?;
    let root = crabgo
        .parent()
        .and_then(|p| p.parent())
        .ok_or_else(|| format_err!("expected crabgo path {}", crabgo.display()))?;
