...
//!
//! Most of the time when you're running crabgo you expect Ctrl-C to actually
//! terminate the entire tree of processes in play, not just the one at the top
//! (crabgo). This currently works "by default" on Unix platforms because Ctrl-C
...
//! not happen and Ctrl-C just kills crabgo.
//!
...
mod imp {
    ...
    pub unsafe fn setup() -> Option<()> {
        ...
        // when-crabgo-is-killed-subprocesses-are-also-killed, but that requires
        // one crabgo spawned to become its own session leader, so we do that
        // here.
        //
...
mod imp {
    ...
    pub unsafe fn setup() -> Option<Setup> {
        ...
        // to force Crabgo to abort in this situation or force others to *not*
