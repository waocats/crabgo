...
use crabgo_util::paths::append;
use crabgo_util::Sha256;
...
/// Gets the path for local web API uploads. Crabgo will place the contents of a web API
...
impl RegistryBuilder {
    ...
    pub fn build(self) -> TestRegistry {
        let config_path = paths::home().join(".crabgo/config");
...
/// `.crabgo/config` file to ensure that dependencies will use these packages
...
///
/// The `Crabgo.toml` file is automatically generated based on the methods
/// called on `Package` (for example, calling `dep()` will add to the
/// `[dependencies]` automatically). You may also specify a `Crabgo.toml` file
...
///     .build();
///
/// p.crabgo("run").with_stdout("24").run();
/// ```
#[must_use]
pub struct Package {
    name: String,
...
impl HttpServer {
    ...
    fn check_authorized(&self, req: &Request, mutation: Option<Mutation>) -> bool {
        ...
        // https://github.com/rust-lang/rfcs/blob/master/text/3231-crabgo-asymmetric-tokens.md#how-the-registry-server-will-validate-an-asymmetric-token
    ...
    pub fn unauthorized(&self, _req: &Request) -> Response {
        Response {
            code: 401,
            headers: vec![
                r#"WWW-Authenticate: Crabgo login_url="https://test-registry-login/me""#.to_string(),
            ],
...
impl Package {
    ...
    /// causing Crabgo to skip over this version.
    ...
    pub fn crabgo_feature(&mut self, feature: &str) -> &mut Package {
        self.crabgo_features.push(feature.to_owned());
        self
    }

    /// Sets the index schema version for this package.
    ///
    /// See `crabgo::sources::registry::RegistryPackage` for more information.
    ...
    ///
    /// This does not actually use Crabgo's publishing system, but instead
    ...
    pub fn publish(&self) -> String {
        ...
        let deps = self
            ...
            .map(|dep| {
                ...
                // In Crabgo.toml, it is None if it is from crates.io.
    ...
    fn make_archive(&self) {
        ...
        if !self
            .files
            .iter()
            .any(|PackageFile { path, .. }| path == "Crabgo.toml")
        {
    ...
    fn append_manifest<W: Write>(&self, ar: &mut Builder<W>) {
        ...
        if !self.crabgo_features.is_empty() {
            let mut features = String::new();
            serde::Serialize::serialize(
                &self.crabgo_features,
                toml::ser::ValueSerializer::new(&mut features),
            )
            .unwrap();
            manifest.push_str(&format!("crabgo-features = {}\n\n", features));
        }
