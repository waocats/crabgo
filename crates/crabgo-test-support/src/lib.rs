//! # Crabgo test support.
//!
//! See <https://rust-lang.github.io/crabgo/contrib/> for a guide on writing tests.
...
use crabgo_util::{is_ci, ProcessBuilder, ProcessError};
use serde_json;
use url::Url;

use self::paths::CrabgoPathExt;
...
pub fn _curr_dir(mut file_path: &'static Path) -> &'static Path {
    if !file_path.exists() {
        ...
        let prefix = PathBuf::from("src").join("tools").join("crabgo");
...
pub use crabgo_test_macro::crabgo_test;
...
/// A crabgo project to run tests against.
///
...
impl ProjectBuilder {
    /// Root of the project, ex: `/path/to/crabgo/target/cit/t0/foo`
    pub fn root(&self) -> PathBuf {
        self.root.root()
    }

    /// Project's debug dir, ex: `/path/to/crabgo/target/cit/t0/foo/target/debug`
    ...
    pub fn build(mut self) -> Project {
        ...
        let manifest_path = self.root.root().join("Crabgo.toml");
        if !self.no_manifest && self.files.iter().all(|fb| fb.path != manifest_path) {
            self._file(
                Path::new("Crabgo.toml"),
        ...
        for file in self.files.iter_mut() {
            ...
            if is_coarse_mtime() {
                ...
                // that if crabgo is called multiple times, the 2nd call will
                // see targets as "fresh". Without this, if crabgo finishes in
...
impl Project {
    ...
    /// Project's target dir, ex: `/path/to/crabgo/target/cit/t0/foo/target`
    ...
    /// File url for root, ex: `file:///path/to/crabgo/target/cit/t0/foo`
    pub fn url(&self) -> Url {
        path2url(self.root())
    }
    ...
    /// ex: `/path/to/crabgo/target/cit/t0/foo/target/debug/examples/libex.rlib`
    ...
    /// ex: `/path/to/crabgo/target/cit/t0/foo/target/debug/foo`
    ...
    /// ex: `/path/to/crabgo/target/cit/t0/foo/target/release/foo`
    ...
    /// ex: `/path/to/crabgo/target/cit/t0/foo/target/i686-apple-darwin/debug/foo`
    ...
    /// Creates a `ProcessBuilder` to run crabgo.
    /// Arguments can be separated by spaces.
    /// Example:
    ///     p.crabgo("build --bin foo").run();
    pub fn crabgo(&self, cmd: &str) -> Execs {
    ...
    /// Safely run a process after `crabgo build`.
    ///
    ...
    ///
    /// If you have a test that needs to do `crabgo run` multiple
    /// times, you should instead use `crabgo build` and use this
    /// method to run the executable. Each time you call this,
    /// use a new name for `dst`.
    /// See rust-lang/crabgo#5481.
    ...
    /// Returns the contents of `Crabgo.lock`.
    pub fn read_lockfile(&self) -> String {
        self.read_file("Crabgo.lock")
    }
    ...
    /// Modifies `Crabgo.toml` to remove all commented lines.
    pub fn uncomment_root_manifest(&self) {
        let contents = self.read_file("Crabgo.toml").replace("#", "");
        fs::write(self.root().join("Crabgo.toml"), contents).unwrap();
    }
...
pub fn crabgo_exe() -> PathBuf {
    snapbox::cmd::crabgo_bin("crabgo")
}
...
pub fn wrapped_clippy_driver() -> PathBuf {
    let clippy_driver = project()
        ...
        .file("Crabgo.toml", &basic_manifest("clippy-driver", "0.0.1"))
        .file(
            "src/main.rs",
            r#"
            fn main() {
        ...
        .build();
    clippy_driver.crabgo("build").run();
...
impl Execs {
    ...
    /// See [`compare`] for supported patterns.
    ///
    /// This is useful when checking the output of `crabgo build -v` since
    ...
    ///
    /// This is typically used when testing crabgo commands that emit JSON.
    ...
    ///
    /// The list of reasons should be why nightly crabgo is needed. If it is
    /// becuase of an unstable feature put the name of the feature as the reason,
    /// e.g. `&["print-im-a-teapot"]`
    pub fn masquerade_as_nightly_crabgo(&mut self, reasons: &[&str]) -> &mut Self {
        if let Some(ref mut p) = self.process_builder {
            p.masquerade_as_nightly_crabgo(reasons);
        }
        self
    }
...
/// The host triple suitable for use in a crabgo environment variable (uppercased).
...
pub trait TestEnv: Sized {
    fn test_env(mut self) -> Self {
        // In general just clear out all crabgo-specific configuration already in the
...
/// Test the crabgo command
pub trait CrabgoCommand {
    fn crabgo_ui() -> Self;
}

impl CrabgoCommand for snapbox::cmd::Command {
    fn crabgo_ui() -> Self {
        Self::new(crabgo_exe())
...
/// Some CI setups are much slower then the equipment used by Crabgo itself.
