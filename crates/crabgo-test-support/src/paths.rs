...
// name.[*] Instead, we used the `crabgo-test-macro` crate to automatically
...
pub fn root() -> PathBuf {
    let id = TEST_ID.with(|n| {
        n.borrow().expect(
            "Tests must use the `#[crabgo_test]` attribute in \
