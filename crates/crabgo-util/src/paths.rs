...
pub fn dylib_path_envvar() -> &'static str {
    if cfg!(windows) {
        ...
        // penalty starting in 10.13. Crabgo's testsuite ran more than twice as
        // slow with it on CI.
        "DYLD_FALLBACK_LIBRARY_PATH"
    } else {
        "LD_LIBRARY_PATH"
...
/// fail, or on Windows returns annoying device paths. This is a problem Crabgo
/// needs to improve on.
...
pub fn mtime_recursive(path: &Path) -> Result<FileTime> {
    ...
    let max_meta = walkdir::WalkDir::new(path)
        ...
        .filter_map(|e| match e {
            Ok(e) => Some(e),
            Err(e) => {
                // Ignore errors while walking. If Crabgo can't access it, the
            ...
            if e.path_is_symlink() {
                ...
                let sym_meta = match std::fs::symlink_metadata(e.path()) {
                    ...
                    Err(err) => {
                        ...
                        // race with unlinking?). Regardless, if Crabgo can't
                ...
                match e.metadata() {
                    ...
                    Err(err) => {
                        // Can't access the symlink target. If Crabgo can't
...
impl<'a> PathAncestors<'a> {
    fn new(path: &'a Path, stop_root_at: Option<&Path>) -> PathAncestors<'a> {
        ...
        PathAncestors {
            current: Some(path),
            //HACK: avoid reading `~/.crabgo/config` when testing Crabgo itself.
            stop_at,
        }
    }
}
...
fn _link_or_copy(src: &Path, dst: &Path) -> Result<()> {
    ...
    let link_result = if src.is_dir() {
        ...
        // `crabgo run` with SIGKILL. Copying seems to avoid the problem.
        // This shouldn't affect anyone except Crabgo's test suite because
        // it is very rare, and only seems to happen under heavy load and
        // rapidly creating lots of executables and running them.
        // See https://github.com/rust-lang/crabgo/issues/7821 for the
        // gory details.
        ...
        if cfg!(target_os = "macos") {
            ...
            // See https://github.com/rust-lang/crabgo/issues/10060 for the details
            fs::copy(src, dst).map(|_| ())
        } else {
...
/// supported on network filesystems. For the current uses in Crabgo, this is a
...
pub fn create_dir_all_excluded_from_backups_atomic(p: impl AsRef<Path>) -> Result<()> {
    ...
    // we would risk a situation where crabgo is interrupted right after the directory
    ...
    // we can infer from it's another crabgo process doing work.
...
# This file is a cache directory tag created by crabgo.
...
fn exclude_from_time_machine(path: &Path) {
    ...
    // doesn't prevent Crabgo from working
}
