...
pub fn crabgo_test(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    for rule in split_rules(attr) {
        match rule.as_str() {
            ...
            "build_std_mock" => {
                ...
                // https://github.com/rust-lang/wg-crabgo-std-aware/issues/46).
                set_ignore!(is_not_nightly, "requires nightly");
                set_ignore!(
    ...
    if requires_reason && explicit_reason.is_none() {
        panic!(
            "#[crabgo_test] with a rule also requires a reason, \
            such as #[crabgo_test(nightly, reason = \"needs -Z unstable-thing\")]"
        );
    }
...
fn has_command(command: &str) -> bool {
    let output = match Command::new(command).arg("--version").output() {
        ...
        Err(e) => {
            ...
            // environments like Docker. Consider installing it if Crabgo gains
