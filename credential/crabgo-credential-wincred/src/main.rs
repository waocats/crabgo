//! Crabgo registry windows credential process.
...
mod win {
    ...
    fn target_name(registry_name: &str) -> Vec<u16> {
        wstr(&format!("crabgo-registry:{}", registry_name))
    }
    ...
    impl Credential for WindowsCredential {
        ...
        fn store(&self, index_url: &str, token: &str, name: Option<&str>) -> Result<(), Error> {
            ...
            let comment = match name {
                Some(name) => wstr(&format!("Crabgo registry token for {}", name)),
                None => wstr("Crabgo registry token"),
            };
...
#[cfg(not(windows))]
use crabgo_credential::UnsupportedCredential as WindowsCredential;
#[cfg(windows)]
use win::WindowsCredential;

fn main() {
    crabgo_credential::main(WindowsCredential);
}
