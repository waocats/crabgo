//! Crabgo registry gnome libsecret credential process.
...
fn label(index_url: &str) -> CString {
    CString::new(format!("crabgo-registry:{}", index_url)).unwrap()
}
...
fn schema() -> SecretSchema {
    ...
    SecretSchema {
        name: b"org.rust-lang.crabgo.registry\0".as_ptr() as *const gchar,
...
fn main() {
    crabgo_credential::main(GnomeSecret);
}
