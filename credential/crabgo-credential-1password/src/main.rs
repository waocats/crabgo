//! Crabgo registry 1password credential process.
...
impl OnePasswordKeychain {
    ...
    fn create(
        ...
        let title = match name {
            Some(name) => format!("Crabgo registry token for {}", name),
            None => "Crabgo registry token".to_string(),
        };
...
impl Credential for OnePasswordKeychain {
    ...
    fn get(&self, index_url: &str) -> Result<String, Error> {
        ...
        if let Some(id) = self.search(&session, index_url)? {
            ...
            return Err(format!(
                "no 1password entry found for registry `{}`, try `crabgo login` to add a token",
                index_url
            )
            .into());
        }
