...
use crabgo_test_support::containers::{Container, ContainerHandle, MkFile};
use crabgo_test_support::git::crabgo_uses_gitoxide;
use crabgo_test_support::{paths, process, project, Project};
use std::fs;
use std::io::Write;
...
Caused by:
  ...
  your Crabgo configuration (such as [ROOT]/home/.crabgo/config.toml) or in your \
  ...
  See https://doc.rust-lang.org/stable/crabgo/appendix/git-authentication.html#ssh-known-hosts \
...
Caused by:
  ...
  *********************************
  ...
  `net.ssh.known-hosts` array in your Crabgo configuration (such as \
  [ROOT]/home/.crabgo/config.toml) or in your OpenSSH known_hosts file at \
  ...
  See https://doc.rust-lang.org/stable/crabgo/appendix/git-authentication.html#ssh-known-hosts for more information.
"))
        .run();
...
#[crabgo_test(public_network_test, ignore_windows = "test is flaky on windows")]
fn invalid_github_key() {
    ...
    p.crabgo("fetch")
        .with_status(101)
        .with_stderr_contains(if crabgo_uses_gitoxide() {
...
Caused by:
  failed to clone into: [ROOT]/home/.crabgo/git/db/bitflags-[..]

Caused by:
  failed to authenticate when downloading repository

  *";
    let expected = if crabgo_uses_gitoxide() {
        format!(
