//! Tests for checking behavior of old crabgos.
//!
...
//!
//! ```console
//! crabgo test --test testsuite -- old_crabgos --nocapture --ignored
//! ```

use crabgo::CrabgoResult;
use crabgo_test_support::paths::CrabgoPathExt;
use crabgo_test_support::registry::{self, Dependency, Package};
use crabgo_test_support::{crabgo_exe, execs, paths, process, project, rustc_host};
...
// This is a test for exercising the behavior of older versions of crabgo with
...
//   syntax.
#[ignore = "must be run manually, requires old crabgo installations"]
#[crabgo_test]
fn new_features() {
    let registry = registry::init();
    if std::process::Command::new("rustup").output().is_err() {
        panic!("old_crabgos requires rustup to be installed");
    }
    ...
    for (version, toolchain) in &toolchains {
        ...
        let pkg_version = |pkg| -> Option<Version> {
            let output = tc_process("crabgo", toolchain)
                .args(&["pkgid", pkg])
                .cwd(p.root())
                .exec_with_output()
                .ok()?;
        ...
        // Runs `crabgo build` and returns the versions selected in the lock.
        let run_crabgo = || -> CrabgoResult<ToolchainBehavior> {
            match tc_process("crabgo", toolchain)
                .args(&["build", "--verbose"])
                .cwd(p.root())
                .exec_with_output()
            {
...
fn index_cache_rebuild() {
    ...
    // `-Znamespaced-features` would prevent the new crabgo from seeing those
    ...
    // This version of Crabgo errors on index entries that have overlapping
    // feature names, so 1.0.1 will be missing.
    execs()
        .with_process_builder(tc_process("crabgo", "1.48.0"))
        .arg("check")
        .cwd(p.root())
        .with_stderr(
            "\
