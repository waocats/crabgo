...
fn use_a_spec_to_select() {
    ...
    let foo = git::repo(&paths::root().join("override"))
        .file("Crabgo.toml", &basic_manifest("baz", "0.2.0"))
...
",
        ...
        .run();
    p.crabgo("update -p https://github.com/rust-lang/crates.io-index#bar")
        .with_stderr(
            "\
[UPDATING] `dummy-registry` index
",
...
fn no_override_self() {
    let deps = git::repo(&paths::root().join("override"))
        .file("far/Crabgo.toml", &basic_manifest("far", "0.1.0"))
        .file("far/src/lib.rs", "")
        .file(
            "near/Crabgo.toml",
            r#"
...
fn override_an_override() {
    ...
    let p = project()
        ...
        .file("serde/Crabgo.toml", &basic_manifest("serde", "0.8.0"))
...
fn no_warnings_when_replace_is_used_in_another_workspace_member() {
    ...
    let p = project()
        ...
        .file(
            "second_crate/Crabgo.toml",
            &basic_manifest("second_crate", "0.1.0"),
        )
        .file("second_crate/src/lib.rs", "")
        .file("local_bar/Crabgo.toml", &basic_manifest("bar", "0.1.0"))
