//! Tests for custom crabgo commands and other global command features.
...
use crabgo_test_support::{
    basic_bin_manifest, crabgo_exe, crabgo_process, paths, project, project_in_home,
};
use crabgo_util::paths::join_paths;
...
fn list_commands_with_descriptions() {
    ...
    p.crabgo("--list")
        ...
        // command, indented.
        .with_stdout_contains(
            "    read-manifest        Print a JSON representation of a Crabgo.toml manifest.",
        )
        .run();
}
...
fn list_custom_aliases_with_descriptions() {
    let p = project_in_home("proj")
        .file(
            &paths::home().join(".crabgo").join("config"),
            r#"
            [alias]
            myaliasstr = "foo --bar"
            myaliasvec = ["foo", "--bar"]
...
fn list_dedupe() {
    let p = project()
        .executable(Path::new("path-test-1").join("crabgo-dupe"), "")
        .executable(Path::new("path-test-2").join("crabgo-dupe"), "")
        .build();
...
fn list_command_looks_at_path() {
    let proj = project()
        .executable(Path::new("path-test").join("crabgo-1"), "")
        .build();

    let mut path = path();
    path.push(proj.root().join("path-test"));
    let path = env::join_paths(path.iter()).unwrap();
    let output = crabgo_process("-v --list")
        .env("PATH", &path)
        .exec_with_output()
        .unwrap();
...
fn list_command_handles_known_external_commands() {
    let p = project()
        .executable(Path::new("path-test").join("crabgo-fmt"), "")
        .build();
...
fn list_command_resolves_symlinks() {
    let proj = project()
        .symlink(crabgo_exe(), Path::new("path-test").join("crabgo-2"))
        .build();
...
",
    ...
    // https://github.com/rust-lang/crabgo/issues/5201
    Package::new("crabgo-biuld", "1.0.0")
        .file(
            "src/main.rs",
            r#"
                fn main() {
    ...
    crabgo_process("install crabgo-biuld").run();
    crabgo_process("biuld")
...
fn find_closest_dont_correct_nonsense() {
    crabgo_process("there-is-no-way-that-there-is-a-command-close-to-this")
        .cwd(&paths::root())
        .with_status(101)
        .with_stderr(
            "\
...
<tab>View all installed commands with `crabgo --list`",
        )
        .run();
}
...
fn displays_subcommand_on_error() {
    crabgo_process("invalid-command")
        .with_status(101)
        .with_stderr(
            "\
...
fn override_crabgo_home() {
    ...
    crabgo_process("new foo2").run();

    assert!(paths::root().join("foo2/.git").is_dir());
}

#[crabgo_test]
fn crabgo_subcommand_env() {
    let src = format!(
        r#"
        use std::env;
    ...
    let p = project()
        .at("crabgo-envtest")
        .file("Crabgo.toml", &basic_bin_manifest("crabgo-envtest"))
    ...
    p.crabgo("build").run();
    assert!(p.bin("crabgo-envtest").is_file());
...
#[crabgo_test]
fn crabgo_cmd_bins_vs_explicit_path() {
    // Set up `crabgo-foo` binary in two places: inside `$HOME/.crabgo/bin` and outside of it
    //
    // Return paths to both places
    fn set_up_crabgo_foo() -> (PathBuf, PathBuf) {
        let p = project()
            .at("crabgo-foo")
            .file("Crabgo.toml", &basic_manifest("crabgo-foo", "1.0.0"))
            .file(
                "src/bin/crabgo-foo.rs",
                r#"fn main() { println!("INSIDE"); }"#,
            )
            .file(
                "src/bin/crabgo-foo2.rs",
                r#"fn main() { println!("OUTSIDE"); }"#,
            )
            .build();
        p.crabgo("build").run();
        let crabgo_bin_dir = paths::home().join(".crabgo/bin");
        crabgo_bin_dir.mkdir_p();
        let root_bin_dir = paths::root().join("bin");
        root_bin_dir.mkdir_p();
        let exe_name = format!("crabgo-foo{}", env::consts::EXE_SUFFIX);
        fs::rename(p.bin("crabgo-foo"), crabgo_bin_dir.join(&exe_name)).unwrap();
        fs::rename(p.bin("crabgo-foo2"), root_bin_dir.join(&exe_name)).unwrap();

        (root_bin_dir, crabgo_bin_dir)
    }
...
#[crabgo_test]
fn crabgo_subcommand_args() {
    let p = echo_subcommand();
    let crabgo_foo_bin = p.bin("crabgo-echo");
    assert!(crabgo_foo_bin.is_file());

    let mut path = path();
    path.push(p.target_debug_dir());
    let path = env::join_paths(path.iter()).unwrap();

    crabgo_process("echo bar -v --help")
        .env("PATH", &path)
...
fn closed_output_ok() {
    ...
    let mut p = crabgo_process("--list").build_command();
...
fn subcommand_leading_plus_output_contains() {
    crabgo_process("+nightly")
        .with_status(101)
        .with_stderr(
            "\
error: no such command: `+nightly`

<tab>Crabgo does not handle `+toolchain` directives.
<tab>Did you mean to invoke `crabgo` through `rustup` instead?",
        )
        .run();
}
