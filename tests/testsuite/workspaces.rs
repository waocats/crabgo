...
use crabgo_test_support::{basic_lib_manifest, basic_manifest, git, project, sleep_ms};
use std::env;
use std::fs;
...
fn simple_explicit() {
    ...
    p.crabgo("build").cwd("bar").run();
    assert!(p.bin("foo").is_file());
    assert!(p.bin("bar").is_file());

    assert!(p.root().join("Crabgo.lock").is_file());
    assert!(!p.root().join("bar/Crabgo.lock").is_file());
}
...
fn non_virtual_default_members_build_other_member() {
    ...
    p.crabgo("check --manifest-path bar/Crabgo.toml")
        .with_stderr(
            "[CHECKING] bar v0.1.0 ([..])\n\
             [..] Finished dev [unoptimized + debuginfo] target(s) in [..]\n",
        )
...
fn transitive_path_dep() {
    ...
    assert!(p.root().join("Crabgo.lock").is_file());
    assert!(!p.root().join("bar/Crabgo.lock").is_file());
    assert!(!p.root().join("baz/Crabgo.lock").is_file());
}
...
fn parent_pointer_works() {
    ...
    p.crabgo("build").cwd("bar").run();
    assert!(p.root().join("foo/Crabgo.lock").is_file());
    assert!(!p.root().join("bar/Crabgo.lock").is_file());
}
...
fn invalid_parent_pointer() {
    ...
    p.crabgo("check")
        ...
        .with_stderr(
            "\
error: failed to read `[..]Crabgo.toml`
...
Caused by:
  failed to read `[..]foo/foo/Crabgo.toml`
...
fn cycle() {
    ...
    p.crabgo("check")
        .with_status(101)
        .with_stderr(
            "[ERROR] root of a workspace inferred but wasn't a root: [..]/foo/bar/Crabgo.toml",
        )
        .run();
}
...
fn explicit_package_argument_works_with_virtual_manifest() {
    let p = project()
        ...
        .file("bar/src/main.rs", "fn main() {}");
    let p = p.build();
    p.crabgo("build --package bar").run();
    assert!(p.root().join("Crabgo.lock").is_file());
    assert!(p.bin("bar").is_file());
    assert!(!p.root().join("bar/Crabgo.lock").is_file());
}
...
fn members_include_path_deps() {
    let p = project()
        ...
        .file("p1/src/lib.rs", "")
        .file("p2/Crabgo.toml", &basic_manifest("p2", "0.1.0"))
        .file("p2/src/lib.rs", "")
        .file("p3/Crabgo.toml", &basic_manifest("p3", "0.1.0"))
        .file("p3/src/lib.rs", "");
    let p = p.build();

    p.crabgo("check").cwd("p1").run();
    p.crabgo("check").cwd("p2").run();
    p.crabgo("check").cwd("p3").run();
    p.crabgo("check").run();
...
fn new_warning_with_corrupt_ws() {
    let p = project().file("Crabgo.toml", "asdf").build();
    p.crabgo("new bar")
        .with_stderr(
            "\
[WARNING] compiling this new package may not work due to invalid workspace configuration

failed to parse manifest at `[..]foo/Crabgo.toml`
...
fn rebuild_please() {
    let p = project()
        ...
        .file("lib/Crabgo.toml", &basic_manifest("lib", "0.1.0"))
        .file(
            "lib/src/lib.rs",
            r#"
                pub fn foo() -> u32 { 0 }
    ...
    p.crabgo("run").cwd("bin").run();

    sleep_ms(1000);

    p.change_file("lib/src/lib.rs", "pub fn foo() -> u32 { 1 }");

    p.crabgo("build").cwd("lib").run();
...
fn you_cannot_generate_lockfile_for_empty_workspaces() {
    let p = project()
        ...
        .file("bar/Crabgo.toml", &basic_manifest("foo", "0.1.0"))
        .file("bar/src/main.rs", "fn main() {}");
    let p = p.build();
...
fn workspace_with_transitive_dev_deps() {
    ...
    p.crabgo("test -p bar").run();
}

#[crabgo_test]
fn error_if_parent_crabgo_toml_is_invalid() {
    let p = project()
        .file("Crabgo.toml", "Totally not a TOML file")
        .file("bar/Crabgo.toml", &basic_manifest("bar", "0.1.0"))
        .file("bar/src/main.rs", "fn main() {}");
    let p = p.build();
...
fn relative_path_for_root_works() {
    let p = project()
        ...
        .file("src/main.rs", "fn main() {}")
        .file("subproj/Crabgo.toml", &basic_manifest("subproj", "0.1.0"))
        .file("subproj/src/main.rs", "fn main() {}");
    let p = p.build();

    p.crabgo("check --manifest-path ./Crabgo.toml").run();

    p.crabgo("check --manifest-path ../Crabgo.toml")
        .cwd("subproj")
        .run();
}
...
fn test_in_and_out_of_workspace() {
    ...
    assert!(p.root().join("ws/Crabgo.lock").is_file());
...
fn test_path_dependency_under_member() {
    let p = project()
        ...
        .file("foo/bar/Crabgo.toml", &basic_manifest("bar", "0.1.0"))
        .file("foo/bar/src/lib.rs", "pub fn f() { }");
    let p = p.build();

    p.crabgo("check").cwd("ws").run();

    assert!(!p.root().join("foo/bar/Crabgo.lock").is_file());
    assert!(!p.root().join("foo/bar/target").is_dir());

    p.crabgo("check").cwd("foo/bar").run();
...
fn excluded_default_members_crate_glob() {
    let p = project()
        ...
        .file("bar/quux/Crabgo.toml", &basic_manifest("quux", "0.1.0"))
    ...
    p.crabgo("build").cwd("bar/quux").run();
...
fn glob_syntax() {
    ...
    p.crabgo("build").cwd("crates/bar").run();
    assert!(p.bin("foo").is_file());
    assert!(p.bin("bar").is_file());

    p.crabgo("build").cwd("crates/baz").run();
    assert!(p.bin("foo").is_file());
    assert!(p.bin("baz").is_file());

    p.crabgo("build").cwd("crates/qux").run();
    assert!(!p.bin("qux").is_file());

    assert!(p.root().join("Crabgo.lock").is_file());
    assert!(!p.root().join("crates/bar/Crabgo.lock").is_file());
    assert!(!p.root().join("crates/baz/Crabgo.lock").is_file());
    assert!(p.root().join("crates/qux/Crabgo.lock").is_file());
}
...
Caused by:
  failed to read `[..]foo/crates/bar/Crabgo.toml`
...
///
/// Ideally, once we solve rust-lang/crabgo#3620, then a single Crabgo build at the top level
/// will be enough.
#[crabgo_test]
...
fn dont_recurse_out_of_cargo_home() {
    let git_project = git::new("dep", |project| {
        project
            .file("Crabgo.toml", &basic_manifest("dep", "0.1.0"))
            .file("src/lib.rs", "")
            .file(
                "build.rs",
                r#"
                    ...
                    fn main() {
                        let crabgo = env::var_os("CRABGO").unwrap();
                        let crabgo_manifest_dir = env::var_os("CRABGO_MANIFEST_DIR").unwrap();
                        let output = Command::new(crabgo)
                            .args(&["metadata", "--format-version", "1", "--manifest-path"])
                            .arg(&Path::new(&crabgo_manifest_dir).join("Crabgo.toml"))
                            .output()
                            .unwrap();
    ...
    p.crabgo("check")
        .env("CRABGO_HOME", p.root().join(".crabgo"))
        .run();
}
...
fn include_and_exclude() {
    ...
    assert!(p.root().join("target").is_dir());
    assert!(!p.root().join("foo/target").is_dir());
    p.crabgo("build").cwd("foo/bar").run();
    assert!(p.root().join("foo/bar/target").is_dir());
}
*/
...
fn cargo_home_at_root_works() {
    ...
    p.crabgo("check").run();
    p.crabgo("check --frozen").env("CRABGO_HOME", p.root()).run();
}
...
fn relative_rustc() {
    ...
    p.crabgo("build").env("RUSTC", &file).run();
}
...
fn ws_warn_path() {
    ...
    p.crabgo("check")
        .with_stderr_contains("[WARNING] [..]/foo/a/Crabgo.toml: the crabgo feature `edition`[..]")
        .run();
}
...
Caused by:
  failed to read `[..]foo/x/Crabgo.toml`
...
Caused by:
  failed to read `[..]foo/bar/baz/Crabgo.toml`
...
fn simple_primary_package_env_var() {
    let is_primary_package = r#"
        ...
        fn verify_primary_package() {{
            assert!(option_env!("CRABGO_PRIMARY_PACKAGE").is_some());
        }}
    "#;
